...
/**
 * Utility class for handling arrays of unknown array types (i.e. when the array is of type Object,
 * Object[], etc...).
 * 
 * @author jeikenberry
 */
public final class ArrayTaint {
  ...
  /**
   * Called prior to any array accesses to ensure that the index supplied had not been overflowed or
   * underflowed.
   * 
   * @param idx int of the array index.
   * @param idxTaint int of the taint associated with idx.
   * @param storedObj Object we are storing into the array (if store instruction), otherwise null.
   * @return int of the index.
   * @throws IOException
   */
  public final static void store(Object[] array, int idx, int idxTaint, Object value)
      throws IOException {
  ...
  /**
   * Called prior to any new array on any non-primitive type to ensure that the length had not
   ...
   * @throws IOException
   */
  public final static int validateNewArrayLength(int len, int len_t, int dimension)
      throws IOException {
  ...
  /**
   * Takes an object and constructs an unknown taint array from that object (provided that the
   * object is a primitive array).
   * 
   * @param obj the array.
   * @return a new taint array representing obj of unknown taint, or obj if obj is not a primitive
   *         array.
   */
  public final static Object toTaintArray(Object obj) {
    ...
    /*
     * FIXME: Technically we need to perform this also on Object[][], Object[][][], etc... but let's
     * just leave this for now and assume it doesn't really happen in the real world.
     * 
     * if (objClass == Object[].class) { // this object array may contain a primitive array as an
     *                                   // element, so let's call toTaintArray() on each of it's
     *                                   // elements.
  ...
  /**
   * Takes an object and obtains the value array from that object (provided that the object is
   * indeed a taint wrapped primitive array type).
   * 
   * @param obj
   * @return the value array from obj, or obj if obj is not a taint wrapped primitive array).
   */
  public final static Object toValueArray(Object obj) {
    ...
    /*
     * FIXME technically we need to perform this also on Object[][], Object[][][], etc... but let's
     * just leave this for now and assume that it doesn't really happen in the real world.
     * 
     * if (objClass == Object[].class) { // this object array may contain a primitive array as an
     *                                   // element, so let's call toValueArray() on each of it's
     *                                   // elements.
     *   Object[] arr = (Object[]) obj;
     *   for (int i = 0; i < arr.length; i++) {
