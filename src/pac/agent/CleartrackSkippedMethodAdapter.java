...
/**
 * <P>
 * This is essentially an instrumented method with the original bytecode for the method body. In
 * otherwords, it's the original method but with the instrumented method descriptor. Taint is
 * stripped from the parameters, and unknown taint is added to return values that expect taint.
 * </P>
 * 
 * <P>
 * This class is used for both dangerous classes, and also situations with abstract methods and
 * interfaces (i.e. when instrumented definitions need to exist in non- instrumented classes).
 * </P>
 * 
 * @author jeikenberry
 */
public class CleartrackSkippedMethodAdapter extends MethodNode {
  ...
  /**
   * Maps the original array register to a new register of where the value array will be extracted.
   */
  ...
  public CleartrackSkippedMethodAdapter(String owner, MethodNode methodNode) {
    ...
    reg = isStatic ? 0 : 1;
    for (int i = 0; i < argTypes.length; i++) {
      if (AsmUtils.isPrimitiveArrayType(argTypes[i])
          || AsmUtils.canBePrimitiveArrayType(argTypes[i])) {
  ...
  public void visitCode() {
    ...
    for (int i = 0; i < argTypes.length; i++) {
      ...
      if (primArrType != null) {
        ...
        String arrayTaintClass =
            primArrType.getSort() == Type.OBJECT ? primArrType.getInternalName()
        ...
        if (argTypes[i].getSort() == Type.ARRAY) // Need to cast back to array of objects.
      ...
      case Opcodes.ARETURN:
        ...
        if (primArrType != null) {
          ...
          super.visitMethodInsn(Opcodes.INVOKESTATIC, "pac/wrap/ArrayTaint", "toTaintArray",
              "(Ljava/lang/Object;)Ljava/lang/Object;", false);
          if (returnType.getSort() == Type.ARRAY) // Need to cast back to array of objects.
