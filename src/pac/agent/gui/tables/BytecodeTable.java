...
public class BytecodeTable extends JTable implements MouseListener {
  ...
  private TableCellEditor[] cellEditors = new TableCellEditor[] {null, new LabelCellEditor(),
      new OpcodeCellEditor(), new InstructionCellEditor()};

  private TableCellRenderer[] cellRenderers =
      new TableCellRenderer[] {new OffsetCellRenderer(), new InstructionCellRenderer()};
  ...
  private void resize(int maxJumpLevels) {
    for (int i = 0; i < getColumnCount(); i++) {
      ...
      for (int r = 0; r < getRowCount(); r++) {
        ...
        Component comp = renderer.getTableCellRendererComponent(BytecodeTable.this,
  ...
  /**
   * @return true iff changes have been made to the underlying MethodNode's bytecode.
   */
  ...
  /**
   * Makes the row at the specified index visible, by adjusting the scroll's viewport. The viewport
   * is left unmodified if either row is negative or it is already visible in the viewport.
   * 
  ...
  private void setVisibleRow(int row) {
    if (row < 0)
      return;
    // Get the visible region, and the region of the newly selected row.
  ...
  public void mouseClicked(MouseEvent e) {
    ...
    if (e.getClickCount() == 2 && selected >= 0) {
      ...
      // Acquire the row for the label to jump to, and select that row.
  ...
  public void paintComponent(Graphics g) {
    ...
    OffsetCellRenderer renderer =
        (OffsetCellRenderer) getCellRenderer(getModel().getRowCount() - 1, 0);
    int start = renderer.getFontMetrics(renderer.getFont())
        .stringWidth("" + (getModel().getRowCount() - 1));
