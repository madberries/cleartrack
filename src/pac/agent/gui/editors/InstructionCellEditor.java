...
/**
 * CellEditor for editing AbstractInsnNode objects that have opcodes >= 0.
 * So, for example, this editor does not support LabelNode.  For LabelNode,
 * you should use LabelCellEditor.
 * 
 * @author jeikenberry
 */
public class InstructionCellEditor extends AbstractCellEditor implements TableCellEditor, LabelChangeListener {
    ...
    /**
     * maintains mapping of LabelNode objects to new LabelNode 
     * objects (for cloning instructions).
     */
    private static final Map<LabelNode, LabelNode> labelMap = new HashMap<LabelNode, LabelNode>();

    /**
     * maintains mapping of LabelNode objects to it's label name (names
     * are assign lexicologically).
     */
    private static final Map<LabelNode, String> labelNames = new HashMap<LabelNode, String>();
    /**
     * maintains mapping of LabelNode objects to their corresponding
     * color.
     */
    ...
    /**
     * Cache of all the types of editor components supported by this
     * cell editor.
     */
    ...
    public InstructionCellEditor() {
        try {
            components = new InstructionComponent[] { new IIncComponent(), new VarComponent(), new TypeComponent(),
                    new NewarrayComponent(), new IntComponent(), new LdcComponent(), new FieldComponent(),
                    new MethodComponent(), new JumpComponent(), new LabelComponent() };
        } catch (ParseException e) {
            // I don't believe this exception is ever thrown,
            // but let's exit just in case.
            e.printStackTrace();
            System.exit(1);
        }

        // Bind each component with their respective ActionListener
        // implementation.
    ...
    public void labelAdded(LabelChangeEvent e) {
        // update the label maps and the components that depend
        // on LabelNode objects
