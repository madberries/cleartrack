...
/**
 * CellEditor for editing LabelNode objects (not instructions that happen
 * to contain LabelNode objects, i.e. JumpInsnNode's).  The component 
 * behind this editor is simply a checkbox.  By checking the box, a new
 * label is either added or removed.  This will trigger a LabelChangeEvent,
 * which will generate a new label name for the newly added LabelNode.
 * 
 * @author jeikenberry
 */
public class LabelCellEditor extends AbstractCellEditor implements TableCellEditor {
    ...
    public Component getTableCellEditorComponent(JTable table, Object value, boolean isSelected, int row, int column) {
        if (value instanceof LabelNode) {
            ...
            boolean enabled = !((BytecodeTableModel) ((BytecodeTable) table).getModel()).isReferenced(label);
