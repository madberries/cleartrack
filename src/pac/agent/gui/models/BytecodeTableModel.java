...
/**
 * The main model for manipulating MethodNode objects.  This maintains information
 * about the instructions (excluding instructions whose opcode is < 0).  It will
 * also verify on each update to the model, as well as maintain an ordering of the
 * branches contained within the method.
 * 
 * @author jeikenberry
 */
public class BytecodeTableModel extends AbstractTableModel {
    ...
    /**
     * The analyzer object for both verifying and producing stack
     * frame information.
     */
    ...
    /** 
     * Mapping of all LabelNode objects to the first "real" instruction
     * (i.e. instruction whose opcode is > 0).
     */
    private Map<LabelNode, AbstractInsnNode> labelToInsnMap;

    /**
    * If there are multiple labels in a row without actual instructions,
    * they will be hidden from the user.  But, if there happens to be a
    * jump to this hidden label, then we must obtain the one that is
    * visible.  This hashmap maps hidden nodes to the one that is visible.
    */
    ...
    /**
     * List of row indices where index i refers to the row of the
     * first instruction at the i+1-th LabelNode in labelToInsnMap.
     */
    private List<Integer> numOfInstructions;

    /**
     * The class name of the ClassNode containing the current 
     * MethodNode object.
     */
    ...
    /**
     * The row of the verify error.  If we were unable to determine
     * a row number this value will be -2.  If there was no verify
     * error, then this number will be -1.
     */
    private int errorRow;

    /**
     * The verify error message, or null if there was no verify
     * error.
     */
    private String errorMsg;

    /**
     * boolean for determining whether changes have been made to
     * the underlying MethodNode object.
     */
    private boolean modified;

    /**
     * Stack frames of the current method node (where the frames of
     * non-negative instruction opcodes have been stripped away).
     */
    private Frame<BasicValue>[] frames;

    /**
     * Create a new ByteCodeTableModel with the supplied ClassNode
     * object.  This will not load a MethodNode into the model but
     * initialize the model to verify over classNode.
     * 
    ...
    /**
     * Loads methodNode into the model and triggers a TableChanged event.  This
     * is like reloadMethods, but will fire a label removed LabelChangeEvent on
     * all labels loaded by the preceeding method.
     * 
    ...
    /**
     * Reloads the current underlying MethodNode object into the model
    ...
    protected void reloadMethod(boolean modified) {
        // Reset all state left behind from the preceeding method
        // node.
        ...
        try {
            // We haven't computed stack and local size, so let's
            // assume some really large value...
        ...
        for (; curNode != null; curNode = curNode.getNext()) {
            if (curNode instanceof LabelNode) {
                ...
                if (!labels.contains(label)) {
                    // We've encountered a new label, so add this to the model
                    // and fire a LabelChangeEvent.
                    labels.add(label);
        ...
        jumps.clear();
        for (curNode = methodNode.instructions.getFirst(); curNode != null; curNode = curNode.getNext()) {
            if (curNode instanceof JumpInsnNode) {
                ...
                if (fromRow < 0 || gotoRow < 0) {
                    // TODO we should investigate why one of these offsets
                    // would be negative... perhaps we encounter multiple
                    // labels in a row or something?
                    System.out.println("INVALID JUMP INSTRUCTION!!");
                    continue;
                }
                Jump newJump = new Jump(fromRow, gotoRow);

                // It's possible that the jump already exists, and we can
                // simply add the offset to the existing branch, in this
                // case.
                ...
                if (!mergedExisting) {
                    // We were unable to find an existing jump, so join the new
                    // jump with each root jump, until we are able to find an
                    // intersecting jump.
    ...
    /**
     * Get the set of all branches of this model's underlying
     * MethodNode.
     * 
     * @return Set&lt;Jump&gt;
     */
    ...
    /**
     * @return The row number containing the verify error, -1 if there
     *   was no verify error, or -2 if there was a verify error but the
     *   verifier was unable to determine the line number.
     */
    ...
    public int getRowOf(AbstractInsnNode insnNode) {
        ...
        // If it's a label, it could possible be an alias and so we
        // need to ensure we grab the actual label node.
        ...
        for (Entry<LabelNode, AbstractInsnNode> entry : labelToInsnMap.entrySet()) {
            ...
            if (insnNode == entry.getKey())
                return lastNum; // the instruction node was found (and it 
                                // happens to be a label), so return the value
                                // that was pulled from the numOfInstructions set.
                                // It's a label, no need to check the instructions that come after
            if (!isLabel) {
    ...
    public Object getValueAt(int rowIndex, int columnIndex) {
        ...
        for (Entry<LabelNode, AbstractInsnNode> entry : labelToInsnMap.entrySet()) {
            ...
            if (lastNum == rowIndex) {
                // The instruction is the first one indicated by the labelToInsnMap.
                // So, if the column is the label column, then chose the key (i.e.
                // LabelNode).  Otherwise, choose the first instruction immediately
                // following this LabelNode.
                if (columnIndex == 1)
                    return entry.getKey();
                else
    ...
    public void setValueAt(Object aValue, int rowIndex, int columnIndex) {
        ...
        case 1: { // LabelNode instructions
            ...
            if (oldValue == null) {
                // There was no label before, so add a new label at rowIndex
                // presuming that the value we are adding is not null.
                if (aValue != null && !methodNode.instructions.contains((AbstractInsnNode) aValue))
                    methodNode.instructions.insertBefore((AbstractInsnNode) getValueAt(rowIndex, columnIndex + 1),
                                                         (AbstractInsnNode) aValue);
                else
                    return;
            } else if (aValue == null) {
                // remove the existing label...
                AbstractInsnNode curNode = ((AbstractInsnNode) getValueAt(rowIndex, columnIndex + 1)).getPrevious();
                while (curNode.getOpcode() < 0) {
            ...
            // If we made it to here then a label has been altered
            // so mark this as modified and reload the method to
            // recalculate flush out the old labels and recompute
            // the labels/branches for methodNode.
            modified = true;
        ...
        case 2: { // Opcode >= 0 instructions
            ...
            if (insnNode.getOpcode() == opcode)
                return;
            // Insert the new instruction relative to the instruction we will
            // delete.
            switch (opcode) {
            ...
            case Opcodes.IFNONNULL:
                Iterator<Entry<LabelNode, AbstractInsnNode>> entries = labelToInsnMap.entrySet().iterator();
        ...
        case 3: { // The operand of the instruction
            ...
            if (insnNode == null) // Ignore null values
                return;
            // Non-jumps are clone so we must insert the new (cloned) instruction, and
            // remove the old one.
        ...
        // If we get here then the model has been altered, so reload the
        // MethodNode (setting modified to true), and issue a TableChangeEvent.
        reloadMethod(true);
        fireTableDataChanged();
    }
    ...
    /**
     * Determines if a LabelNode has been referenced in either a
     * jump, switch, local variable table, or exception table.
     * 
    ...
    public String toHtml() {
        String header = StringEscapeUtils.escapeHtml(className + "." + methodNode.name + methodNode.desc);
