 ...
 * overlapping edges. A jump can have multiple offsets that we brance from, and these edges do
 * overlap to produce a neat representation of the jumps.
 * 
 * @author jeikenberry
 */
public class Jump {
  ...
  /**
   * All child jumps (this either intersect the parent jump, or are perfectly contained in the
   * parent jump).
   */
  ...
  /**
   * The supplied jump will merge with this, iff the two jumps branch to the same instruction.
   * 
   * @param jump Jump
  ...
  /**
   * Joins this jump with the appropriate sub-jump, presuming that the jump intersects.
   * 
   * @param jump Jump
  ...
  /**
   * Draws a branch onto table at a particular level number and at the supplied offset relative to
   * the instruction number column (in pixels). The level represents the ordering in which the
   * branches are drawn to the table (lower comes first). This is to prevent overlap of branches.
   * 
  ...
  public void draw(Graphics2D g2, BytecodeTable table, int level, int offset) {
    ...
    for (int from : offsets) {
      ...
      // FIXME Apparently clicking on a cell will only cause a repaint on that row.
      // Color color;
      ...
      // Overlapping with anti-aliasing on can cause some weird darkening effects, so let's prevent
      // the overlap.
      if (first) {
