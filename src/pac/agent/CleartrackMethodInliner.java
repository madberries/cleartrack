...
/**
 * Dynamic-agent used to inline wrapped methods annotated with the InstrumentationMethod annotation
 * (when inline is set to true). This class is necessary for reflection to work properly without
...
public class CleartrackMethodInliner extends ClassNode {
  ...
  /**
   * Clones the MethodNode represented by methodCall and returns the cloned method. We need to
   * operate on the clone, since there may be many invocations of a single MethodNode.
   * 
  ...
  public static MethodNode createInlinedMethod(String actualOwner, MethodInsnNode methodCall) {
    ...
    MethodNode inlinedMethod = new MethodNode(Opcodes.ASM5, methodToInline.access,
        getInlinedMethodName(actualOwner, methodCall), methodToInline.desc,
        methodToInline.signature, methodToInline.exceptions == null ? null
            : methodToInline.exceptions.toArray(new String[0]));
    synchronized (methodToInline) {
      // Since multiple copies can be made, make sure we reset the labels such that stackmaps can be
  ...
  public static String getInlinedMethodName(String actualOwner, MethodInsnNode methodCall) {
    return actualOwner.replace('/', '_').replaceAll("\\$", "__") + "__" + methodCall.name
        + "_inlined";
  }
