 ...
 * org.objectweb.asm.tree.analysis.SimpleVerifier}).  The problem with
 * ASM's verifier is that it relies on all of the classes to be loaded
 * into the relevant {@code ClassLoader}.  We circumvent that by handling
 * all class relations via the {@code ClassHierarchy} that is constructed
...
public class CleartrackVerifier extends BasicVerifier {
     ...
     *            if the class that is verified is an interface.
     */
    public CleartrackVerifier(final Type currentClass, final Type currentSuperClass, final boolean isInterface) {
        ...
        if (!v.equals(w)) {
            ...
            if (tSort == Type.OBJECT || tSort == Type.ARRAY) {
                if (uSort == Type.OBJECT || uSort == Type.ARRAY) {
                    ...
                    // if both types are arrays, then the merged type will be some
                    // sort of array type.  unless both types are primitive array
                    // types, in which case the merged type will be Object.
                        ...
                        if (tEleType.getSort() == Type.OBJECT) {
                            if (uEleType.getSort() == Type.OBJECT) {
                                ...
                                if (!eleMerge.equals(objValue)) {
                                    // if the merged type of the array element types is not
                                    // an object, then we can only return an array type of
                                    // this merged type of the array dimensions of both
                                    // types are the same.
                                    if (tDims == uDims)
                                        return toArrayValue(eleMerge, tDims);
                                }
                            } else {
                                // primitive arrays need an extra dimension to be 
                                // compatible with Object array types
                                uDims--;
                            }
                        } else {
                            // primitive arrays need an extra dimension to be 
                            // compatible with Object array types
                            tDims--;
                            // check if both types are primitive arrays, and if so
                            // return Object as the merged type.
                            if (uEleType.getSort() != Type.OBJECT)
                                return objValue;
                        }
