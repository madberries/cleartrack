...
/**
 * This is used by {@code CleartrackTaintClassAdapter} to translate a {@code MethodNode} into one
 * that will be instrumented upon visiting the instructions. The instrumented copy will contain new
 * type signatures that will allow for the new primitive types, as well as instrumented instructions
...
public class CleartrackTaintMethodAdapter extends MethodNode {
  ...
  /** Old param register to new param register map. */
  ...
  public CleartrackTaintMethodAdapter(String owner, int ownerAccess, Frame<BasicValue>[] frames,
      MethodNode methodNode, boolean dynamicMode) {
    super(Opcodes.ASM5, removeVarargs(methodNode.access), methodNode.name,
        toPrimitiveDesc(methodNode.desc),
    ...
    varOffset++;
    // We could end up with verify errors in some cases if we don't take the max of maxLocals and
    // regs (i.e. synchronized native methods). In this case, maxLocals will be zero, and since we
    // add code to synchronize prior to invoking the original native method, we will have
    // potentially stomped over the value of a parameter.
    ...
    if (primType != null) {
      switch (primType.getSort()) {
        case Type.INT:
          ...
          super.visitLocalVariable(name + FIELD_SUFFIX, primType.getDescriptor(), signature, start,
              end, taintIndex);
          break;
      }
    }
  ...
  public void visitVarInsn(int opcode, int var) {
    ...
    if (!isStatic && var == 0) {
      ...
      // If the type of parameter does not line up with the type of store/load, then treat this
      // parameter as a local.
      if (!isParam) {
        Ansi.warn("var %s is not really a type specified by (%s) in %s%s at offset %s", owner, var,
            opcode, originalMethod.name, originalMethod.desc, varOffset);
  ...
  /**
   * This method should only be called on load/store instructions, where var refers to a valid
   * parameter register.
   * 
   * @param opcode of the store/load instruction.
   * @param var register of the store/load.
   * @return true iff the instruction at opcode matches with the respective argument in the
   *         descriptor.
   */
    ...
    switch (opcode) {
      case Opcodes.ANEWARRAY:
        if (!shouldCheckOverflow(opcode)) {
          ...
          visitNumberInsn(0);
          super.visitMethodInsn(Opcodes.INVOKESTATIC, "pac/wrap/ArrayTaint",
              "validateNewArrayLength", "(III)I", false);
        }
  ...
  /**
   * Visit ACMP_EQ and ACMP_NE instructions. We only need to replace this instruction with a call to
   * our wrapper method handling this instruction if the stack at this instructions matches one of
   * the following:
  ...
  private int visitAcmpInsn(int opcode) {
    // Let's be smart about whether we really need to replace the ACMP instruction with a wrapped
    // version. In many cases, this is unnecessary. Fortunately, we only need to look at the top two
    // words on the stack.
    ...
    if (top == null || second == null) {
      super.visitMethodInsn(Opcodes.INVOKESTATIC, "pac/inst/taint/InstructionInstrumentation",
          "acmp", "(Ljava/lang/Object;Ljava/lang/Object;)I", false);
    } else {
      // If the type is a primitive array type, ensure it is only of single dimension.
      Type topPrimArrayType = AsmUtils.toPrimitiveArrayType(top);
      boolean topIsPrimArray =
          topPrimArrayType != null && topPrimArrayType.getSort() == Type.OBJECT;
      boolean topIsObject =
          top.getSort() == Type.OBJECT && top.getInternalName().equals("java/lang/Object");
      boolean topIsString =
          top.getSort() == Type.OBJECT && top.getInternalName().equals("java/lang/String");
      Type secondPrimArrayType = AsmUtils.toPrimitiveArrayType(second);
      boolean secondIsPrimArray =
          secondPrimArrayType != null && secondPrimArrayType.getSort() == Type.OBJECT;
      boolean secondIsObject =
          second.getSort() == Type.OBJECT && second.getInternalName().equals("java/lang/Object");
      boolean secondIsString =
          second.getSort() == Type.OBJECT && second.getInternalName().equals("java/lang/String");

      if (topIsPrimArray) {
        if (secondIsPrimArray) {
          super.visitMethodInsn(Opcodes.INVOKESTATIC, "pac/inst/taint/InstructionInstrumentation",
              "acmp", "(Lpac/wrap/TaintableArray;Lpac/wrap/TaintableArray;)I", false);
        } else if (secondIsObject) {
          super.visitMethodInsn(Opcodes.INVOKESTATIC, "pac/inst/taint/InstructionInstrumentation",
              "acmp", "(Ljava/lang/Object;Lpac/wrap/TaintableArray;)I", false);
        } else {
        ...
        super.visitMethodInsn(Opcodes.INVOKESTATIC, "pac/inst/taint/InstructionInstrumentation",
            "acmp", "(Lpac/wrap/TaintableArray;Ljava/lang/Object;)I", false);
      } else if (topIsString) {
        if (secondIsString) {
          super.visitMethodInsn(Opcodes.INVOKESTATIC, "pac/inst/taint/InstructionInstrumentation",
              "acmp", "(Ljava/lang/String;Ljava/lang/String;)I", false);
        } else if (secondIsObject) {
          super.visitMethodInsn(Opcodes.INVOKESTATIC, "pac/inst/taint/InstructionInstrumentation",
              "acmp", "(Ljava/lang/Object;Ljava/lang/String;)I", false);
        } else {
          // Optimization: no special check needed.
          return opcode;
        }
        ...
        super.visitMethodInsn(Opcodes.INVOKESTATIC, "pac/inst/taint/InstructionInstrumentation",
            "acmp", "(Ljava/lang/String;Ljava/lang/Object;)I", false);
    ...
    // We have altered this ACMP instruction to invoke a wrapped version of this instruction that
    // will take an int of the comparison result. We therefore need to replace the original
    // instruction with IFEQ/IFNE depending on if the original instruction was IF_ACMPEQ/IF_ACMPNE
    // (respectively).
    return opcode == Opcodes.IF_ACMPEQ ? Opcodes.IFEQ : Opcodes.IFNE;
  }

  /**
   * Called to determine whether an overflow check should be inserted. We should not check overflow
   * on ==/!= branch instructions, since it's very unlikely for a number to overflow and the ==/!=
   * no longer return the same result.
   * 
   * @param opcode of the instruction that can overflow.
   * @return true iff we should check overflow for this instruction.
   */
  private boolean shouldCheckOverflow(int opcode) {
    return !(inJdk || opcode == Opcodes.IFEQ || opcode == Opcodes.IFNE
        || opcode == Opcodes.IF_ICMPEQ || opcode == Opcodes.IF_ICMPNE
        || (this.owner.equals("org/junit/internal/MethodSorter$1") && this.name.equals("compare"))
    // Junit has a bug in this method (as in it does not sort correctly). We find this bug!
    );
  }

  @Override
  public void visitLookupSwitchInsn(Label dflt, int[] keys, Label[] labels) {
    super.visitInsn(Opcodes.POP); // Pop the taint off the stack.
  ...
  public void visitInvokeDynamicInsn(String name, String desc, Handle bsm, Object... bsmArgs) {
    ...
    for (int i = 0; i < argTypes.length; i++) {
      ...
      Type stackType =
          stackIdx < 0 || argTypes == null ? null : frames[instOffset].getStack(stackIdx).getType();
      hasTaintToPop |=
          AsmUtils.isPrimitiveType(argTypes[i]) | (AsmUtils.canBePrimitiveArrayType(argTypes[i])
  ...
  public void visitMethodInsn(int opcode, String owner, String name, String desc, boolean itf) {
    ...
    // We need to handle a special case of when the owner is actually a dangerous type or if the
    // owner is a primtive array type.
    if (owner.charAt(0) == '[') { // Method invocations on an array... We need to replace [I with
                                  // IntArrayTaint, for example.
    ...
    } else if (name.equals("clone") && owner.equals("java/lang/Object")
      ...
      if (curInstFrame != null) {
        ...
        if (size >= 1) {
          ...
          if (top != null) {
            switch (top.getSort()) {
              case Type.ARRAY:
                // Check if the array element has a corresponding wrapped taint array type. If so,
                // then replace the owner of the invocation with this class name.
    ...
    // We need to convert between taint types and non taint-tracked types for skipped methods.
    if (skipped) {
      Type[] argTypes = Type.getArgumentTypes(desc);
      ...
      // Hack for code that requires information from the stack, since the stack will now be off by
      // one.
      //
      ...
      // Convert all return types to a tainted type with unknown taint.
      visitToTaintType(Type.getReturnType(desc), UNKNOWN_OPCODE);
    } else {
      // The call is instrumented, so the only thing we need to do is push the Ret object on the
      // stack prior to the invocation, then acquire the taint out of the Ret object and push it
  ...
  private void visitMethodInsnBefore(String owner, String name, String desc, String newDesc) {
    ...
    if (!hierarchy.isJdkClass(this.owner)) {
      // We only want to load configuration desc rules for methods invoked in application methods.
  ...
  private void visitToTaintType(Type origType, int taintOpcode) {
    Type primType = AsmUtils.toPrimitiveType(origType);
    if (primType != null) { // The return type is either a primitive, or primitive array.
      switch (primType.getSort()) {
        case Type.INT:
        ...
        default:
          ...
          super.visitMethodInsn(Opcodes.INVOKESTATIC, arrayTaintType.getInternalName(),
              "toTaintArray", "(" + origType.getDescriptor() + ")" + primType.getDescriptor(),
              false);
      }
      ...
      if (origType.getSort() == Type.ARRAY) { // Need to cast back to array of objects.
  ...
  private void visitToValueType(Type origType, Type typeOnStack) {
    ...
    if (primType == null) { // Must be an Object or Object[].
      if (typeOnStack != null && AsmUtils.isPrimitiveArrayType(typeOnStack)) {
        ...
        super.visitMethodInsn(Opcodes.INVOKESTATIC, arrayTaintType.getInternalName(),
            "toValueArray", "(" + primType.getDescriptor() + ")" + typeOnStack.getDescriptor(),
            false);
      ...
      switch (primType.getSort()) {
        ...
        default:
          Type arrayTaintType = AsmUtils.toWrappedArrayType(origType.getElementType());
          super.visitMethodInsn(Opcodes.INVOKESTATIC, arrayTaintType.getInternalName(),
              "toValueArray", "(" + primType.getDescriptor() + ")" + origType.getDescriptor(),
              false);
      }
    }
  }
  ...
  public void visitFieldInsn(int opcode, String owner, String name, String desc) {
    ...
    // From this point on either the field is a primitive, primitive array, or an object/object[]
    // whose actual type COULD be a primitive array.
    ...
    switch (opcode) {
      ...
      case Opcodes.GETSTATIC: {
        if (!skipped && hierarchy.isConstant(fieldInsnNode)) {
          // Constant fields that are not skipped have taint that do not need updated.
          ...
          if (primType != null && primType.getSort() == Type.INT) {
            ...
            if (expectingObject) {
              ...
              super.visitFieldInsn(Opcodes.PUTFIELD, owner,
                  AsmUtils.getFieldInstName(owner, name, desc),
                  primType == null ? desc : primType.getDescriptor());
              ...
              visitToTaintType(fieldType, UNKNOWN_OPCODE);
              super.visitInsn(Opcodes.DUP);
              super.visitFieldInsn(Opcodes.PUTSTATIC, owner,
                  AsmUtils.getFieldInstName(owner, name, desc),
                  primType == null ? desc : primType.getDescriptor());
      ...
      case Opcodes.PUTSTATIC: {
        ...
        if (skipped) {
          ...
          if (primType == null || primType.getSort() != Type.INT) { // Must be an Object, Object[],
                                                                    // or primitive array.
  ...
  public void visitMultiANewArrayInsn(String desc, int dims) {
    ...
    if (arrayType.getDimensions() == dims && primArrayType != null) {
      // In some weird bytecode, it's possible that the array is a primitive array with a single
      // dimension.
      Type eleType =
          primArrayType.getSort() != Type.ARRAY ? primArrayType : primArrayType.getElementType();
  ...
  /**
   * Simulate an array store by invoking the store() method in the appropriate array taint class.
   * 
   * @param arrayEleType Type of elements in the array.
   */
  private void visitArrayStore(Type arrayEleType) {
    ...
    super.visitMethodInsn(Opcodes.INVOKEVIRTUAL, arrayTaintType.getInternalName(),
        shouldCheckOverflow(Opcodes.ASTORE) ? "store" : "store_noCheck", Type.getMethodDescriptor(
            Type.VOID_TYPE, Type.INT_TYPE, Type.INT_TYPE, arrayEleType, Type.INT_TYPE),
        false);
  }

  /**
   * Simulate an array load by invoking the load() method in the appropriate array taint class.
   * 
   * @param arrayEleType Type of elements in the array.
   */
  private void visitArrayLoad(Type arrayEleType) {
    ...
    super.visitVarInsn(Opcodes.ALOAD, retObjReg);
    super.visitMethodInsn(Opcodes.INVOKEVIRTUAL, arrayTaintType.getInternalName(),
        shouldCheckOverflow(Opcodes.ALOAD) ? "load" : "load_noCheck", Type.getMethodDescriptor(
            arrayEleType, Type.INT_TYPE, Type.INT_TYPE, Type.getType(Ret.class)),
        false);
    super.visitVarInsn(Opcodes.ALOAD, retObjReg);
    super.visitFieldInsn(Opcodes.GETFIELD, "pac/util/Ret", "taint", "I");
  }
  ...
  private static boolean isStream(String className) {
    return hierarchy.isA(className, "java/io/InputStream")
        || hierarchy.isA(className, "java/io/Reader")
        || hierarchy.isA(className, "java/io/OutputStream")
        || hierarchy.isA(className, "java/io/Writer");
  }
  ...
  public void visitInsn(int opcode) {
    ...
    switch (opcode) {
      ...
      case Opcodes.DALOAD:
        ...
        break;
      case Opcodes.BALOAD:
        if (second != null && second.getSort() == Type.ARRAY
            && second.getElementType().getSort() == Type.BOOLEAN)
      ...
      case Opcodes.BASTORE:
        if (third != null && third.getSort() == Type.ARRAY
            && third.getElementType().getSort() == Type.BOOLEAN)
      ...
      /*
       * ISTORE tmp1, DUP2_X1, POP2, ISTORE tmp2, (DADD), ILOAD tmp1, ILOAD tmp2, IOR
       */
      case Opcodes.LADD:
      ...
      case Opcodes.F2I:
        ...
        break;
      case Opcodes.I2B:
        visitOperation(inJdk ? "i2b" : "i2b_app", Type.INT_TYPE.getDescriptor(), 1,
            Type.BYTE_TYPE.getDescriptor());
        break;
      case Opcodes.I2C:
        visitOperation(inJdk ? "i2c" : "i2c_app", Type.INT_TYPE.getDescriptor(), 1,
            Type.CHAR_TYPE.getDescriptor());
        break;
      case Opcodes.I2S:
      ...
      case Opcodes.L2I:
        visitOperation(inJdk ? "l2i" : "l2i_app", Type.LONG_TYPE.getDescriptor(), 1,
          ...
          switch (top.getSort()) {
            case Type.ARRAY:
              ...
              if (arrayClass == null || top.getDimensions() > 1) {
                // We know this will be an actual array, so leave the instruction as is.
                super.visitInsn(opcode);
                super.visitInsn(TRUSTED_OPCODE);
              } else {
                // super.visitMethodInsn(Opcodes.INVOKEVIRTUAL, arrayClass, "length", "()I");
              ...
              break;
            default:
              // We don't know the real type, so call the generic arraylength method.
              super.visitMethodInsn(Opcodes.INVOKESTATIC, "pac/wrap/ArrayTaint", "length",
                  "(Ljava/lang/Object;)I", false);
              super.visitInsn(TRUSTED_OPCODE);
              break;
          }
        } else { // This should never happen, presuming that we calculate the stacks correctly.
          super.visitMethodInsn(Opcodes.INVOKESTATIC, "pac/wrap/ArrayTaint", "length",
              "(Ljava/lang/Object;)I", false);
          super.visitInsn(TRUSTED_OPCODE);
        }
      ...
      case Opcodes.RETURN:
        if (this.name.equals("<init>") && isStream(this.owner)) {
          ...
          for (Type argType : types) {
            if (argType.getSort() == Type.OBJECT && isStream(argType.getInternalName())) {
              ...
              super.visitFieldInsn(Opcodes.GETFIELD, getStreamType(argType.getInternalName()),
                  "ss_socktype", "Lpac/inst/taint/SocketInstrumentation$SocketType;");
          ...
          if (hasStream) {
            super.visitVarInsn(Opcodes.ALOAD, 0);
            super.visitInsn(Opcodes.SWAP);
            super.visitFieldInsn(Opcodes.PUTFIELD, getStreamType(this.owner), "ss_hasUniformTaint",
                "Z");
  ...
  /**
   * Called on binary/unary operations that will mimic the original instruction. The new
   * instructions will propagate taint and calculate overflow/underflow/infinity taint bits.
   * 
   * @param methodName String of the method to be called in the InstructionInstrumentation class.
   * @param typeDesc String of the ASM type desc of the type of operation (i.e. int, long, double,
   *        etc...)
   * @param numOfOperands int of the number of operands for this instruction.
   */
   ...
   *        etc...)
   * @param numOfOperands int of the number of operands for this instruction.
   * @param returnType String of the ASM type desc that will be returned from this operation (i.e.
   *        int, long, double, etc...)
   */
  private void visitOperation(String methodName, String typeDesc, int numOfOperands,
      String returnType) {
    boolean lastOperandIsInt =
        methodName.equals("lushr") || methodName.equals("lshr") || methodName.equals("lshl");
  ...
  public void visitFrame(int type, int nLocal, Object[] local, int nStack, Object[] stack) {
    // No sense adding these, since they'll be recomputed when the class is written.
    instOffset++;
  }
  ...
  /**
   * Called when visiting a stack operation (i.e. POP, POP2, SWAP, DUP, DUP2, etc...). These
   * operations change since taint is interleaved on the stack. So we must consider all possible
   * stack combinations, by looking at the verified stack.
   * 
   * @param opcode
   */
          ...
          default:
            switch (second.getSort()) {
              ...
              default: {
                ...
                super.visitVarInsn(getLoadOpcode(second), val1);
                super.visitVarInsn(Opcodes.ILOAD, taint1);
                // throw new RuntimeException("stack operations on double words not supported yet: "
  ...
  private void visitDupx2(Type top, Type second, Type third) {
      ...
      default:
        switch (second.getSort()) {
          ...
          default:
            ...
            super.visitVarInsn(top.getSort() == Type.FLOAT ? Opcodes.FSTORE : Opcodes.ISTORE,
                valVar1);
            super.visitVarInsn(Opcodes.ISTORE, taintVar2);
            super.visitVarInsn(second.getSort() == Type.FLOAT ? Opcodes.FSTORE : Opcodes.ISTORE,
                valVar2);
            super.visitVarInsn(top.getSort() == Type.FLOAT ? Opcodes.FLOAD : Opcodes.ILOAD,
                valVar1);
            ...
            super.visitVarInsn(second.getSort() == Type.FLOAT ? Opcodes.FLOAD : Opcodes.ILOAD,
                valVar2);
    ...
    throw new RuntimeException("stack operations on double words not supported yet: " + top + ", "
        + second + ", " + third);
  }
      ...
      case Type.LONG:
        switch (second.getSort()) {
          ...
          case Type.ARRAY:
            ...
            super.visitVarInsn(Opcodes.ISTORE, taint1);
            super.visitVarInsn(top.getSort() == Type.LONG ? Opcodes.LSTORE : Opcodes.DSTORE,
                doubleWord);
            super.visitVarInsn(Opcodes.ASTORE, object);
            super.visitVarInsn(top.getSort() == Type.LONG ? Opcodes.LLOAD : Opcodes.DLOAD,
                doubleWord);
            super.visitVarInsn(Opcodes.ILOAD, taint1);
  ...
  private void visitDup2x2(Type top, Type second, Type third, Type forth) {
    ...
    /*
     * DD DAA DAI DIA- DII AAD AID IAD IID AAAA AAAI AAIA AAII AIAA AIAI AIIA AIII IAAA IAAI IAIA
     * IAII IIAA IIAI IIIA IIII
     */
    // DIA
    switch (top.getSort()) {
      case Type.OBJECT:
      ...
      case Type.LONG: {
        switch (second.getSort()) {
          ...
          case Type.LONG: {
            ...
            super.visitVarInsn(top.getSort() == Type.DOUBLE ? Opcodes.DSTORE : Opcodes.LSTORE,
                longVal);
            super.visitVarInsn(Opcodes.ISTORE, longTaint2);
            super.visitVarInsn(second.getSort() == Type.DOUBLE ? Opcodes.DSTORE : Opcodes.LSTORE,
                longVal2);
            super.visitVarInsn(top.getSort() == Type.DOUBLE ? Opcodes.DLOAD : Opcodes.LLOAD,
                longVal);
            super.visitVarInsn(Opcodes.ILOAD, longTaint);
            super.visitVarInsn(second.getSort() == Type.DOUBLE ? Opcodes.DLOAD : Opcodes.LLOAD,
                longVal2);
          ...
          default:
            switch (third.getSort()) {
              ...
              case Type.ARRAY:
                ...
                super.visitVarInsn(third.getSort() == Type.FLOAT ? Opcodes.FSTORE : Opcodes.ISTORE,
                    intVal);
                ...
                super.visitVarInsn(third.getSort() == Type.FLOAT ? Opcodes.FLOAD : Opcodes.ILOAD,
                    intVal);
  ...
  protected static void callUninstrumented(String owner, MethodNode methodNode,
    ...
    // Remove the native access modifier and add instructions to call the original native method on
        ...
        case Type.OBJECT:
          if (AsmUtils.canBePrimitiveArrayType(argTypes[i])) {
            methodNode.instructions
                .add(new MethodInsnNode(Opcodes.INVOKESTATIC, "pac/wrap/ArrayTaint", "toValueArray",
                    "(Ljava/lang/Object;)Ljava/lang/Object;", false));
          }
          break;
        case Type.ARRAY:
          ...
          if (primType != null) {
            ...
            methodNode.instructions
                .add(new MethodInsnNode(Opcodes.INVOKESTATIC, "pac/wrap/ArrayTaint", "toValueArray",
                    "(Ljava/lang/Object;)Ljava/lang/Object;", false));
            methodNode.instructions
                .add(new TypeInsnNode(Opcodes.CHECKCAST, argTypes[i].getInternalName()));
          }
          break;
        case Type.LONG:
        case Type.DOUBLE:
    ...
    methodNode.instructions
        .add(new MethodInsnNode(!isStatic ? Opcodes.INVOKEVIRTUAL : Opcodes.INVOKESTATIC, owner,
    ...
    if (returnOpcode == Opcodes.ARETURN) {
      ...
      if (primType != null) {
        ...
        methodNode.instructions.add(new MethodInsnNode(Opcodes.INVOKESTATIC,
            ...
            "(" + returnType.getDescriptor() + ")" + primType.getDescriptor(), false));
        ...
        methodNode.instructions
            .add(new TypeInsnNode(Opcodes.CHECKCAST, returnType.getInternalName()));
      }
      ...
      methodNode.instructions
          .add(new FieldInsnNode(Opcodes.PUTFIELD, "pac/util/Ret", "taint", "I"));
    }
  ...
  public void visitEnd() {
    // Reset the descriptor for static class block methods, since these cannot obviously take
    // arguments.
    if (this.name.equals("<clinit>")) {
      // Reset the method descriptor back to the original descriptor and initialize Ret variable at
      // the start of the static class block.
      this.desc = "()V";
      InsnList insnList = new InsnList();
      insnList.add(new TypeInsnNode(Opcodes.NEW, "pac/util/Ret"));
      insnList.add(new InsnNode(Opcodes.DUP));
      insnList
          .add(new MethodInsnNode(Opcodes.INVOKESPECIAL, "pac/util/Ret", "<init>", "()V", false));
      ...
      /*
       * FIXME: This isn't exactly right... We shouldn't need to do this for abstract methods since
       * this is corrected for (see references to ClassHierarchy.overridesInstrumentedMethod()). The
       * problem is that some dangerous types extend abstract classes. Consider the following
       * example:
       *
    ...
    if (!inJdk
      ...
      // We need to turn off the synchronized modifier on both methods. Synchronization in the
      // copied method will happen via ReentrantLocks, and the original method will call the copied
      // method since we are in an application method. So the original method will not need to be
      // synchronized.
      ...
      for (AbstractInsnNode insnNode = instructions.getFirst(); insnNode != null; insnNode =
          insnNode.getNext()) {
        switch (insnNode.getOpcode()) {
          ...
          case Opcodes.DRETURN: {
            ...
            insnList.add(new MethodInsnNode(Opcodes.INVOKEVIRTUAL, "pac/util/ThreadMonitor",
                "monitorexit", "(Ljava/lang/Object;)V", false));
            insnList.add(endTry);
      ...
      // Add instructions to enter the lock.S
      InsnList insnList = new InsnList();
      insnList.add(new FieldInsnNode(Opcodes.GETSTATIC, "pac/util/ThreadMonitor", "INSTANCE",
          "Lpac/util/ThreadMonitor;"));
      if (isStatic)
      ...
      insnList.add(new MethodInsnNode(Opcodes.INVOKEVIRTUAL, "pac/util/ThreadMonitor",
          "monitorenter", "(Ljava/lang/Object;)V", false));
      ...
      instructions.add(new MethodInsnNode(Opcodes.INVOKEVIRTUAL, "pac/util/ThreadMonitor",
          "monitorexit", "(Ljava/lang/Object;)V", false));
  ...
  // ********************************************************************************
  // THE METHODS BELOW WILL ONLY BE ADDED AFTER THE INSTRUMENTATION HAS BEEN PROPERLY
  // BOOTSTRAPPED:
  // ********************************************************************************
  ...
  private void instrumentBeforeChecks(pac.config.Desc d, String methodOwner, String methodDesc) {
    ...
    // Insert descriptions precondition to skip check based on a static method (that takes no
    ...
    for (int i = types.length - 1; i >= 0; i--) {
      ...
      if (isInstrumented)
        primType =
            i < 1 || types[i].getSort() != Type.INT ? null : AsmUtils.toWrappedType(types[i - 1]);
      else
        primType = AsmUtils.toWrappedType(types[i]);
      String primClass = primType == null ? null : primType.getInternalName();

      if (isInstrumented && i == types.length - 1) { // Oop the Ret object
        super.visitInsn(Opcodes.DUP_X1);
        super.visitInsn(Opcodes.SWAP);
        super.visitInsn(Opcodes.POP2);
        continue;
      ...
      // Insert instructions to convert all primitives to a wrapped object.
      if (primClass != null) {
        if (isInstrumented) {
          super.visitVarInsn(Opcodes.ALOAD, retObjReg);
          super.visitMethodInsn(Opcodes.INVOKESTATIC, primClass, "valueOf",
              "(" + types[i - 1].getDescriptor() + "ILpac/util/Ret;)L" + primClass + ";", false);
          i--;
        } else {
          super.visitMethodInsn(Opcodes.INVOKESTATIC, primClass, "valueOf",
      ...
      // If this argument has a condition, then insert a condition that will allow the skip to check
      // and pop the arguments back into their place so the method can resume as normal.
      if (conds[k] != null) {
        String methodStr = conds[k].getMethod();
        if (methodStr != null) {
          super.visitMethodInsn(Opcodes.INVOKEVIRTUAL, methodOwner, methodStr,
              "()Ljava/lang/String;", hierarchy.isInterface(Type.getType(methodOwner)));
        }
    ...
    super.visitLdcInsn(d.getCommand()); // Stack size is maxStack + 3 at this point.
    super.visitMethodInsn(Opcodes.INVOKESTATIC, "pac/config/Notify", "run_checks",
        "([Ljava/lang/Object;Ljava/lang/String;)[Ljava/lang/Object;", false);

    // Iterate over types and push the (potentially new) arguments onto the stack.
    k = 0;
    for (int i = 0; i < types.length; i++) {
      if (isInstrumented && i == types.length - 1) { // We must put Ret obj back on the stack
        super.visitInsn(Opcodes.POP);
        super.visitVarInsn(Opcodes.ALOAD, retObjReg);
  ...
  private void instrumentAfterCheck(pac.config.Desc d, String methodOwner) {
    // Acquire the object type from the top of the stack just after the method invocation. If there
    // is no object on top of the stack, then ignore this check. It's possible that an Object can be
    // created and never stored, for example. If the check happened to be on this constructor, we
    // would fail here.
    ...
    // Insert an after condition that operates on either the constructed object (if method was a
    // constructer) or the returned object.
