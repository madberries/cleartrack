...
/**
 * This class represents a class in our hierarchy. This class is not thread-safe, but the only place
 * it really needs synchronization is in ClassHierarchy.process().
 * 
 * @author jeikenberry
 */
public class ClassType implements Serializable, Comparable<ClassType> {
  ...
  /*
   * Be careful to synchronize over the ClassType object before modifying these fields in methods
   * that can be invoked by multiple threads.
   */
  protected InstrumentationClass target;
  protected String name, checksum;
  protected boolean isClass, isCompatibility, inJdk, isSkipped, hasDefaultConstructor,
      hasDuplicatePrimField, hasMain;

  /*
   * These data structures need to be thread-safe, since they can be modified by multiple threads.
   */
  ...
  /**
   * Add instrumentation MethodType objects to this ClassType for all methods in the type ClassType
   * that are instrumented.
   * 
   * @param type ClassType that is either the same as this ClassType, or is the dangerous classType
   *        (and type is the subclass).
   */
  public void addInstrumentedMethods(ClassType type) {
    for (MethodType method : type.methods.toArray(new MethodType[0])) {
      ...
      MethodType newMethod = new MethodType(type, method.access, method.name,
          CleartrackTaintMethodAdapter.toPrimitiveDesc(method.desc), method.signature,
          method.exceptions);
  ...
  protected FieldType process(FieldNode fieldNode) {
    FieldType fieldType =
        new FieldType(this, fieldNode.name, fieldNode.desc, fieldNode.value != null || !isClass,
  ...
  /**
   * Processes the java Field into this class, for ClassTypes that are loaded late into the class
   * hierarchy.
   * 
   * @param field Field
   * @return FieldType of field that was added.
  ...
  protected FieldType process(Field field) {
    FieldType fieldType = new FieldType(this, field.getName(), Type.getDescriptor(field.getType()),
        false, (field.getModifiers() & Modifier.STATIC) == Modifier.STATIC);
    fields.add(fieldType);
    return fieldType;
  }
  ...
  protected MethodType process(MethodNode methodNode) {
    if (methodNode.name.equals("<init>") && methodNode.desc.equals("()V"))
      ...
      hasMain = true;
    MethodType methodType = new MethodType(this, methodNode.access, methodNode.name,
        methodNode.desc, methodNode.signature, methodNode.exceptions);
    methods.add(methodType);
    return methodType;
  }

  /**
   * Processes the java Method into this class, for ClassTypes that are loaded late into the class
   * hierarchy.
   * 
   * @param method Method
   * @return MethodType of method that was added.
  ...
  /**
   * Processes the java Constructor into this class, for ClassTypes that are loaded late into the
   * class hierarchy.
   * 
  ...
  protected MethodType process(Constructor<?> constructor) {
    ...
    if (constructorDesc.equals("()V"))
      hasDefaultConstructor = true;
    MethodType methodType = new MethodType(this, constructor.getModifiers(), "<init>",
        constructorDesc, null, new ArrayList<String>(0));
    methods.add(methodType);
    return methodType;
  }
  ...
  /**
   * Finds the first declared method matching the given name and compatible with the given method
   * descriptor.
   * 
  ...
  /**
   * Gets the method defined in this ClassType matching the given name and compatible with the given
   * method descriptor.
   * 
   * @param name String of the method name to match.
   * @param desc String of the method descriptor to match.
  ...
  /**
   * Compares two descriptors and determines whether one is compatible with the other (i.e. they can
   * both be called using the same arguments).
   * 
  ...
  /**
   * Finds all MethodType objects that should be replaced by the wrapped method refered to by
   * instMethod and matching the InstrumentationMethod target.
   * 
  ...
  protected Set<MethodType> getMethods(Method instMethod, InstrumentationMethod target) {
    ...
    // Acquire the original descriptor from target, or construct it automically from instMethod if a
    // descriptor was not supplied.
    ...
    // Acquire the method name to match from the target, or construct it from instMethod if one was
    // not supplied.
    ...
    //
    // 1) Methods matching the target method annotation will be added as a target all way up the
    //    chain of overriding methods. This is because it is possible that this wrapper refers to a
    //    method that has been overridden in the respective class.
    // 2) If the current class is skipped, but we have a method wrapper that is the instrumented
    //    version of this method. Then add this method as a target, and add a new method type for this.
    //    We do this in the event that we wish to have an instrumented wrapper for some class that is
    //    skipped.
    ...
    while (!methodFound && !parentStack.isEmpty()) {
      ...
      for (MethodType method : current.methods) {
        if (method.name.equals(name) && method.desc.equals(desc)) {
          if (current != this) {
            // Create a dummy method so that instrumentation call lookup is easy.
            MethodType dummyMethod = new MethodType(this, method.access, name, desc,
          ...
          methodFound = true;
          break;
        } else if (!origDesc.equals("") && current.isSkipped && current == this
            && method.name.equals(name) && method.desc.equals(origDesc)) {
          // Add a psuedo-method to this type, so that it may be properly replaced.
          MethodType dummyMethod =
              new MethodType(this, method.access, name, desc, method.signature, method.exceptions);
          isSkipped = dummyMethod.isSkipped = current.isSkipped;
          results.add(dummyMethod);
          methods.add(dummyMethod);
          methodFound = true;
    ...
    // If this is only a shallow search, then we are done.
    if (!target.canExtend())
      return results;
    ...
    // Continue by recursively calling this on all child types.
  ...
  /**
   * Make sure classes come before interfaces, but this is otherwise sorted alphabetically.
   */
  @Override
