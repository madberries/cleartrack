...
/**
 * This class represents a method in our hierarchy. This class is not thread-safe, but the only
 * place it really needs synchronization is in ClassHierarchy.process().
 * 
 * @author jeikenberry
 *
 */
public class MethodType implements Serializable, Comparable<MethodType> {
  ...
  /*
   * TODO: Method objects are not serializable, so we cannot serialize this field, which means that
   * CleartrackInstrumentationAdapter is essentially broken if used dynamically (i.e. as a java
   * agent).
   */
  ...
  MethodType(ClassType owner, int access, String name, String desc, String signature,
  ...
  /**
   * Checks whether this method overrides some parent method, or is overridden by some child method.
   * 
   * @return boolean
   */
  ...
  public void addTarget(Method method, InstrumentationMethod target) {
    ...
    if (targets != null) {
      ...
      // I'm not really sure if these will come in some order, since we are at the whim of the
      // reflections API. So, to be safe, let's use an ordered tree.
  ...
  private Entry<Method, InstrumentationMethod> getTargetEntry(MethodInsnNode method) {
    ...
    outerloop: for (Entry<Method, InstrumentationMethod> entry : targets.entrySet()) {
      // String owner = Type.getType(target.getDeclaringClass()).getInternalName();
