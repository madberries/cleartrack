...
/**
 * Internal structure of the entire class hierarchy that has been processed by the instrumenter.
 * This is used to determine many things about how to instrument or (in many cases) not instrument
 * classes, fields, and methods. This class is thread-safe.
 * 
 * @author jeikenberry
 */
public class ClassHierarchy implements Serializable {
  ...
  /**
   * This cannot be serialized because of the parser generator that generates the parser defining
   * these rules.
   */
  ...
  private ClassHierarchy() {
    ...
    // Read in all methods that should be left uninstrumented and all of the class rules for
    // instrumentation.
  ...
  /**
   * Populates methodMap by parsing the file skipFileName, and loading it into the map mapping the
   * class names of each skipped method to the set of all method names method descriptors that are
   * skipped for that class.
   * 
   * @param methodMap Map&lt;String, Set&lt;String&gt;&gt;
   * @param skipFileName path to the method skip file to load.
   */
  public static void loadSkippedMethods(Map<String, Set<String>> methodMap, String skipFileName) {
    ...
    boolean asAgent = cl == null; // If cl is null,then we must be running with dynamic agent.
  ...
  /**
   * Parses the logic used to determine whether a class should be skipped or not.
   */
  private void loadSkippedClasses() {
    ...
    SkippedClassesParser parser =
        new SkippedClassesParser(this, asAgent ? ClassLoader.getSystemResourceAsStream(fileName)
   ...
   * 
   * @param typeName String of the class name to lookup
   ...
   * 
   * @param methodInsnNode MethodInsnNode
   * @return MethodType
   */
   ...
   * 
   * @param className String of the classname (internal or descriptor).
  ...
  /**
   * Functionally equivalent to t.isAssignableFrom(u), where t and u are Class objects.
   */
  ...
  /**
   * Determines whether a given type should be considered dangerous (i.e. if the JVM has hardcoded
   * field offsets for these types).
   * 
   ...
   * field offsets for these types).
   * 
   * @param type String of the classname in question (internal name or descriptor).
   * @return boolean
   */
  ...
  /**
   * Determines whether we should not instrument a class, as specified by the set of rules in the
   * classes.skip file. Also note, that all array based Object types are left uninstrumented,
   * regardless of whether the underlying type is instrumented or not.
   * 
   * NOTE: This method should only be used internally.
   * 
   * @param className String of the classname in question (internal name only).
   * @return boolean true iff class {@code className} should be skipped.
   */
   ...
   * regardless of whether the underlying type is instrumented or not.
   * 
   * @param classNode ClassNode in question.
   * @return boolean true iff the underlying class in {@code classNode} is skipped.
   */
  ...
  /**
   * Determines whether we should not instrument a field, as determined by a hierarchical search of
   * the declared field. The fieldNode is not instrumented if and only if the declared field
   * representing this node is not instrumented.
   * 
   * @param classNode ClassNode of the owner.
   * @param fieldNode FieldNode of the field.
   * @return boolean true iff the underlying field in {@code fieldNode} and class in
   *         {@code classNode} should be skipped.
   */
   ...
   * representing this node is not instrumented.
   * 
   * @param fieldInsnNode FieldInsnNode
   * @return boolean true iff the underlying field in {@code fieldInsnNode} should be skipped.
   */
  ...
  /**
   * Determines whether we should not instrument a method, as determined by a hierarchical search of
   * the declared method. The methodNode is not instrumented if and only if the declared method
   * representing this node is not instrumented. All methods whose owner is an array type is
   * essentially of type Object and should not be instrumented.
   * 
   * @param classNode ClassNode
   * @param methodNode MethodNode
   * @return boolean true iff the underlying method in {@code methodNode} and class in
   *         {@code classNode} should be skipped.
   */
   ...
   * 
   * @param methodInsnNode methodInsnNode
   * @return boolean true iff the underlying method in {@code methodInsnNode} should be skipped.
   */
  public boolean isSkipped(MethodInsnNode methodInsnNode, boolean inJdk) {
    ...
    ClassType type = types.get(methodInsnNode.owner);
    MethodType method =
        type == null ? null : type.findMethod(methodInsnNode.name, methodInsnNode.desc);
    if (method == null) {
      // It's possible that we could have a method invocation that was never preprocessed (if the
      // method body was not analyzed).
    ...
    if (method.isSkipped) {
      ...
      // Check to see if we added an instrumentation wrapper to a method this is not instrumented.
      // We will still say this is instrumented, since the wrapper will be replaced on this method.
      if (method != null) {
        // We do not perform method replacements on INVOKESPECIAL (other than constructors).
        ...
        // If this method invocation is in the JDK and the wrapper will only be replaced in the
        // application, then this should still be skipped.
  ...
  /**
   * Presume that all of the methods in methods.skip file are there because the method was too
   * large.
   * 
   * @param classNode
   * @param methodNode
   * @return true iff the method body in {@code methodNode} is too large.
   */
  ...
  /**
   * @param child String of the internal name of the child.
   * @param parent String of the internal name of the parent.
   * @return true if and only if child is a direct or indirect subclass of parent.
   */
  public boolean isA(String child, String parent) {
    ...
    if (childType == null) {
      if (parentType == null)
        return false;
      // We may not know the exact type, but at least we know it is of type Object.
  ...
  /**
   * @param className String of the internal name.
  ...
  /**
   * It is entirely possible for a classfile to contain duplicate fields of different types. This is
   * problematic for us, since we are changing the underlying type in the instrumented field.
   * 
   * @param className String of the classname in question (in internal form).
  ...
  /**
   * Determines whether owner overrides an instrumented abstract method compatible with the method
   * descriptor desc and matching the same name as the given method name.
   * 
   * @param owner String of the owner classname.
   * @param name String of the method name.
   * @param desc String of the method descriptor.
   * @return boolean
   */
  ...
  /**
   * Determines whether classNode overrides an instrumented abstract method compatible with
   * methodNode.
   * 
   * @param classNode ClassNode
   * @param methodNode MethodNode
  ...
  /**
   * Determines whether methodInsnNode's owner overrides an instrumented abstract method compatible
   * with methodInsnNode.
   * 
   * @param methodInsnNode MethodInsnNode
   * @return boolean
  ...
  public boolean overridesInstrumentedMethod(MethodInsnNode methodInsnNode) {
    return overridesInstrumentedMethod(methodInsnNode.owner, methodInsnNode.name,
        methodInsnNode.desc);
  }

  /**
   * Obtains all inherited MethodType objects for classNode that inherit an instrumented abstract
   * method, whose method is not defined in classNode. This can happen when a class implements an
   * interface with foo(), and extends a class defining foo().
   * 
   * @param classNode ClassNode
      ...
      for (ClassType next : inherited) {
        for (MethodType method : next.methods) {
          if (method.name.equals("<init>") || method.name.equals("<clinit>")) {
            ...
            if (argTypes.length > 0) {
              ...
              if (lastArg.getSort() == Type.OBJECT
                  && lastArg.getInternalName().equals("pac/util/Ret")) {
                continue;
              }
            }
          }
          ...
          if (type.getMethod(method.name, method.desc) == null) {
            // Get only the first unique method up the chain, for any other method can't be
            // abstract.
            ...
            while (iter.hasNext()) {
              ...
              if (resultMethod.name.equals(method.name) && resultMethod.desc.equals(method.desc)) {
                // Always write over the method that is not skipped and not abstract. we will remove
                // them in the end.
  ...
  /**
   * @param classNode ClassNode
   * @return String of the internal form name of the first class found that has a default
   *         constructor (starting from classNode and walking up).
   */
  ...
  /**
   * This needs to be called once in CleartrackMain prior to the instrumenter running, such that we
   * don't introduce a data race.
   * 
  ...
  /**
   * Loads an instance from a serialized object. This should only be called if being used as a
   * dynamic agent (and called only once).
   * 
  ...
  public static ClassHierarchy loadInstance() throws IOException, ClassNotFoundException {
    try (ObjectInputStream in =
        new ObjectInputStream(ClassLoader.getSystemResourceAsStream("hierarchy.obj"))) {
  ...
  /**
   * Serializes an instance of the ClassHierarchy. This should only be called once the class
   * hierarchy has been constructed in it's entirety (i.e after postProcess() has been called).
   * 
  ...
  /**
   * Processes a single classNode into the ClassHierarchy instance. This needs to be thread-safe,
   * since we are writing/reading to fields and this method can be called by many threads.
   * 
   * @param classNode ClassNode
   * @param inJdk - true if classNode is defined in a JDK jar.
   * @param isCompatibility - true if classNode came from a compatibility jar file (i.e. an
   *        application library jar that we are entirely skipping).
   * @return true if and only if classNode contains a JSR or RET instruction.
   */
  public boolean process(ClassNode classNode, boolean inJdk, boolean isCompatibility,
      String checksum, String jarName) {
    ...
    // Strategy: keep the ClassType whose checksum is alphabetically first, except keep JDK
    // ClassTypes over application ClassTypes.
    ...
    synchronized (types) {
      ...
      if (classType == null) {
        ...
        existingType = true;
        /*
         * FIXME: This could cause a race condition on classType (though it would be very
         * unlikely)... Off the top of my head, I'm not sure how to make this the way it ought to be
         * without introducing a deadlock scenario.
         */
      }
    }
    ...
    synchronized (classType) {
      if (existingType) {
        // If the existing type is a JDK class and the type we are processing is not a JDK class,
        // then keep the existing JDK type.
        if (classType.inJdk && !inJdk) { // prefer JDK over app
          return processMethods(classNode);
        } else if (classType.checksum != null) {
          // Replace the existing ClassType if either the currently processed type is a JDK class
          // and the current type is not or the checksum of this currently processed class comes
          // before the existing types checksum.
      ...
      if (oldClassType != null) {
        synchronized (oldClassType) {
          // We need to first invalidate this child with both it's child types and parent types, so
          // that they may be added again with this new ClassType object.
  ...
  /**
   * Processes a Class object that is loaded into the JVM, but has not been pre-processed into the
   * hierarchy (i.e. instrumentation wrappers that refer to cleartrack classes).
   * 
  ...
  private void processFields(ClassNode classNode, ClassType classType) {
    // Process all of the field types and mark any class that contains duplicate field names of
  ...
  private boolean processMethods(ClassNode classNode) {
    // If the class file version is less than Java 1.5, then this class will not have stack frames
    // and so let's not recompute them.
  ...
  private boolean processMethods(ClassNode classNode, ClassType classType, boolean inJdk) {
    ...
    // Process all of the methods, marking all JUnit test classes and the ClassType if there is a
    // method with a JSR or RET instructions.
    for (MethodNode methodNode : classNode.methods) {
      ...
      // No need to go through the instructions, since we presume that the application has been
      // compiled with Java 1.7. Therefore, there should be no JSR or RET instructions.
      if (hasJsrOrRetInsn)
        continue;

      for (AbstractInsnNode curNode =
          methodNode.instructions.getFirst(); curNode != null; curNode =
          curNode.getNext()) {
        switch (curNode.getOpcode()) {
          case Opcodes.INVOKESTATIC:
          case Opcodes.INVOKEVIRTUAL:
            // Only mark methods as recursive if they are virtual/static methods that share the same
            // name and descriptor.
  ...
  /**
   * Only call this method once all class types have been processed. This will make a second pass
   * over all class types and finalize the methods/fields/classes that we are not instrumenting. We
   * can only know this once the entire heirarchy has been built (hence the second pass). We also
   * need add instrumented methods to all ClassType objects in our hierarchy, so that we can quickly
   * lookup the instrumentation wrappers for these methods.
   * 
   * This does not need to be thread safe, since this is called only once after everything has
   * already been processed by ASM.
   * 
   * @param outputDirFile
   * @param instPackage
   */
  public void postProcess(File outputDirFile, String instPackage) {
    ...
    for (ClassType type : types.values()) {
      // If the type has no parent, then the class definition must be missing. Let's just safely
      // assume the parent is java/lang/Object.
    ...
    // Process all dangerous types into the hierarchy, so that the verifier can work correctly.
    ...
    System.out.printf("Native methods:              jdk = %7d     application = %7d\n",
        nativeMethodsJdk.intValue(), nativeMethodsApp.intValue());
    // System.out.println("Main classes:");
    // for (String mainClass : mainClasses)
    //   System.out.printf(" * %s\n", mainClass);

    // Dynamically load all instrumented classes from their annotation.
    ...
    // Serialize this class hierarchy object as an xml file, for later viewing.
  ...
  /**
   * Takes a classNode annotated with pac.inst.InstrumentationClass, and processes all methods
   * annotated by pac.inst.InstrumentationMethod.
   * 
  ...
  private void addInstrumentationClass(Class<?> classObj) {
    ...
    if (type == null) {
      try {
        // It's possible that this instrumentation class refers to a cleartrack class, which will
        // not have been processed by cleartrack. So in this case, we load this type and it's parent
        // types into the hierarchy (presuming it's on the classpath).
    ...
    for (Method method : classObj.getMethods()) {
      ...
      if (null != target) {
        if (INST_MODIFIERS == (method.getModifiers() & INST_MODIFIERS)) {
          ...
          Ansi.warn("instrumentation method '%s' is either not declared public or not static.",
              null, method);
        }
      }
    }
