...
public class CleartrackInstrumentationAdapter extends ClassNode {
    ...
    /**
     * This is the last method called by ClassReader when directing a
     * ClassVisitor (of which this is a subclass) through the visitation of some
     * bytecode. It can be taken as a signal that the tree model of the class in
     * question has been constructed and is ready for processing.
     */
    @Override
    public void visitEnd() {
        ...
        // We use this set to mark which methods that we have inlined
        // into the class, but not inlined directly into the method.  In
        // other words, for the cases where we directly copy the inlined
        // method into the class.  We need to mark these to ensure that
        // we do not have duplicate methods.
        Set<String> inlinedMethods = new HashSet<String>();

        // Perform our instrumentation processing.
        // instrument all this class's methods
        ListIterator<MethodNode> methodIter = this.methods.listIterator();
        while (methodIter.hasNext()) {
            MethodNode methodNode = methodIter.next();

            // We should method swap on all methods in compatibility
            // classes.
            boolean isInstrumented = isInstrumentedMethod(methodNode);

            // scan through all the opcodes in this method and perform
            // modifications
            ...
            while (null != currentNode) {
                if (currentNode instanceof MethodInsnNode) {
                    ...
                    // Do not insert or replace instrumented calls on super
                    // calls...
                    ...
                    // perform different behaviors depending on the type of the
                    // invocation
                    ...
                    if (null != method) {
                        ...
                        if (null != targets) {
                            for (Entry<Method, InstrumentationMethod> entry : targets.entrySet()) {
                                ...
                                // Determine whether we should skip targets depending
                                // on the instrumentation level...
                                ...
                                if (inline) {
                                    // We are inlining this method, so we need to determine what
                                    // the owner class is by looking at the InstrumentationClass
                                    // annotation of the declaring type.
                                    String key = owner + "." + name + desc;
                                    InstrumentationClass instClass = declaringClass
                                            .getAnnotation(InstrumentationClass.class);
                                    if (instClass == null) {
                                        ...
                                        // Check to see if we've already added an inlined method
                                        // to this class.
                                        if (!inlinedMethods.contains(key)) {
                                            ...
                                            // Check to see if an inlined method could be
                                            // created, or not.
                                            if (inlinedMethod == null) {
                                                Ansi.warn("failed to inline the wrapper method %s.%s%s", this.name,
                                                          owner, name, desc);
                                            } else {
                                                // We cannot actually inline a method directly into some
                                                // calling method, if the method to inline has try-catch
                                                // blocks.  We do not really know where to add the inlined
                                                // catch block in these cases.
                                                if (!inlinedMethod.exceptions.isEmpty()) {
                                                    methodIter.add(inlinedMethod);
                                                    inlinedMethods.add(key);
                                                    owner = this.name;
                                                    name = CleartrackMethodInliner
                                                            .getInlinedMethodName(instClass.value(), methodInsnNode);
                                                } else {
                                            ...
                                            // There is already a method definition for the inlined
                                            // method in this class node, so just update the MethodInsnNode
                                            // instruction to invoke this method.
                                            owner = this.name;
                                ...
                                case REPLACE:
                                    // If we are replacing this constructor call
                                    // with our instrumented
                                    // version, we have to remove any potential
                                    // DUP instructions.
                                    boolean shouldInstrument = true;
                                    if (methodInsnNode.name.equals("<init>")) {
                                        shouldInstrument = removeRelevantDups(methodNode, methodInsnNode);
                                        /*
                                         * TODO: We should address what we
                                         * should do with super() calls...
                                         */
                                    }
                                    ...
                                    break;
                                case INSERT_BEFORE:
                                    MethodInsnNode insertedCallInsn = new MethodInsnNode(Opcodes.INVOKESTATIC, owner,
                                            name, desc, false);
                                    methodNode.instructions.insertBefore(methodInsnNode, insertedCallInsn);
                                    break;
                                case INSERT_AFTER:
                                    insertedCallInsn = new MethodInsnNode(Opcodes.INVOKESTATIC, owner, name, desc,
                                            false);
    ...
    /**
     * Called to inline inlinedMethod into methodNode, replacing methodInsnNode with
     * these instructions.  All returns are essentially converted into GOTO instructions
     * that branch to the end of the inlinedMethod.  Note, that this should not be
     * called if the method you are inlining contains try-catch blocks.
     * 
     * @param methodNode MethodNode to inline into.
     * @param methodInsnNode AbstractInsnNode of the method invocation
     * 	to be inlined.
     * @param inlinedMethod MethodNode that we are inlining.
     * @param instType InstrumentationType of this inline operation. This will 
     * 	determine what to do with methodInsnNode, and how the instructions will
     * 	be inlined.
     * @return LabelNode marking the end of the inlined method.
     */
    private LabelNode inlineMethod(MethodNode methodNode, AbstractInsnNode methodInsnNode, MethodNode inlinedMethod,
        ...
        // Construct a label map mapping inlined method labels to new labels.  This
        // will be needed when we clone the inlined instructions.
        Map<LabelNode, LabelNode> labelMap = new HashMap<LabelNode, LabelNode>();
        for (AbstractInsnNode curNode = inlinedMethod.instructions.getFirst(); curNode != null; curNode = curNode
                .getNext()) {
            if (curNode instanceof LabelNode)
                labelMap.put((LabelNode) curNode, new LabelNode());
        }

        // Process the method body of the inlined instructions (these need to
        // be cloned).  We also need to offset all variables by maxLocals so
        // that there are no collisions.  Returns will turn into a GOTO that
        // jumps to a location after the method body.
        ...
        for (AbstractInsnNode curNode = inlinedMethod.instructions.getFirst(); curNode != null; curNode = curNode
            ...
            case Opcodes.DRETURN:
                ...
                break;
            default:
                AbstractInsnNode clonedNode = curNode instanceof LabelNode ? labelMap.get((LabelNode) curNode)
        ...
        // Append the instructions, remove the method invocation, and update
        // maxLocals.
        instructions.insert(methodInsnNode, endInline);
        instructions.insert(methodInsnNode, methodBody);

        // Add store instructions storing all of the parameter types into new
        // local registers.
    ...
    /**
     * Determines the first free register of methodNode.  We can't rely on maxLocals 
     * being up to date, so we have to calculate this each time.
     * 
    ...
    private int findStartReg(MethodNode methodNode) {
        ...
        // Read in all of the instructions, and ensure that we update
        // startReg such that it's big enough to not overlap with any
        // existing VarInsnNode or IincInsnNode instruction.
    ...
    private boolean isInstrumentedMethod(MethodNode methodNode) {
        ...
        if (lastType.getSort() == Type.OBJECT && lastType.getInternalName().equals("pac/util/Ret"))
            return true;
        // Check if it's an instrumented recursive method.  These
        // methods will have an additional count parameter at the
        // end, after the Ret parameter.
    ...
    /*
     * FIXME: this is an imperfect way of handling DUP instructions. Ideally a
     * verifier would correct for these DUP instructions by analyzing the stack.
     */
    private boolean removeRelevantDups(MethodNode methodNode, MethodInsnNode methodInsnNode) {
         ...
         * 
         * If there is a gap between DUP and NEW instructions, then insert pop
         * after instrumented constructor...
         */
        ...
        } else { // assume that the NEW type does not get DUPed for now
