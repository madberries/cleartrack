...
public class CleartrackMain {
    ...
    public static void main(String[] args) throws ParseException, IOException, URISyntaxException {
        ...
        if (cmdLine.hasOption("h") || cmdLine.hasOption("help")) {
            ...
            System.out.printf("Cleartrack version %s (built by %s on %s)%n", SSVersion.revision, SSBuild.build_user,
                              SSBuild.build_time);
            if (version)
                System.exit(0);
        }
        ...
        if (prefix)
            confinementOff = true; // force confinement off
        ...
        if (verbose) {
            ...
            boolean noExtDirs = extDirs == null || extDirs.length == 0 || Arrays.toString(extDirs).equals("[]");
        ...
        if (quiet) { // be quiet from here on out...
            System.setOut(new PrintStream(new NullOutputStream()));
        }

        MainInstrumenter instrumenter = new MainInstrumenter(outputDirFile, flags, threads, hideProgress, editMode);
        ...
        if (prefix) {
            ...
            // pre-process and instrument the JDK and files specified at the
            // command line...
            instrumenter.instrumentFiles(jdkDir, extDirs, files, outputDirFile, true);
            ...
            //#if bootstrap==false
            if (files.length > 0) {
                // we only need to generate the configuration class file if
                // there is an application to instrument.
                ...
                JarOutputStream configOut = new JarOutputStream(
                        new FileOutputStream(new File(outputDirFile, "ss-config.jar")));
                ...
                JarEntry hierarchyEntry = new JarEntry(
                        "hierarchy.obj");
    ...
    private static Options getCommandLineOptions() {
        ...
        options.addOption("a", "agent", false, "only preprocess classes for the purpose of running as an agent");
        ...
        options.addOption("p", "prefix", false, "prefix packages (intended for libraries required by cleartrack)");
        ...
        options.addOption("i", "interactive", false, "interactively edit bytecode when encountering a verify error");
    ...
    /**
     * Custom instrumenter used to define how classes are to be
     * instrumented.
     * 
     * @author jeikenberry
     */
    private static class MainInstrumenter extends CleartrackInstrumenter {
        ...
        public MainInstrumenter(File outputDir, int flags, int threads, boolean hideProgress, boolean editMode) {
        ...
        protected ClassVisitor instrumentClass(ClassVisitor visitor, String checksum, boolean inJdk) {
            if (isFlagSet(PREFIX)) {
                ...
                if (isFlagSet(CONFINEMENT)) {
                    ...
                    visitor = new CleartrackTaintClassAdapter(new CleartrackInstrumentationAdapter(visitor), editMode,
                            false);
                } else {
        ...
        protected ClassVisitor instrumentSubclass(ClassVisitor visitor) {
            // This will only get called if getInstrumentedSubclass() returns
            // a non-null value...
