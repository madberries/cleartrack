...
/**
 * This thread will monitor all java threads and interrupt any misbehaved threads. It works by
 * spawning a very low-priority daemon thread that queries the JVM for possible deadlock conditions.
 * If a deadlock is found, we simply interrupt an arbitrary thread (in the cycle of the deadlocked
 * threads). This interrupted thread will then throw a CleartrackException at the point where the
 * lock() was attempted. All other threads will continue as normal.
 * 
 * In addition to this, the daemon will also perform memory checks when the heap is over 75% full.
 * If that is the case, the thread that has allocated the most bytes will be interrupted (provided
 * that it is not this thread or the main thread). Once this thread is found, the daemon will invoke
 * the garbage collector until we are back under 75% heap occupancy.
 * 
 * @author jeikenberry
 */
public class ThreadMonitor extends Thread {
  ...
  /**
   * The maximum allowed memory that can be used by the application (as a fraction).
   */
  private final float maxMemRatio;

  /**
   * The maximum number of open file handles that can be open at one time, by the application.
   */
  private final int maxFiles;

  /**
   * Maximum loop time for possibly/definitely tainted loops (respectively).
   */
  private final int maxLoopTime, maxLoopTimeDef;

  /**
   * Current timestamps created by the daemon thread for possibly/definitely tainted loops
   * (respectively).
   */
  public Timestamp curTimestamp, curTimestampDef;

  /**
   * Queue of all timestamps created by the daemon thread for possibly/definitely tainted loops
   * (respectively).
   */
  ...
  public void handleInterrupts() {
    ...
    int cwe = 0;
    synchronized (deadlockedThreadIds) {
      // Make sure that the current thread that's interrupted is the one that caused the deadlock.
      if (interruptedThread == curThread) {
        if (memAlloc > 0) {
          errorMsg = "Interrupting " + curThread + " due to a memory over-allocation: " + memAlloc
              + " bytes";
          cwe = 400;
        } else if (openFiles > 0) {
          errorMsg = "Interrupting " + curThread + " due to too many open files: " + openFiles
              + " files open.";
          cwe = 774;
          // Clean up the file handles and update the total number of open files.
        ...
        } else if (BaseConfig.getInstance().areDeadlocksEnabled()
            && !deadlockedThreadIds.isEmpty()) {
          errorMsg = "Interrupting " + curThread
              + " due to a detected deadlock condition on thread ids: " + deadlockedThreadIds;
          cwe = 833;
          deadlockedThreadIds.clear();
    ...
    if (errorMsg != null) {
      NotifyMsg notifyMsg =
          new NotifyMsg("DeadlockDetector.monitorenter()", "DeadlockDetector.monitorenter()", cwe);
  ...
  /**
   * Mimic's Object.wait() calls, using the ReentrantLock's condition that was generated.
   * 
   ...
   * 
   * @param obj Object to wait on.
   * @param timeout long of the timeout (in milliseconds).
   * @throws InterruptedException
   */
   ...
   * @param obj Object to wait on.
   * @param timeout long of the timeout (in milliseconds).
   * @param nanos int of additional timeout (in nanoseconds).
   * @throws InterruptedException
   */
  ...
  /**
   * Mimic's Object.notify() calls, using the ReentrantLock's condition that was generated.
   * 
   * @param obj Object to notify on.
   */
  ...
  /**
   * Mimic's Object.notifyAll() calls, using the ReentrantLock's condition that was generated.
   * 
   * @param obj Object to notify all on.
   */
  ...
  /**
   * This method should be called when creating a closeable that is backed by an open file
   * descriptor. If we exceed some limit on the number of open files (at set by the config file) we
   * will interrupt the offending thread. Once this interrupt is detected, a CleartrackException
   * will be raised.
   * 
   * @param c Closeable
   */
  ...
  /**
   * This method should be called once a closeable's close() method has been called.
   * 
   * @param c Closeable
   */
  ...
  /**
   * Metadata that we keep for active loops. The update should be maintained by the daemon thread
   * and should be reasonably accurate, by passing the time that the daemon thread sleeps (per
   * iteration).
   * 
   * @author jeikenberry
   */
  public class Timestamp {
    ...
    /**
     * Updates this Timestamp, and determines whether this Timestamp is still alive.
     * 
