...
public class EnvInfo {
    ...
    public static void init_env_info(Properties sysProps) {
        ...
        // We check that all execs take from the standard path, so we don't care
        // what this path value is.  Arguably we should put the standard paths
        ...
        try (ObjectInputStream in = new ObjectInputStream(new FileInputStream(propertyGoalFile));) {
            ...
            for (Entry<Object, Object> entry : sysProps.entrySet()) {
                ...
                // FIXME MITRE tests are instrumented and executed on different
                // machines, so we will need a good solution for this.  Always
                // trust base properties for the time being...
                //			if (actualVal.equals(goalVal)
                //					|| key.equals("user.dir")
                //					|| key.equals("java.class.path")
                //					|| key.equals("sun.boot.class.path")
                //					|| key.equals("java.ext.paths"))
    ...
    /**
     * Returns a map with valid environment variable information.
     * Variables in the map have either correct information or ANY_VALUE
     * as a value.  If they have ANY_VALUE, then any setting for that
     * name is acceptable (and can be thought of as trusted) Variables
     * in the environment and not in the map should be considered
     * untrusted.  If used as part of an exec() call, they should be
