...
public final class TaintUtils {
   ...
   * @param s_unencoded - No encodings. Contains both mapped/unmapped chars Is the string sent to.
   *        URLEncoder.encode(s_unencoded) which returned String s. This param is required only for
   *        chartaint.
   */
  public static final String urlEncodedTaint(String s, String s_unencoded) {
    ...
    // Ensure that each encoded value starting with '%'vhas the same conservative joint taint in the
  ...
  /**
   * @param s The decoded string returned from URLDecoder.decode(str-unmapped, ...) If running
   *        chartaint contains only unmapped chars.
   * @param str Is a string that was passed to URLDecorder.decode(str-unmapped, ...) If running
   *        chartaint contains mapped/unmapped chars.
   * @return String s but each char in s tainted according to the corresponding char is str. Note
   *         that the char in s may correspond to a three character encoding String in str - in
   *         which case if any character in the three char encoding string is tainted, then the
   *         corresponing char in s is tainted.
   */
  public static final String urlDecodedTaint(String s, String str) {
    ...
    // Ensure that the string was properly decoded, by going through encoded values starting with
    // '%' and finding the joint taint.
    for (int i = 0; i < last; i++) {
      if (encodedValue[k] == '%') {
  ...
  public static final void copyTaint(Reader in, Writer out, Ret ret) throws IOException {
    try {
      ...
      // TODO: Don't really know if we should be closing the input stream.
      in.close();
      out.close();
    }
  }
  ...
  /**
   * If all of path is trusted, then trust canonicalPath. Otherwise copy all of the taint from path
   * to canonicalPath up to the differing character index. Then taint the remaining characters in
   * canonicalPath.
   * 
    ...
    if (path.equals(CWD)) {
      ...
      while ((idx = path.indexOf(File.separator, idx + 1)) >= 0) {
        ...
        if (canonPath.equals(CWD)) {
          // We found a canonicalized prefix that equals CWD, so trust that part of the path.
  ...
  /**
   * Returns ORed taint over all regions of taint in a given String and over a specified bit-mask.
   **/
  ...
  /** 
   * @param taint value in question.
   * @return true if {@code taint} indicates trusted, false otherwise.
   */
  ...
  /** 
   * @param taint value in question.
   * @return true if {@code taint} indicates untrusted, false otherwise.
   */
  ...
  /**
   * Trusts a String constant. Do not call this unless you know what you're doing. This should only
   * be called by the instrumentation since we make some assumptions that a constants taint cannot
   * be mutated (since a String itself is immutable).
   * 
   * @param s program literal.
   * @return s the trusted String literal.
   */
  public static final String trustConstant(String s) {
    /*
     * TODO make sure that we can do safely do this optimization... i don't believe that we can ever
     * taint a string constant, since strings themselves are immutable. there could be some
     * instrumentation code that does this though.
     */
    if (s.value_t == null) {
      char[] chArr = s.value;
      s.value_t = new CharArrayTaint(chArr, new int[chArr.length]);
  ...
  /**********************************************************************************************
   * The following methods are never called, but only exist because the JUnit tests make use of *
   * these methods. The calls, however, will refer to the instrumented call of this method.     *
   **********************************************************************************************/
  ...
  public static final boolean isTrusted(boolean value) {
    return false;
  }
  ...
  public static final StringBuilder replace(StringBuilder s, int beginIndex, int endIndex,
      String replaceStr) {
  ...
  /**
   * Call this method to retrieve a mapped/unmapped value from a Map where the key can be
   * mapped/unmapped.
   */
