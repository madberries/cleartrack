...
public final class Overflows {
     ...
     * @throws IOException
     */
    public static final void checkOverflow(int taint, String operation, String type, Object value, int... indices) {
        //#if log_overflow==true
        logOverflow(taint, operation);
        //#endif
        if (!TaintUtils.isTrusted(taint)) {
            if ((taint & TaintValues.INFINITY) == TaintValues.INFINITY) {
                BaseConfig.getInstance().handleInfinity(buildDescription(operation, type, value, indices), operation);
            }
            if ((taint & TaintValues.OVERFLOW) == TaintValues.OVERFLOW) {
                BaseConfig.getInstance().handleOverflow(buildDescription(operation, type, value, indices), operation);
            }
            if ((taint & TaintValues.UNDERFLOW) == TaintValues.UNDERFLOW) {
                BaseConfig.getInstance().handleUnderflow(buildDescription(operation, type, value, indices), operation);
            }
        }
    }

    /**
     * Builds a description of the overflow check for array indexing.  For compatibility
     * reasons, do not attempt to call this method directly from the operation.  Instead,
     * it needs to be called only when a tainted underflow/overflow/infinity has occurred.
     * 
     * @param operation
     * @param type
     * @param value
     ...
     * @return
     */
    private static final String buildDescription(String operation, String type, Object value, int... indices) {
     ...
     * @throws IOException
     */
    public static final void checkOverflow(int op1_taint, int op2_taint, String description, String operation)
            throws IOException {
        //#if log_overflow==true
        logOverflow(op1_taint | op2_taint, operation);
        //#endif
        if (!TaintUtils.isTrusted(op1_taint)) {
            if (!TaintUtils.isTrusted(op2_taint) && (op1_taint & TaintValues.INFINITY) == TaintValues.INFINITY
    ...
    /**
     * Called to clear overflow/underflow from taint only if one of the
     * operands does not have an overflow or underflow bit set.
     * 
     * @param op1_taint first operands taint
     * @param op2_taint second operands taint
     * @param operation that caused the clear
    ...
    public static final void outOfBounds(String operation, String type, Object array, int idx) {
        NotifyMsg notifyMsg = new NotifyMsg(buildDescription(operation, type, null, idx), operation, 129);
    ...
    public static final int checkAllocSize(String type, int size) throws IOException {
        ...
        if (size >= max) {
            return BaseConfig.getInstance().handleMaxAllocation(buildDescription("new-array", type, null, size),
                                                                "new-array", size);
        }
        return size;
    }
    ...
    private static final void print(Writer out, String title, Map<Overflow, Integer> overflows) throws IOException {
        if (overflows == null)
            return;
        out.write("\n" + title + ": \n");
        int trusted = 0;
        ...
        for (String op : totalMap.keySet()) {
            out.write("  " + op + " -- " + totalMap.get(op) + " total / " + distinctMap.get(op) + " distinct\n");
        }
        ...
        public Overflow(int taint, String op) {
            ...
            for (int i = 3; i < stackTrace.length; i++) {
                if (!foundTop && !stackTrace[i].getClassName().startsWith("pac.")) {
                    this.stackTop = stackTrace[i].getClassName() + "." + stackTrace[i].getMethodName() + " [line "
