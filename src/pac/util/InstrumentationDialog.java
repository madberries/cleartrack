...
public class InstrumentationDialog implements Runnable {
  ...
  public static void showRandomizationDialog(String sql) {
    if (showPopUps) {
      ...
      InstrumentationDialog id =
          new InstrumentationDialog(message.toString(), "Parse Randomized SQL");
  ...
  public static void showSanitizationDialog(String sql, String sanitizedSql) {
    if (showPopUps) {
      ...
      message.append("</font></center></html>");
      InstrumentationDialog id =
          new InstrumentationDialog(message.toString(), "Sanitized Tainted SQL");
      Thread t = new Thread(id);
      t.start();
    }
  }
  ...
  public static String htmlFormatTrackedString(String sourceString) {
    ...
    while (iter.hasNext()) {
      ...
      // We have to compensate for our inclusive region, since substring's endIdx is exclusive.
      output.append(simpleHTMLEscape(sourceString.substring(startIndex, endIndex + 1)));
      output.append("</font>");
   ...
   * @return {@code message} that has been HTML-escaped.
   */
