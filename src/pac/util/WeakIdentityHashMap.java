...
/**
 * This is a modified version of {@link java.util.WeakHashMap} from JDK 1.5, that uses
 * System.identityHashCode() rather than the object's hash code. All equality checks are identity
 * checks (==) rather than objet equality (.equals); @see{IdentityHashMap} for more information on
 * the changes made in an identity hash map.
 *
 * <hr>
 *
 * A hashtable-based <tt>Map</tt> implementation with <em>weak keys</em>. An entry in a
 * <tt>WeakIdentityHashMap</tt> will automatically be removed when its key is no longer in ordinary
 ...
 * class behaves somewhat differently than other <tt>Map</tt> implementations.
 *
 * <p>
 * Both null values and the null key are supported. This class has performance characteristics
 * similar to those of the <tt>HashMap</tt> class, and has the same efficiency parameters of
 * <em>initial capacity</em> and <em>load factor</em>.
 *
 * <p>
 * Like most collection classes, this class is not synchronized. A synchronized
 * <tt>WeakIdentityHashMap</tt> may be constructed using the <tt>Collections.synchronizedMap</tt>
 * method.
 *
 * <p>
 * The behavior of the <tt>WeakIdentityHashMap</tt> class depends in part upon the actions of the
 * garbage collector, so several familiar (though not required) <tt>Map</tt> invariants do not hold
 * for this class. Because the garbage collector may discard keys at any time, a
 * <tt>WeakIdentityHashMap</tt> may behave as though an unknown thread is silently removing entries.
 * In particular, even if you synchronize on a <tt>WeakIdentityHashMap</tt> instance and invoke none
 * of its mutator methods, it is possible for the <tt>size</tt> method to return smaller values over
 * time, for the <tt>isEmpty</tt> method to return <tt>false</tt> and then <tt>true</tt>, for the
 ...
 *
 * <p>
 * Each key object in a <tt>WeakIdentityHashMap</tt> is stored indirectly as the referent of a weak
 ...
 *
 * <p>
 * <strong>Implementation note:</strong> The value objects in a <tt>WeakIdentityHashMap</tt> are
 * held by ordinary strong references. Thus care should be taken to ensure that value objects do not
 ...
 * <tt>WeakIdentityHashMap</tt> itself; that is, a value object may strongly refer to some other key
 * object whose associated value object, in turn, strongly refers to the key of the first value
 * object. One way to deal with this is to wrap values themselves within <tt>WeakReferences</tt>
 * before inserting, as in: <tt>m.put(key, new WeakReference(value))</tt>, and then unwrapping upon
 * each <tt>get</tt>.
 *
 * <p>
 ...
 */
@SuppressWarnings({"unchecked", "rawtypes"}) // old, non-typesafe Sun code, not worth annotating or
                                             // checking
public class WeakIdentityHashMap<K, V> extends AbstractMap<K, V> implements Map<K, V> {
  ...
  /**
   * Constructs a new, empty <tt>WeakIdentityHashMap</tt> with the given initial capacity and the
   * given load factor.
   *
  ...
  /**
   * Constructs a new, empty <tt>WeakIdentityHashMap</tt> with the default initial capacity (16) and
  ...
  /**
   * Constructs a new <tt>WeakIdentityHashMap</tt> with the same mappings as the specified
   * <tt>Map</tt>. The <tt>WeakIdentityHashMap</tt> is created with default load factor, which is
   * <tt>0.75</tt> and an initial capacity sufficient to hold the mappings in the specified
   * <tt>Map</tt>.
   *
   ...
   */
  public static boolean eq(Object x, /* @Nullable */ Object y) {
    return x == y;
  }
  ...
  /**
   * Returns the value to which the specified key is mapped in this weak hash map, or <tt>null</tt>
   * if the map contains no mapping for this key. A return value of <tt>null</tt> does not
   * <i>necessarily</i> indicate that the map contains no mapping for the key; it is also possible
   * that the map explicitly maps the key to <tt>null</tt>. The <tt>containsKey</tt> method may be
  ...
  void resize(int newCapacity) {
    ...
    /*
     * If ignoring null elements and processing ref queue caused massive shrinkage, then restore old
     * table. This should be rare, but avoids unbounded expansion of garbage-filled tables.
     */
    if (size >= threshold / 2) {
   ...
   * collection is a <tt>Map.Entry</tt>. The collection is backed by the map, so changes to the map
   * are reflected in the collection, and vice-versa. The collection supports element removal, which
