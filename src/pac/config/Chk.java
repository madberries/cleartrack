...
public class Chk {
  ...
  private enum Encoding {
    XPATH_TYPE(TaintValues.XPATH_TYPE), CSS_TYPE(TaintValues.CSS_TYPE),
    DN_TYPE(TaintValues.DN_TYPE), HTML_TYPE(TaintValues.HTML_TYPE),
    JAVASCRIPT_TYPE(TaintValues.JAVASCRIPT_TYPE), LDAP_TYPE(TaintValues.LDAP_TYPE),
    OS_TYPE(TaintValues.OS_TYPE), SQL_TYPE(TaintValues.SQL_TYPE),
    URL_TYPE(TaintValues.URL_TYPE), VBSCRIPT_TYPE(TaintValues.VBSCRIPT_TYPE),
    XML_TYPE(TaintValues.XML_TYPE);
  ...
  Vector<String> directivesList; // Elems are directive strings from config file between check
                                 // <name> and end check.

  /**
   * Called for each config file line that is between "check <name>" and "end check".
   * 
   * @param knownCheckNames - Contains every check name having a definition in the config file. The
   *        caller has already read once through the config file. On reading a "check <check_name>"
   *        line the <check_name> is added to knownCheckNames.
   */
  public void buildChk_parseLine(final String line, final Vector<String> knownCheckNames,
      AbstractConfig config) throws MsgException {
    ...
    if (lineRay[0].equals(PREFIX_WHITE_TKN)) {
      ...
      directivesList.addElement(line);
    } else if (lineRay[0].equals(DESC_TKN) || lineRay[0].equals(CHECK_TKN)
        || lineRay[0].equals(LOG_FILE_TKN) || lineRay[0].equals(SHELL_LIST_TKN)) {
      throw new MsgException("Illegal key word token read between \"" + CHECK_TKN + " " + check_name
          + "\"" + " and " + END_TKN);
   ...
   * </pre>
   * 
   * @param called_for_white true if processing WHITE_FAILURE_ACTION_TKN false if processing
   *        BLACK_FAILURE_ACTION_TKN.
   * @param lineRay lineRay[0] white/black_failure_action lineRay[1] action lineRay[2] replace_char
   *        or exception_subtype.
   */
  private void build_black_white_failure(final boolean called_for_white, final String[] lineRay)
      throws MsgException {
    ...
    // Ensure that action is a legal string value.
    // Ensure that action is legal for the white list or black list failure.
    if (action == RunChecks.REPLACE_ACTION && called_for_white) {
      throw new MsgException("Is illegal to set action for " + WHITE_FAILURE_ACTION_TKN + " to "
          + RunChecks.REPLACE_ACTION_TKN);
    }
    ...
    if ((action == RunChecks.EXCEPTION_ACTION) || (action == RunChecks.REPLACE_ACTION)) {
      if (lineRay.length < 3) {
        throw new MsgException(
            lineRay[0] + " specifies action: " + RunChecks.actionIntToString(action) + ".\n" + // MsgException
                ...
                + (action == RunChecks.EXCEPTION_ACTION ? "exception subclass"
                    : "replace character")
                + " is missing.");
      }
      ...
      if (action == RunChecks.EXCEPTION_ACTION) {
        ...
        if (replace_char.length() > 1) {
          throw new MsgException("Illegal replace-character follows " + lineRay[0] + " "
       ...
       *   throw new MsgException("Illegal character entered for " + RunChecks.BLACK_REPLACE_CHAR_TKN +
       *   " " + black_replace_char + "\n" + "Must be a single character.");
       * }
       */
    }
  ...
  private void build_encoding_action(final boolean called_for_white, final String[] lineRay)
      throws MsgException {
    if (lineRay.length < 2) {
      throw new MsgException(lineRay[0] + " required an action to be specified.");
    }
    ...
    if (action == RunChecks.EXCEPTION_ACTION) {
      ...
      /*
       * TODO: would like to implement this check by undoing the improper encoding and then properly
       * encoding it. It doesn't appear that ESAPI supports decoding of all types. So to properly
       * implement this I would have to keep a HashMap of all encoded strings to the String before
       * it was encoded.
       */
  ...
  private static void check_legal_array(final String line, Vector<String> knownCheckNames)
      throws MsgException {
      ...
      if (!knownCheckNames.contains(tokens[2])) {
        throw new MsgException(
            MSG + "Where check_name must be a check name defined in the configuration file.\n"
                + tokens[2] + " is not an existing check name.");
      }

    } catch (NumberFormatException ex) {
      throw new MsgException(
          MSG + line + "Illegal array number read: " + tokens[1] + "\n" + ex.getMessage());
    }
  }
  ...
  private void build_qstring(final String[] lineRay) throws MsgException {
    ...
    try {
      if (tokenizer == null) {
        throw new MsgException(
            "Check \"" + check_name + "\" must reference a tokenizer to perform a qstring.");
      }
    ...
    if (Q_action == RunChecks.EXCEPTION_ACTION) {
      if (lineRay.length < 3) {
        throw new MsgException(
            "The action for " + lineRay[0] + " is " + RunChecks.actionIntToString(Q_action) + // MsgException
  ...
  private void build_traversal_count(final String[] lineRay) throws MsgException {
    ...
    if (traversal_count_action == RunChecks.EXCEPTION_ACTION) {
      if (lineRay.length < 3) {
        throw new MsgException("The action for " + lineRay[0] + " is "
            + RunChecks.actionIntToString(traversal_count_action) + // MsgException
            ".\nExpected a subclass of RuntimeException to follow "
  ...
  private Pattern compilePattern(final String[] lineRay, final int tokenIndex) throws MsgException {
    try {
      if (lineRay.length - 1 < tokenIndex) {
        throw new MsgException(
            "Unable to access token " + tokenIndex + " from line in configuration file.");
      }
      ...
      throw new MsgException(
          "Bad regular expression read in configuration file.\n" + ex.getMessage());
    }
  }
  ...
  private void build_env_var(final String line) throws MsgException {
    ...
    try {
      if (ray.length < 4) {
        throw new MsgException(
            MSG + "Expected minimum of 4 tokens. There are " + ray.length + " tokens.");
      }
      ...
      if (VAR_VALUE_WHITE_TKN.equals(ray[0])) {
        ...
        if ((envVar.black_failure_action == RunChecks.REPLACE_ACTION)
          ...
          if (ray.length < 5) {
            throw new MsgException("The action for " + ray[0] + " is " + ray[3] + ".\n"
                ...
                + (envVar.black_failure_action == RunChecks.REPLACE_ACTION ? "replacement character"
        ...
        throw new MsgException(
            "Keyword error: expected " + VAR_VALUE_WHITE_TKN + " or " + VAR_VALUE_BLACK_TKN);
      }
  ...
  /**
   * Called when reading through config file in "check" section and have read a line that starts
   * with "token_whitespace".
   *
   * This method sets token_whitespace_action sets token_whitespace_exeption_constructor - if action
   * is exception sets token_whitespace_replace_char - if action is replace.
   */
  ...
  // Called to process a single env var entry. processMe looks like: "<var_name>=<value>".
  private String runEnvChk(final String processMe, final NotifyMsg notifyMsg)
      ...
      if (!white_failure && envVar.blacklist != null) {
        ...
        if (black_failure) {
          ...
          if (envVar.black_failure_action == RunChecks.REPLACE_ACTION) {
            ...
            newProcessMe =
                TaintUtils.copyTaint(processMeTkns[0], newProcessMe, processMeTkns[0].length());
  ...
  private boolean isWhitelisted2(final String section, final String checkName,
      final String directive, final int white_failure_action, final NotifyMsg notifyMsg,
  ...
  private boolean isWhitelisted(final String section, final NotifyMsg notifyMsg, boolean isPrefix,
    ...
    if (isPrefix) {
      retval = isWhitelisted2(section, check_name, PREFIX_WHITE_TKN, white_failure_action,
          notifyMsg, prefix_whitelist);
    }
    if (retval && useInternal) {
      retval = isWhitelisted2(section, check_name, INTERNAL_WHITE_TKN, white_failure_action,
          notifyMsg, internal_whitelist);
    }
    if (retval && isSuffix) {
      retval = isWhitelisted2(section, check_name, SUFFIX_WHITE_TKN, white_failure_action,
          notifyMsg, suffix_whitelist);
    }

    return retval;
  }

  private boolean isBlacklisted2(final String section, final String checkName,
      final String directive, final int black_failure_action, final NotifyMsg notifyMsg,
      final Pattern pattern) {
  ...
  private boolean isBlacklisted(final String section, final NotifyMsg notifyMsg, boolean isPrefix,
    ...
    boolean retval = false;
    if (isPrefix) {
      retval = isBlacklisted2(section, check_name, PREFIX_BLACK_TKN, black_failure_action,
          notifyMsg, prefix_blacklist);
    }
    if (!retval && useInternal) {
      retval = isBlacklisted2(section, check_name, INTERNAL_BLACK_TKN, black_failure_action,
          notifyMsg, internal_blacklist);
    }
    if (!retval && isSuffix) {
      retval = isBlacklisted2(section, check_name, SUFFIX_BLACK_TKN, black_failure_action,
          notifyMsg, suffix_blacklist);
    }
    return retval;
  }
  ...
  /**
   * <pre>
   * Each bit of data that is checked because of a config file "desc" entry goes through this
   * method. This method uses the directives listed in global directivesList inspect String
   * processMe.
   * </pre>
   *
   ...
   */
  public String runFromChk(String processMe, Tokenized tokenized, final int runningShell,
      final NotifyMsg notifyMsg) throws MsgException, IOException {
    ...
    for (int i = 0; i < directivesList.size(); i++) {
      ...
      if (cmd.equals(TOKEN_TKN)) {
        ...
        final String nth_processMe_tkn =
            (tokenized == null) ? Utils.getTokenAtIndexWithTaint(processMe, tkn_num)
        ...
        if (newChk != null) {
          final String new_nth =
              newChk.runFromChk(nth_processMe_tkn, tokenized, runningShell, notifyMsg); // MsgException
                                                                                        // IOException
      ...
      } else if (cmd.equals(PREFIX_WHITE_TKN) || cmd.equals(INTERNAL_WHITE_TKN)
          || cmd.equals(SUFFIX_WHITE_TKN) || cmd.equals(PREFIX_BLACK_TKN)
          || cmd.equals(INTERNAL_BLACK_TKN) || cmd.equals(SUFFIX_BLACK_TKN)
          || cmd.equals(TRAVERSAL_COUNT_TKN)) {
        // Do these only once, and all at once. Because traveling through alternating
        // trusted/untrusted sections is expensive.
        ...
        if (chk_2 == null) {
          ...
          // 0 means error, has nothing to do with attack.
          Notify.appendToLogFile(
              "ERROR in configuration file " + file + "\nUnknown token read " + cmd, 0);
        } else {
          processMe = chk_2.runFromChk(processMe, tokenized, runningShell, notifyMsg); // MsgException
                                                                                       // IOException
        }
      }
    }
  ...
  /**
   * @param section Is presumably an untrusted section. Each sequence of chars in section that
   *        matches "pattern" is replaced with a string of "replaceChar".
   *
   *        <p>Note: For charTaint, "section" is unmapped and pattern was compiled from an unmapped Sting
   *         - so pattern.matcher(section) will work as expected.</p>
   ...
   */
  private static String blacklistReplace2(final String section, final Pattern pattern,
    ...
    if (pattern != null) {
      ...
      if (strbuf.length() != 0) {
        // We are altering newSection, so copy taint from section to newSection.
  ...
  private String blacklistReplace(final String section, final boolean isPrefix,
      final boolean useInternal, final boolean useSuffix) {
    String newSection = section;
    if (isPrefix) {
  ...
  private boolean isSectionPrefix(final String processMe, int trStart) {
    ...
    return (trStart < 0 || processMe.charAt(trStart) == '\"' || processMe.charAt(trStart) == '\''
   ...
   * </pre>
   *
   * @param processMe the String to process.
  ...
  private boolean isSectionSuffix(final String processMe, int trEnd) {
    trEnd++;
    return (trEnd >= processMe.length() - 1 || processMe.charAt(trEnd) == '\"'
        || processMe.charAt(trEnd) == '\'' || Character.isWhitespace(processMe.charAt(trEnd)));
  }
  ...
  private String tokenizeAndRunWhiteBlack(final String processMe, final NotifyMsg notifyMsg)
    ...
    // Form a Vector of Tkn in class Quotes. Each Tkn element represents a string token as a shell
    // would see it and is the token that the regular expressions are to be matched against.
    ...
    // tknStrAtIndex retrieve string token. The return string has the outer-most quotes,
   ...
   * @return True if this criteria check has passed, otherwise false. If no check_critera has been
   *         set for this check, return true.
   */
  ...
  /**
   * @param processMe Can contain alternating trusted/untrusted sections. Discover dangerous
   *        untrusted data in this string.
   * @param For notify. The API signature of caller. no param names or ret val. ex.
   *        "Runtime.exec(String, String[])".
   * @return processMe. If white_failure_action or black_failure_action is REPLACE_ACTION and
   *         dangerous untrusted data is discovered in processMe, then the returned String will
   *         differ from processMe.
   * @throws MsgException
   */
  private String runWhiteBlackSections(String processMe, final NotifyMsg notifyMsg)
      throws IOException, MsgException {
    if (traversal_count_action >= 0) {
      // NOT NEEDED:
      // -----------
      // processMe can look like "/bin/ls ../file"
      // doTraversalCount splits the string on '/' and does not expect the string it is processing
      // to contain whitespace. traversal_count_on_tokens does the following:
      ...
      // If a traversal count should be enforced, then check it against the string in question and
    ...
    int startCpy = 0; // Always points to next avail place in processMe where to start copying char
                      // from.
    ...
    while (iter.hasNext()) {
      ...
      if (sr.isUntrusted()) { // region is not trusted...
        ...
        // ***
        // Do white list.
        // Use internal whitelist if section is neither prefix nor suffix, or if neither of these
        // conditions exist:
        ...
        final boolean whitelistFailure =
            !isWhitelisted(section, notifyMsg, isPrefix, useInternal, isSuffix);

        if (whitelistFailure) {
          if (whiteAction == RunChecks.EXCEPTION_ACTION
              || whiteAction == RunChecks.TERMINATE_ACTION) {
            notifyMsg.prepareForExceptionOrTerminate(white_exception_constructor,
                white_failure_action);
        ...
        // ***
        // Do black list.
        // Use internal blacklist if section is neither prefix nor suffix, or if neither of these
        // conditions exist:
        ...
        final boolean blacklistFailure =
            isBlacklisted(section, notifyMsg, isPrefix, useInternal, isSuffix);

        // The case where we need to keep looping - action is replace.
        if (blacklistFailure) {
          if (blackAction == RunChecks.REPLACE_ACTION) {
            ...
            if (!newStr.equals(section)) {
              TaintUtils.append(alteredProcessMeBuf,
                  TaintUtils.substring(processMe, startCpy, trStart));
              TaintUtils.append(alteredProcessMeBuf, newStr);
              startCpy = trStart + newStr.length();
            }
          } else if (blackAction == RunChecks.EXCEPTION_ACTION
              || blackAction == RunChecks.TERMINATE_ACTION) {
            notifyMsg.prepareForExceptionOrTerminate(black_exception_constructor,
                black_failure_action);
            Notify.notifyAndRespond(notifyMsg); // IOException or terminate.
          }
        }
      }
   ...
   * variables, and properties should be tainted. The data source is considered trusted if and only
   * if the underlying regular expression matches the supplied string.
   * 
   * Is like runWhiteBlackSections except the entire string is processed without regard to
   * trusted/untrusted sections Return true if processMe is a trusted.
   * 
  ...
  public boolean runTrustedDataSource(String processMe, final NotifyMsg notifyMsg) {
    final boolean whitelisted = isWhitelisted(processMe, notifyMsg, // Message will added, but never
                                                                    // used.
        false, // isPrefix
        true, // useInternal
        false); // isSuffix
  ...
  private void tokenWhiteSpaceMsg(final String processMe, final NotifyMsg notifyMsg,
   ...
   */
  private String runTokenWhiteSpace(final String processMe, NotifyMsg notifyMsg,
      final String directive) throws IOException {
    ...
    final String replace_str =
        (token_whitespace_replace_char == null ? "_" : token_whitespace_replace_char);
    
    // Use when either whiteAction or blackResponse is REPLACE_ACTION.
    final StringBuilder alteredProcessMeBuf = TaintUtils.newStringBuilder(); 
    ...
    if (alteredProcessMeBuf.length() > 0 && startCpy < processMe.length()) { // Action is replace &
                                                                             // replaced untrusted
                                                                             // whitespace.
      notifyMsg.setAction(token_whitespace_action);
      TaintUtils.append(alteredProcessMeBuf, TaintUtils.substring(processMe, startCpy));
    ...
    if (alteredProcessMeBuf.length() > 0) { // Action was replace and untrusted whitespace was
                                            // replaced.
      newProcessMe = TaintUtils.toString(alteredProcessMeBuf);
    }

    if (!keepLooking || alteredProcessMeBuf.length() > 0) { // Action is log_only or replace and
                                                            // found untrusted white space.
      tokenWhiteSpaceMsg(processMe, notifyMsg, directive);
      if (newProcessMe != processMe) {
   ...
   */
  private String replace_nth_token(final String processMe, final String orig_tkn,
      final String new_nth, final int tkn_num) {
   ...
   * @throws IOException
   */
  private String doTraversalCount(String path, NotifyMsg notifyMsg)
      throws MsgException, IOException {
    ...
    if (becameNeg) {
      ...
      if (traversal_count_action == RunChecks.REPLACE_ACTION) {
        ...
        notifyMsg.prepareForExceptionOrTerminate(traversal_count_exception_constructor,
            traversal_count_action);
        Notify.notifyAndRespond(notifyMsg); // IOException or terminate
      }
    }
    return path;
  ...
  /**
   *
   * @param processMe If Q_action is replace, replace untrusted quotes surrounded by trusted quotes
   *        with escaped quotes.
   * @param directive The first token of the line read in.
   * @return processMe Apply the Qsting check to this line.
   */
  private String doQString(final String processMe, final NotifyMsg notifyMsg,
      final String directive) throws MsgException, IOException {
    // In processMe within each pair of trusted replaceMe, look for untrusted replaceMe and replace
    // each untrusted "replaceMe" with "replaceWith".
  ...
  private void runEncodingCheck(String processMe, NotifyMsg notifyMsg, String directive)
      throws IOException {
    ...
    while (iter.hasNext()) {
      ...
      if (sr.isUntrusted()) { // Not trusted.
        ...
        } else if ((encodings_whitelist
            | (TaintValues.ENCODING_MASK & taint)) != (encodings_whitelist
  ...
  private String encodingCheckTakeAction(String processMe, NotifyMsg notifyMsg, String directive,
      int encoding, boolean isBlack) throws IOException {
  ...
  private boolean whitePatternMatches(final Pattern pattern, final String str_param) {
    ...
    return matcher.matches(); // Note: whitelist reg exp use Matcher.matches(). blacklist reg exp
                              // use Matcher.find().
  }
  ...
  // Called when are finished building this single Chk.
  ...
  void debug_verify() throws MsgException {
    ...
    if (envVec != null) {
      for (int i = 0; i < envVec.size(); i++) {
        final EnvVar envVar = envVec.elementAt(i);
        if (envVar.black_failure_action == RunChecks.REPLACE_ACTION
            && envVar.black_replace_char == null) {
          strbuf = new StringBuffer();
          strbuf.append("In check ");
          strbuf.append(check_name);
          strbuf.append(": entry ");
  ...
  void dump() {
    ...
    if (white_exception_constructor != null) {
      System.out
          .println("    white_exception_constructor: " + white_exception_constructor.getName());
    }
    if (black_exception_constructor != null) {
      System.out
          .println("    black_exception_constructor: " + black_exception_constructor.getName());
    }
    ...
    for (int i = 0; i < envVec.size(); i++) {
      try {
        ...
        System.out
            .println(envVar.black_replace_char == null ? "null" : (envVar.black_replace_char));
