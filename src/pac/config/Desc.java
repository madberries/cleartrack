...
/**
 * 
 * @author rjk This class exists to "remember" each api description as it is
 *         read in from the config file. Each api description is stored in an
 *         instance of Desc and added to a global list.
 */
public class Desc {
    String command;
    Condition descCond;
     ...
     * 
     * @param command
     *            is the java dot notation name being processed. ex.
     *            "java.io.File.createTempFile"
     * @param paramTypes
     *            The type of each formal parameter in the api description is in
     *            this list.
     * @param paramChkNames
     *            The optional name found to the right of the paramType in the
     *            api description. The name, if given, is the name of a check,
     *            defined elsewhere in the config file. At run time the check of
     *            this name will be used to check the parameter in this
     *            corresponding position.
     */
    private Desc(final String command, final String descCond, final String[] paramTypes, final Chk[] paramChks,
                 final Condition[] paramConds, final int[] paramCwes, final String afterChk, AbstractConfig config) {
     ...
     * 
     * @param line
     *            Is a line from config file. line has been trimmed. Looks like
     *            "desc Runtime.exec(String, int[]check_name)" White space may
     *            not be right. This method parses out "Runtime.exec" and the
     *            check_name that may or may not exist for each parameter.
     * @return A filled Desc. Desc.command is "Runtime.exe" Desc.paramChkNames
     *         is a Vector<String> Desc.paramChkNames[n] is null or a check_name
     *         according as whether the config file specified a check_name.
     */
    public static Desc createDesc(String line, AbstractConfig config) {
        boolean hasBeforeChk = false;

        // in "line" change white space around the brackets to make it be easier
        // to parse
        // ie change "desc   Runtime.exec(float   a, String  [   ]b  int[ ]  b)"
        // to "desc   Runtime.exec(float   a, String[] b int[] b)"
        // This makes it easier to parse aout a, b, and c
        // ie eliminate all white space left of '['
        // eliminate all white space between '[' and ']'
        // insure exactly one white space is to right of ']'
        ...
        final StringBuilder cmd = new StringBuilder(line_2.substring(startClass, start).trim()); // Get "java.lang.Runtime.exec"
        ...
        for (int i = 0;;) {
            ...
            if (name != null) {
                idx = name.indexOf('{');
                if (idx > 0) {
                    paramCwes[i] = Integer.parseInt(name.substring(idx + 1, name.length() - 1).split("\\-")[1]);
        ...
        final Desc desc = new Desc(cmd.toString(), descCond, paramTypes, paramChks, paramConds, paramCwes, afterChk,
                config);
    ...
    public static class Condition {
        ...
        public Condition(String cond) {
            ...
            if (idx == 0) {
                ...
                // method refers to a static method in ConfigFile that takes no
