...
public class Quotes {
  ...
  // This class exists to make it easier to tokenize string and quote in a way that resembles a
  // shell.
  //
  ...
  // The strings are either single quoted strings, double quoted strings, or strings delimited
  // either by white space or a single quoted string token or double quoted string token.
  ...
  // Class is to remember location of an open and closed pair of either single quotes or double
  // quotes. When Tkn's are compressed into a single token and contiguous quotes areas are
  // compressed, the locations of each pair of quotes is stored in a RemQuote.
  class RemQuote {
  ...
  class Tkn {
    ...
    final int howQuoted; // One of: UNQUOTED_TKN SINGLE_QUOTE_TKN DOUBLE_QUOTE_TKN.
    boolean openIsTrusted, closedIsTrusted;
    Vector<RemQuote> remQuotes; // Not used until one or more contiguous Tkns are combined into a
                                // single Tkn.
    ...
    /**
     * Call this method after a group of contiguous Tkns have been combined into a single Tkn. The
     * caller has collected the indices of all quotes.
     *
     * @param remQuotes
     */
   ...
   * Each point.y is an index in processMe of the matching closing trusted quote untrustedQuotes
   * Each Point is an untrusted quote. Note: point.x or point.y in this collection is between the
   * point.x and point.y of any point in the trustedQuotes collection.
   */
  void fillQuotePairs() {

    boolean ESCAPE_STATE = false;
    //***
    // Only one of these can be true at a given time:
    boolean FOUND_TRUSTED_OPEN_QUOTE = false;
    boolean FOUND_UNTRUSTED_OPEN_QUOTE = false;
    //***
      ...
      if (ESCAPE_STATE) {
        ...
        if (ch == '\\') {
          ...
          if (TaintUtils.isTrusted(processMe, ch_index, ch_index)) {
            ...
            // If ch is enclosed between trusted single or double quotes, i.e. if
            // FOUND_TRUSTED_OPEN_QUOTE is true, then skip any/all untrusted quotes.
            if (FOUND_TRUSTED_OPEN_QUOTE) {
              // Skip - do nothing.
  ...
  /**
   * Call constructor Call fillQuotePairs Then call this method.
   *
   * This method tokenizes string processMe. Each token is put in a Tkn. The Tkns are kept in global
   * tknVector. Each token can be; a string surrounded by trusted quotes a string surrounded by
   * untrusted quotes a string delimited by white space, a single quoted string or a double quoted
   * string When a complete token is read in, it is put in holdingArea. The holding area contains a
   * group of tokens that are contiguous. When a white space char is read, or when a splitter char
   * is read, all the tokens in the holding area are combined into a single Tkn. That single token
   * is ready for a regular expression check to be applied.
   *
   * @throws MsgException
   */
  void tokenize() {
    ...
    final Vector<Tkn> holdingArea = new Vector<Tkn>(); // As tokens are gathered, store them here.
                                                       // Upon reading a whitespace char or a
                                                       // splitter char combine all tokens in the
                                                       // holding area into a single token have no
                                                       // quotes.
    ...
    while (i < processMe.length()) {
      ...
      // If we are on a opening quote that is (single or double) and (trusted or untrusted).
      if ((ch == '\"' || ch == '\'')
          && (((point = findQuotePairFromOpeningQuoteIndex(trustedQuotes, i)) != null)
    ...
    if (readingNormalTkn) {
      final Tkn tkn =
          new Tkn(TaintUtils.substring(processMe, normalTknStart, normalTknEnd + 1), UNQUOTED_TKN);
      holdingArea.addElement(tkn);
      readingNormalTkn = false;
    }
  ...
  public static void main(String[] argv) {
     ...
     * //  "a\"  \"../this \"\"\"\"\"\' my little dog\'\";\'a b\'\\\'is\\\"a string");
     * quotes.tokenize(); quotes.dump();
     * quotes.reconstructAllTkns();
     */
  }
}
