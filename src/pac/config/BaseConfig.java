...
/**
 * This provides the configuration functionality which will interface with the
 * application-specific configuration that was generated as a RuntimeConfig object.
 * 
 * @author jeikenberry
 */
public class BaseConfig extends AbstractConfig {
    ...
    // The sanitized string at a socket resulting from the backstop confinement.  (For testing purpose only.)
    ...
    */
    ...
    // If envp is null search $PATH for the first path where leaf exe
    // is located
    ...
    /**
    * If str is not trusted follow the action specified in config file
    * trusted_exec_paths_action
    *
    * @param str is the part of a $PATH variable that follows "="
    */
    public void testPathComponentForTaint(final String str, final NotifyMsg notifyMsg) throws IOException {
    ...
    //
    // If some part of $PATH is tainted, take the actions specified by
    // config directive "trusted_exec_paths_action"
    //
    // Return an array of environment variables containing only those
    // env vars in envp whose value is trusted
    public String[] processEnvp(final String[] envp, final String desc, final String cmd) throws IOException {

        if (envp == null) {
            return null;
        }

        final NotifyMsg notifyMsg = new NotifyMsg(desc, cmd, 642); // cwe-642 is "External control of critical state data"
    ...
    /**
     * @param exe is an executable file. Do nothing if exe is not a leaf
     *        (ie contains '/' char)
     *
     * If the leaf does not live on a path listed in trusted_exec_paths,
     * take the action specified by trusted_exec_paths_action
     */
    public void exeIsLeafInLegalDir(final String exe_param, final NotifyMsg notifyMsg, final String[] envp)
            throws IOException {
    ...
    // entry point
    // Return null if cmd is not a shell
    // Return o cmd: If cmd matched a shell listed in config file
    //        o A String from Config: if cmd is a leaf and it matches the leaf of some shell
    //                                path found in the config file
        ...
        if (shells != null) {
            ...
            if (shells.contains(cmd)) {
                ...
                if (!cmd.startsWith("/") && containsSlash) {
                    // If cmd is "csh" and shells contains "/bin/csh,
                    // convert "csh" to "/bin/csh"
    ...
    /**
    * Called when processing a parameter send to a java method. This method
    * identifies the name of the Chk indicated by the description sent by 
    * the caller. The caller uses the returned Chk to process the parameter.
    *
    * @param description The api description from code. Must search methoDescs
    *                    for the matching description.
    *               ex. "Runtime.exec(String, String[]name, String[]"
    ...
    */
    ...
    public void handleToctou(String method, String msg) {
        ...
        notifyMsg.append(msg);
        if (toctou_action == RunChecks.EXCEPTION_ACTION || toctou_action == RunChecks.TERMINATE_ACTION) {
    ...
    public void handleOverflow(String method, String desc) {
        ...
        if (overflow_action == RunChecks.EXCEPTION_ACTION || overflow_action == RunChecks.TERMINATE_ACTION) {
    ...
    public void handleUnderflow(String method, String desc) {
        ...
        if (underflow_action == RunChecks.EXCEPTION_ACTION || underflow_action == RunChecks.TERMINATE_ACTION) {
    ...
    public void handleInfinity(String method, String desc, boolean dangerousOp) {
        ...
        if (dangerousOp) {
            ...
            notifyMsg.append("Attempting to divide a primitive value by zero. Returning MAX_VALUE instead.");
        }
    ...
    /**
     * Called from a recursive method to check whether the relative stack size
     * is bigger than an amount specified by this config file.
     * 
    ...
    /**
     * Called from tainted (or possible tainted loops) to check whether we have
     * gone over the maximum number of iterations as specified by this
     * configuration. If so, then take whatever action was specified by this
     * configuration.
     * 
     * @param int count the number of iterations so far
     * @param boolean defTainted true iff the loop containing this check is
     *        definitely tainted
     * @param boolean shouldThrow true iff the loop containing this check is
     *        unable to break (and therefore must throw an exception).
     * @param String
     *            method the method that this was called from (i.e.
     *            "SomeClass.foo()").
     * @throws IOException
     */
    public boolean handleLoopCheck(int count, boolean defTainted, boolean shouldThrow, String method, Timestamp timeObj)
            throws IOException {
        int maxCount, action;
        ...
        if (defTainted) {
            ...
            if (timeObj != null && timeObj.isFinished()) {
                ...
                if (count < maxCount)
                    return false;
                errorMsg = "Definitely tainted loop exceeded " + maxCount + " iterations, as set by the config file.";
            }
            ...
            if (timeObj != null && timeObj.isFinished()) {
                ...
                if (count < maxCount)
                    return false;
                errorMsg = "Possibly tainted loop exceeded " + maxCount + " iterations, as set by the config file.";
            }
    ...
    /**
     * Called when Exceptions are encountered inside of a designated
     * server loop, as determined through analyis.  This method
     * will log the encountered exception and the server will continue
     * running as normal.
     *
    ...
    public void handleServerException(Throwable e, int cweNumber, String type) {
        ...
        final NotifyMsg notifyMsg = new NotifyMsg("ConfigFile.handleServerException(" + exceptionClass + ")",
                e.toString(), cweNumber);
        // set action to something other than log only, since we are
        // clearly intervening in this case...
        notifyMsg.setAction(RunChecks.REMOVE_ACTION);
    ...
    /**
     * Called in the finally block of Runnable.run() methods, to handle the cleanup of
     * improperly locked locks (to ensure we don't deadlock other threads).
     */
    public void handleThreadLocks() {
        ...
        // These appear to be the only types of locks that we can
        // check whether it's held by the current thread.
        unlock(lock);
        iter.remove();
        while (iter.hasNext()) {
            lock = iter.next();
    ...
    /**
     * This method will unlock a ReentrantLock until the current thread no
     * longer holds this lock.  If it's not a ReentrantLock, then presume
     * that we only need to unlock it once.
     * 
     * @param lock Lock
     */
        ...
        if (obj == null) {
            ...
            try {
                if ((type.getModifiers() & Modifier.FINAL) != Modifier.FINAL) {
                    // We may not have a precise type and so any downcast would be
                    // dangerous...
                    throwEx = true;
                } else {
                    obj = ClassInstrumentation.newInstance(type);
                    notifyMsg.setAction(RunChecks.LOG_ONLY_ACTION);
                    notifyMsg.append("Encountered a null object that may be dereferenced. Returning a new object [ hc: "
    ...
    public int errorCheck(int value, int value_t, int errorCode, int errorCode_t, String method, Ret ret)
            throws NoSuchMethodException, SecurityException, IOException {
        if ((value_t & TaintValues.TRUST_MASK) != TaintValues.TRUSTED && value == errorCode) {
            NotifyMsg notifyMsg = new NotifyMsg(method, method, 253);
            notifyMsg.setAction(RunChecks.EXCEPTION_ACTION);
            notifyMsg.append("Tainted value matched an unhandled error code: " + value + " == " + errorCode);
            notifyMsg.prepareForExceptionOrTerminate(CleartrackException.class.getConstructor(String.class),
                                                     RunChecks.EXCEPTION_ACTION);
            Notify.notifyAndRespond(notifyMsg);
        }
    ...
    public long errorCheck(long value, int value_t, long errorCode, int errorCode_t, String method, Ret ret)
            throws NoSuchMethodException, SecurityException, IOException {
        if ((value_t & TaintValues.TRUST_MASK) != TaintValues.TRUSTED && value == errorCode) {
            NotifyMsg notifyMsg = new NotifyMsg(method, method, 253);
            notifyMsg.setAction(RunChecks.EXCEPTION_ACTION);
    ...
    public boolean errorCheck(boolean value, int value_t, boolean errorCode, int errorCode_t, String method, Ret ret)
            throws NoSuchMethodException, SecurityException, IOException {
        if ((value_t & TaintValues.TRUST_MASK) != TaintValues.TRUSTED && value == errorCode) {
            NotifyMsg notifyMsg = new NotifyMsg(method, method, 253);
            notifyMsg.setAction(RunChecks.EXCEPTION_ACTION);
    ...
    /**
     * Takes the appropriate action when encountering a file whose type is
     * not allowed by the system.  Depending on the configuration file the program
     * will either throw an exception, terminate, or jail the file to a safe
     * directory.
     *
    ...
    public void handleUntrustedFileType(File file, String errorMessage) throws IOException {
        ...
        if (action == RunChecks.JAIL_ACTION) {
            ...
            notifyMsg.append((success) ? (".. File successfully jailed.\n") : (".. Unable to jail file."));
            if (!success) {
        ...
        if (action == RunChecks.REMOVE_ACTION) {
            ...
            notifyMsg.append((success) ? (".. File successfully removed.\n") : (".. Unable to remove file.\n"));
            if (!success) {
                // If we were unable to remove the file then terminate the application to
                // prevent the application from using the malicious file.
    ...
    public void addSuppressed(Throwable origException, Throwable nestedException) throws IOException {
        ...
        notifyMsg.setAction(RunChecks.LOG_ONLY_ACTION);
        notifyMsg.append("Uncaught exception thrown inside of catch-block. " + "Re-throwing the original exception.");
    ...
    public int reportNullByte(String description, String message) throws IOException {
        ...
        if (null_byte_action == RunChecks.EXCEPTION_ACTION || null_byte_action == RunChecks.TERMINATE_ACTION) {
    ...
    /**
     * Called when entering a catch block.  This will push a new
     * NotifyMsg object for the current thread's exception stack.
     * 
    ...
    /**
     * Called when exiting a catch block.  This will pop a
     * NotifyMsg object off the current thread's exception
     * stack and take action only if there was a trusted
     * write on that NotifyMsg.
    ...
    /**
     * Identifies the language of the output at a client socket, performs the corresponding 
     * language check and returns the resulting sanitized result.
    ...
    */
     ...
     * @throws IOException 
     */
    public String runServerSocketInputCheck(String desc, String processMe, Ret ret) throws IOException {
     ...
     * @throws IOException 
     */
    private String checkHTTPRequest(String desc, String processMe, Matcher m, Ret ret) throws IOException {
        ...
        String queryStr = path.substring(queryStrStart + 1, TaintValues.TRUSTED, queryStrEnd, TaintValues.TRUSTED, ret);
        if (!isLegalQueryString(queryStr)) {
            ...
            if (malformed_http_request_action == RunChecks.REPLACE_ACTION) {
                ...
                sb.append(meth, ret);
                sb.append(path.substring(0, TaintValues.TRUSTED, queryStrStart, TaintValues.TRUSTED, ret), ret);
    ...
    private static boolean isLegalKeyOrValue(String key) {
        for (int i = 0; i < key.length(); i++) {
            ...
            case '_':
                continue;
            case '%':
                if (i + 2 >= key.length() || !isHEXDigit(key.charAt(i + 1)) || !isHEXDigit(key.charAt(i + 2)))
                    return false;
                else
                    continue;
            default:
    ...
    /**
     * Sanitizes the query string in a URL by encoding all the illegal characters in the
     * parameter specs using the URL encoding.
    ...
    /* The given string might be (partially) encoded. So we need to decode it first
     * before performing the URL encoding.
     */
    private String urlEncode(String str, Ret ret) throws IOException {
        return URLEncoderInstrumentation.encode(URLDecoderInstrumentation.decode(str, "UTF-8", ret), "UTF-8", ret);
    }
    ...
    /**
     * Run tainted HTTP redirect check on the output string at a server socket.
     * redirect response.
     * @param desc The API method description WITHOUT return type and WITHOUT parameter identifiers
     * @param processMe The socket output string to be checked
     * @return The sanitized socket output string
     */
    public String runServerSocketOutputCheck(String desc, String processMe, Ret ret) throws IOException {
     ...
     */
    private String checkHTTPRedirectResponse(String desc, String processMe, Matcher m, Ret ret) throws IOException {
        ...
        if (untrustedRedirectHeader != null) {
            ...
            notifyMsg.append("URL redirection to untrusted site: " + "HTTP response contains tainted location: ");
