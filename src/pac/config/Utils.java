...
//
// The file contains class name of parent classes and class names of that classes immediate
// superclasses. This class read the file to build Hashtable in ParentTree.
//
...
public class Utils {
  ...
  public static int nextTrustedQuote(final String str, final char quoteChar, final char escapeChar,
      final int start) {
    ...
    /*
     * rjk int quoteIndex = str.indexOf(quoteChar, start); boolean selfEscaped =
    ...
    while (true) {
      ...
      if (selfEscaped) {
        ...
        if (firstTrusted && TaintUtils.charEquals(secondChar, escapeChar)) { // rjk
          ...
          // Rule 1 and 2 apply.
          quoteIndex++;
        } else if (!firstTrusted && secondTrusted
            && TaintUtils.charEquals(secondChar, escapeChar)) {
        ...
        // rjk (str.charAt(quoteIndex-1) != escapeChar || !TaintUtils.isTrusted(str, quoteIndex-1,
        // quoteIndex-1))) {
        // Escape character and the quote is trusted, but the previous character is not a trusted
        // escape character.
        break;
      }
   ...
   * @return Elements are Points. Point.x indexes the opening quote.. Point.y indexes closing quote.
   *         processMe.substring(point.x, point.y) produces a quoted string contained in processMe
   *         without the quotes.
   */
   ...
   * @throws MsgException
   */
  public static String getTokenAtIndexWithTaint(final String line, final int tknIndex)
      throws MsgException {
    final String[] ray =
        TaintUtils.split(line, TaintUtils.trust(TaintUtils.whiteSpaceRegExp() + "+"));
    if (ray.length < tknIndex + 1) {
      throw new MsgException("token at index " + tknIndex + " out of bounds for line: " + line);
    }
  ...
  /**
   * From an Exception package spec build a constructor. Test the constructor before returning the
   * constructor.
   *
  ...
  private static void testConstructor(final Constructor<?> constructor) throws MsgException {
    ...
    if (!caught) {
      throw new MsgException(
          "Unable to create a working RuntimeException: " + constructor.getName());
    }
  }
  ...
  static RuntimeException newException(final Constructor c, final String msg) throws MsgException {
    try {
      ...
      final RuntimeException ex = (RuntimeException) obj; // ClassCastException
