...
/**
 * The cleartrack configuration abstraction layer. This class is extended by both the
 * instrumentation configuration and the runtime configuration. This class really only needs to
 * define methods that are needed by both the instrumentation-time configuration and the run-time
 * configuration.
 * 
 * @author jeikenberry
 */
public abstract class AbstractConfig {
  ...
  protected static final Pattern HTTPREQUEST_PAT = Pattern.compile(
      "(GET\\s+|HEAD\\s+|POST\\s+|OPTIONS\\s+|PUT\\s+|DELETE\\s+|TRACE\\s+|CONNECT\\s+)(.+)(\\s+HTTP/[0-9.]+)");
  protected static final Pattern HTTPRESPONSE_PAT = Pattern.compile(
      "HTTP/[0-9.]+\\s+[0-9]{3}\\s+.*\\r\\n([-a-zA-Z0-9]+:\\s+.*\\r\\n)+\\r\\n[\\x00-\\x7F]*");
  ...
  /**
   * Collection of method descriptors (ex. "desc Runtime.exec(String[]<check_name>)").
   */
  protected HashMap<String, Desc> methodDescMap;
  ...
  /**
   * List of directories. If untrusted String Runtime.exe or ProcessBuilder is running an executable
   * given by "leaf_name" where leaf_name and contains no path delimiters.
   *   o Then if that leaf executable does not live in any of the directories listed in
   *     trustedExecPaths, then tack action specified by trusted_exec_paths_action.
   *   o Else if that leaf executable does live in one of the directories listed in trustedExecPaths
   *     Then if the first dir in env var PATH where leaf_name is exists differs from the
   *     trustedExecPaths dir, then take the action specified by trusted_exec_paths_action.
   */
  protected Vector<String> trustedExecPaths;
  ...
  /**
   * What to do when an executable leaf is specified and that leaf does not exist in one of the dirs
   * listed in config file as a trusted_exe_paths (or it does exist but PATH env var says the leaf
   * should be executed from some other dir).
   */
  protected int trusted_exec_paths_action;
  ...
  protected int overflow_action, underflow_action, infinity_action, overflow_print_action,
      underflow_print_action, infinity_print_action;
