...
public class CmdSplitter {
    ...
    private String processMeA; // string to left of trusted command separator.
                               // will not end with white space
    private String dividerAndSpace; // The divider string plus all contiguous white space
                                    // to the left and all contiguous white space to right.
    private String processMeB; // string to right of trusted command separator.
                               // will not begin with white space
    ...
    /**
    *
    * @return the string that is to the left of the left-most trusted command separator string.
    *         The string to left (processMeA) will not end with contiguous white space - trusted or not
    ...
    */
    String getA() {
        ...
        while (startSearch < processMe.length() - 1) {
            ...
            for (int i = 0; i < dividers.length; i++) {
                ...
                int found = -1;
                for (int index = 0; (found == -1) && ((index = TaintUtils
                        .stringIndexOf(processMe, divider, index + startSearch)) != -1); index++) {
            ...
            if (loBologne < Integer.MAX_VALUE) {
                ...
                // If loBologne is surrounded by trusted quotes, advance startSearch to the right of the quoted area
        ...
        if (loBologne < Integer.MAX_VALUE) {
            ...
            // Point left at one character to right of the last non-whitespace char that precedes loBologne
            int left = loBologne - 1;
            while (left > 0 && (processMe.substring(left, left + 1).matches(TaintUtils.whiteSpaceRegExp()))) {
                left--;
            }
            if (left < loBologne && !processMe.substring(left, left + 1).matches(TaintUtils.whiteSpaceRegExp())) {
                left++;
            }
    ...
    /**
    *
    * @return the divider string plus and white space that was to left and right of the divider string
    */
    ...
    /**
     *
     * @return the string that is to the right of the left most trusted divider string.
     *         The returned string does not contain contiguous white space
     *         (trusted or not) to right of the trusted divider string.
     *         If preceding white space is not eliminated, then when caller does split("\\s+")
     *         on the returned string, "" is returned as the first token in the returned array.
     */
    String getB() {
        return processMeB;
    }
