...
public final class FileOutputStreamInstrumentation extends OutputStreamInstrumentation {
    ...
    public static final FileOutputStream init(File file, Ret ret)
        ...
        // We do not need to worry about a resource leak, presuming that the
        // application properly closes this FileOutputStream. This is because
        // both streams share the same file descriptor.
    ...
    public static final FileOutputStream init(File file, boolean append, int append_t, Ret ret)
        ...
        if (append) {
            // We are appending, so we don't need to worry about overwriting the
            // file.
    ...
    @InstrumentationMethod
    public static final void write(FileOutputStream fos, ByteArrayTaint b, Ret ret) throws IOException {
    ...
    @InstrumentationMethod
    public static final void write(FileOutputStream fos, ByteArrayTaint b, int off, int off_t, int len, int len_t,
                                   Ret ret)
            throws IOException {
        if (RuntimeInstrumentation.isShellOutputStream(fos)) {
            String s = RuntimeInstrumentation.checkCommand(new String(b, off, off_t, len, len_t, ret), ret);
            fos.write((byte[]) s.getBytes(ret).value);
        } else {
    ...
    public static final void handleFileClose(File file) throws IOException {
        /* FIXME: Not sure what we want to allow if it there are no read
         * permissions, but let it through for now.  If there are no 
         * permissions to read this file, then we will get an error when
        ...
        // If we were unable to determine file type for whatever reason
        ...
        if (mimeTypes != null) {
            ...
            if (allTypesAllowed.contains(contentType)) {
                ...
                if (contentTypes == null) {
                    ...
                    BaseConfig.getInstance().handleUntrustedFileType(file, "file '" + file.getName()
                            + "' has MIME type '" + contentType + "' when it is expecting one of: " + contentTypes);
                }
            } else {
                BaseConfig.getInstance().handleUntrustedFileType(file, "file '" + file.getName() + "' has MIME type '"
                        + contentType + "' which is not allowed according to the configuration file.");
            }
        }
    }
