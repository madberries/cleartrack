...
public final class SocketInstrumentation {
    ...
    @InstrumentationMethod(invocationType = InvocationType.CONSTRUCTOR, instrumentationLocation = InstrumentationLocation.APP)
    public static final Socket init(InetAddress address, int port, InetAddress localAddr, int localPort)
            throws IOException {
    ...
    @Deprecated
    @InstrumentationMethod(invocationType = InvocationType.CONSTRUCTOR, instrumentationLocation = InstrumentationLocation.APP)
    public static final Socket init(String host, int port, boolean stream) throws UnknownHostException, IOException {
        Socket s = new Socket(host, port, stream);
        setSocketTimeout(s);
        return s;
    }
    ...
    public static final String checkSocketOutput(Writer sink, String desc, String s, Ret ret) throws IOException {
        String result = s;
    ...
    public static final String checkSocketOutput(OutputStream sink, String desc, String s, Ret ret) throws IOException {
        String result = s;
        if (sink.ss_socktype == SocketType.CLIENT_SINK) {
            result = BaseConfig.getInstance().runClientSocketOutputCheck(desc, s, ret);
        } else if (sink.ss_socktype == SocketType.SERVER_SINK) {
    ...
    public static final ByteArrayTaint checkSocketOutput(OutputStream sink, String desc, ByteArrayTaint b, Ret ret) {
        if (sink.ss_socktype == SocketType.CLIENT_SINK) {
    ...
    protected static final void setSocketTimeout(Socket socket, boolean server) throws SocketException {
