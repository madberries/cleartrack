...
public final class RuntimeInstrumentation {
  ...
  protected synchronized static final boolean isShellOutputStream(FileOutputStream fos)
      throws IOException {
  ...
  public static final Process exec(Runtime runtime, String cmd, String[] envp) throws IOException {
    ...
    final String[] newEnvp =
        BaseConfig.getInstance().processEnvp(envp, "Runtime.exec(String, String[])", cmd); // IOException
  ...
  @InstrumentationMethod(instrumentationLocation = InstrumentationLocation.APP)
  public static final Process exec(Runtime runtime, String cmd, String[] envp, File dir)
      throws IOException {
    ...
    final String[] newEnvp =
        BaseConfig.getInstance().processEnvp(envp, "Runtime.exec(String, String[], File)", cmd); // IOException
  ...
  public static final Process exec(Runtime runtime, String[] cmd) throws IOException {
    ...
    // Magic check: if shell_list is defined in config file, check if cmd[0]vis a shell and alter
    // cmd[0] to be complete path to shell if needed.
    // Ignore return value
    // More magic: if cmd[0] is a shell and if check execShellOnElementTwo is defined, then
    // execShellOnElementTwo is applied to cmd[2].
  ...
  @InstrumentationMethod(instrumentationLocation = InstrumentationLocation.APP)
  public static final Process exec(Runtime runtime, String[] cmd, String[] envp)
      throws IOException {
    ...
    final String[] newEnvp =
        BaseConfig.getInstance().processEnvp(envp, "Runtime.exec(String[], String[])", command); // IOException

    RunChecks.runFromDescArray(cmd, notifyMsg, 0, newEnvp); // IOException - attack with
                                                            // action=exception
  ...
  public static final Process exec(Runtime runtime, String[] cmd, String[] envp, File dir)
    ...
    final NotifyMsg notifyMsg =
        new NotifyMsg("Runtime.exec(String[], String[], File)", command, 78);
    notifyMsg.addTaintOutput(cmd);
    notifyMsg.addTaintOutput(envp);

    final String[] newEnvp = BaseConfig.getInstance().processEnvp(envp,
        "Runtime.exec(String[], String[], File)", command); // IOException
