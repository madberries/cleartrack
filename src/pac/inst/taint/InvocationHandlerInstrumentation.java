...
public final class InvocationHandlerInstrumentation {
    ...
    @InstrumentationMethod(inline = true, canExtend = true, instrumentationLocation = InstrumentationLocation.TRANS)
    public static final Object invoke(InvocationHandler invHandler, Object proxy, Method method, Object[] args, Ret ret)
            throws Throwable {
        String methodName = method.getName();
        Class<?>[] paramTypes = method.getParameterTypes();
        if (paramTypes.length > 0 && paramTypes[paramTypes.length - 1] == Ret.class) {
            // attempt to get the original method since reflection is expecting all methods
            // to be uninstrumented...
            try {
                Method uninstMethod = method.getDeclaringClass()
                        .getDeclaredMethod(methodName, MethodInstrumentation.getUninstrumentedTypes(paramTypes));
