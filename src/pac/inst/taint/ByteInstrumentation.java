...
public final class ByteInstrumentation {
    ...
    @InstrumentationMethod(invocationType = InvocationType.STATIC, name = "valueOf", descriptor = "(BILpac/util/Ret;)Ljava/lang/Byte;")
    ...
    public static final byte parseByte(String s, Ret ret) {
        ...
        try {
            ...
            if (TaintUtils.isTainted(s)) {
                NotifyMsg notifyMsg = new NotifyMsg("Byte.parseByte(String)", "Byte.parseByte(" + s + ")", 20);
                notifyMsg.setAction(RunChecks.REPLACE_ACTION);
                notifyMsg.append("Tainted string \"" + s + "\" is not a parseable byte (returning the default value: "
                        + DEFAULT_VALUE + ")");
