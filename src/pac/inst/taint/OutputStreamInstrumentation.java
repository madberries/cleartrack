...
public class OutputStreamInstrumentation {
  ...
  protected static final void blockTrustedChars(ByteArrayTaint b, int offset, int length,
      NotifyMsg notifyMsg) {
    ...
    if (BaseConfig.getInstance().getTrustedExceptionAction() != RunChecks.REPLACE_ACTION) {
      for (int i = offset; i < last; i++) {
        if ((b.taint[i] & TaintValues.TRUST_MASK) == TaintValues.TAINTED
            || isAllowableTrustedChar(valueArr[i]))
          continue;
  ...
  public static final void write(OutputStream os, ByteArrayTaint b, Ret ret) throws IOException {
    ...
    if (stack == null || stack.isEmpty()) {
      if (os instanceof FileOutputStream) {
        ...
        ByteArrayTaint bwrap = SocketInstrumentation.checkSocketOutput(os,
            "OutputStream.write(byte[], int, int)", b, ret);
        os.write(bwrap, ret);
      }
      return;
    }
  ...
  @InstrumentationMethod(canExtend = true)
  public static final void write(OutputStream os, ByteArrayTaint b, int off, int off_t, int len,
      int len_t, Ret ret) throws IOException {
    Stack<NotifyMsg> stack = Thread.currentThread().ss_estack;
    if (stack == null || stack.isEmpty()) {
      if (os instanceof FileOutputStream) {
        FileOutputStreamInstrumentation.write((FileOutputStream) os, b, off, off_t, len, len_t,
            ret);
      } else {
        ByteArrayTaint bwrap = SocketInstrumentation.checkSocketOutput(os,
            "OutputStream.write(byte[], int, int)", b, ret);
