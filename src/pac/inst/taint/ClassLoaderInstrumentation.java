...
public final class ClassLoaderInstrumentation {
    ...
    //	public static final Enumeration<URL> findResources(ClassLoader cl, String name, Ret ret) throws IOException {
    //		final Enumeration<URL> urlEnum = cl.findResources(name, ret);
    //		if (urlEnum == null)
    //			return null;
    //		return new URLTaintEnumeration(urlEnum,
    //				TaintUtils.isTrusted(name) ||
    //				!TaintUtils.isTracked(name));
    //	}

    @InstrumentationMethod(canExtend = true)
    public static final Enumeration<URL> getResources(ClassLoader cl, String name, Ret ret) throws IOException {
        final Enumeration<URL> urlEnum = cl.getResources(name, ret);
        if (urlEnum == null)
            return null;
        return new URLTaintEnumeration(urlEnum, TaintUtils.isTrusted(name) || !TaintUtils.isTracked(name));
    }
    ...
    //		if (url == null)
    //			return null;
    //		boolean isTrusted = TaintUtils.isTrusted(name) ||
    //				!TaintUtils.isTracked(name);
    //		if (isTrusted) {
