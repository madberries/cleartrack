...
public final class FileInstrumentation {
    ...
    protected static final Map<String, FileStat> attrMap = Collections
            .synchronizedMap(new WeakHashMap<String, FileStat>());
     ...
     * @throws IOException
     */
    protected static final FileStat timeOfCheck(File file, boolean symbolic, boolean exists) throws IOException {
        try {
            // Make sure that a NullPointerException is thrown in the event that
            // file is null.  Otherwise a native call on a null object could crash
            // the JVM.
    ...
    /**
     * Called to ensure that the file we are using is in a consistent state with
     * the file object that was checked prior to this usage.
     * 
    ...
    /**
     * Recursively copies taint from the original path to the resolved canonical
     * path by starting from the root and working up.
     * 
    ...
    public static final File createTempFile(String prefix, String suffix, File directory, Ret ret) throws IOException {
    ...
    @InstrumentationMethod
    public static final String getAbsolutePath(File file, Ret ret) throws NoSuchMethodException, SecurityException {
    ...
    @InstrumentationMethod
    public static final File getAbsoluteFile(File file, Ret ret) throws NoSuchMethodException, SecurityException {
    ...
    /**
     * @param file If this file's file.canonicalFile() is tracked, return it.
     *             If this file's file.canonicalFile() is not tracked, taint or trust the String based
     *             on file.toString() trust/taint, and return it
    ...
    public static final String getCanonicalPath(File file, Ret ret)
        ...
        try {
            ...
            // It's possible that the canonical path exists, but the original path
            // does not resolve at some point (i.e. nonexisting_dir/..).  So, let's
            // presume this is ok.
        ...
        if (canonStat == null && fileStat == null) {
            ...
            if (!fileStat.equals(canonStat)) {
                BaseConfig.getInstance()
                        .handleToctou("Files.getCanonicalPath()",
    ...
    /**
    * @param file If this file's file.canonicalFile().toString() is tracked, return it.
    *             If this file's file.canonicalFile().toString() is not tracked, taint or trust
    *             the canonical String based on file.toString() trust/taint, create a File from
    *             the canonical String, and return that File
    * @return A File whose toString() is tracked and whose trust/untrust is derived from input file
    * @throws IOException
    * @throws SecurityException 
    * @throws NoSuchMethodException 
