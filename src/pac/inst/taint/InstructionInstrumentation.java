...
/**
 * Instrumentation methods to be applied to individual JVM opcodes.  Like any other method
 * wrapper, these can be inlined for further optimization.
 * 
 * @author ppiselli
 */
...
public final class InstructionInstrumentation {
    ...
    @InstrumentationMethod(invocationType = InvocationType.STATIC, instrumentationLocation = InstrumentationLocation.COMPAT, inline = true)
    ...
    public static final int iadd(int op1, int op1_t, int op2, int op2_t, Ret ret) {
        ...
        if (!TaintValues.isSet(op1_t, TaintValues.BITWISE_EXPR) || !TaintValues.isSet(op2_t, TaintValues.BITWISE_EXPR))
    ...
    public static final int idiv(int op1, int op1_t, int op2, int op2_t, Ret ret) {
        ...
        if (op1 == Integer.MIN_VALUE && op2 == -1) {
            ...
            notifyMsg.append("Attempting to divide " + op1 + " by zero (" + TaintUtils.toTrustString(op2_t)
    ...
    public static final int handleUnsafeUSHR(int result, int result_t, int mask, int mask_t, Ret ret)
        ...
        if (x != result && (result_t & TaintValues.TRUST_MASK) != TaintValues.TRUSTED) {
            ...
            notifyMsg.append("Performed a dangerous unsigned right-shift on a tainted value. Masking off sign bits.");
            Notify.notifyAndRespond(notifyMsg);
        }
        return x;
    }
