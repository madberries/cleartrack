...
public final class LongInstrumentation {
    ...
    @InstrumentationMethod(invocationType = InvocationType.STATIC, name = "valueOf", descriptor = "(JILpac/util/Ret;)Ljava/lang/Long;")
    ...
    public static final long parseLong(String s, Ret ret) {
        ...
        try {
            ...
            if (TaintUtils.isTainted(s)) {
                NotifyMsg notifyMsg = new NotifyMsg("Long.parseLong(String)", "Long.parseLong(" + s + ")", 20);
                notifyMsg.setAction(RunChecks.REPLACE_ACTION);
                notifyMsg.append("Tainted string \"" + s + "\" is not a parseable long (returning the default value: "
                        + DEFAULT_VALUE + ")");
