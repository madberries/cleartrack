...
public final class IntegerInstrumentation {
  ...
  @InstrumentationMethod(invocationType = InvocationType.STATIC, name = "valueOf",
      descriptor = "(IILpac/util/Ret;)Ljava/lang/Integer;")
  public static final CleartrackInteger valueOf(int i, int i_t, Ret ret) {
    if (i >= CleartrackIntegerCache.low && i <= CleartrackIntegerCache.high)
      return CleartrackIntegerCache.cache[i_t & TaintValues.TRUST_MASK][i
          + (-CleartrackIntegerCache.low)];
  ...
  public static final int parseInt(String s, int radix, int radix_t, Ret ret) {
    ...
    try {
      ...
      if (TaintUtils.isTainted(s)) {
        NotifyMsg notifyMsg =
            new NotifyMsg("Integer.parseInt(String)", "Integer.parseInt(" + s + ")", 20);
        notifyMsg.setAction(RunChecks.REPLACE_ACTION);
        notifyMsg.append("Tainted string \"" + s
            + "\" is not a parseable int (returning the default value: " + DEFAULT_VALUE + ")");
