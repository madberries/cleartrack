...
public final class ListInstrumentation extends CollectionInstrumentation {
    ...
    /*
     * These methods refer to instrumented methods that do not exist in List, because
     * list is not instrumented.  However, they will still be used, since we set
     * the skippedDescriptor field in the InstrumentationMethod annotation.
     */

    @InstrumentationMethod(instrumentationLocation = InstrumentationLocation.APP, canExtend = true, skippedDescriptor = "(I)Ljava/lang/Object;")
    public static final <T> T get(List<T> list, int idx, int idx_t, Ret ret) {
        try {
            Overflows.checkOverflow(idx_t, "List.get()", "object", null, idx);
            if ((idx_t & TaintValues.TRUST_MASK) != TaintValues.TRUSTED && (idx < 0 || idx >= list.size())) { // index out of bounds
    ...
    @InstrumentationMethod(instrumentationLocation = InstrumentationLocation.APP, canExtend = true, skippedDescriptor = "(ILjava/lang/Object;)Ljava/lang/Object;")
    ...
    @InstrumentationMethod(instrumentationLocation = InstrumentationLocation.APP, canExtend = true, skippedDescriptor = "(Ljava/lang/Object;)I")
