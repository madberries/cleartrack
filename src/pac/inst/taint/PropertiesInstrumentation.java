...
public final class PropertiesInstrumentation {
  ...
  public static final void load(Properties props, InputStream inStream, Ret ret)
      throws IOException {
    // Keep around existing properties so we can easily tell what properties had changed.
    Map<?, ?> oldProps = (Map<?, ?>) props.clone();

    props.load(inStream, ret); // IOException

    // For each new or altered property, trust or taint based on the taint of the stream.
  ...
  /**
   * Sets the taint on the acquired property. Properties are trusted if they match their correct
   * (gold) value or if the config file trusts them.
   */
  @InstrumentationMethod
  public static final String getProperty(Properties props, String key, Ret ret) {
    ...
    // Calling notifyAndRespond() causes infinite recursion: appending to log file calls
    // getProperty().
    final NotifyMsg notifyMsg =
        new NotifyMsg("Properties.getProperty(String)", "Properties.getProperty(" + key + ")");

    // Notify.log ("%s istracked = %b\n", key, TaintValues.isTracked(res));
     ...
     * try {
     *   // ORIG FIXED
     *   if (! TaintValues.isTracked(res)) {
     *     RunChecks.checkLegalProperty(key, res, notifyMsg); // MssgException
     *     if (notifyMsg.isMessage()) { // Output message to log file only. Because cwe is 0, message
     *                                  // will not go to test harness
    ...
    /*
     *   // ORIG
     *   if (!RunChecks.checkLegalProperty(key, res, notifyMsg)) {
  ...
  @InstrumentationMethod
  public static final String getProperty(Properties props, String key, String defaultValue,
      Ret ret) {
    String res = props.getProperty(key, defaultValue);
    if (res == null)
      return null;

    // Must check in_application first, because enter_check generates  recursive calls.
