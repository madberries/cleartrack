    ...
    public static final float parseFloat(String s, Ret ret) {
        ...
        try {
            ...
            if (TaintUtils.isTainted(s)) {
                NotifyMsg notifyMsg = new NotifyMsg("Float.parseFloat(String)", "Float.parseFloat(" + s + ")", 20);
                notifyMsg.setAction(RunChecks.REPLACE_ACTION);
                notifyMsg.append("Tainted string \"" + s + "\" is not a parseable float (returning the default value: "
                        + DEFAULT_VALUE + ")");
