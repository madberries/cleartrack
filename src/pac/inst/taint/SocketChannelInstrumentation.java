...
public class SocketChannelInstrumentation {
    ...
    public static final int read(SocketChannel sc, ByteBuffer dat) throws IOException {
        // It doesn't make sense to timeout a SocketChannel that is
        // non-blocking.  In this case, invoke the original read().
        if (!sc.isBlocking())
            return sc.read(dat);
    ...
    @InstrumentationMethod
    public static final long read(SocketChannel sc, ByteBuffer[] dat, int offset, int length) throws IOException {
        // It doesn't make sense to timeout a SocketChannel that is
        // non-blocking.  In this case, invoke the original read().
        if (!sc.isBlocking())
            return sc.read(dat);
    ...
    public static final int write(SocketChannel channel, ByteBuffer src) throws IOException {
        if (!isServersideSocketChannel(channel)) {
            src = BaseConfig.getInstance().runClientSocketOutputCheck("SocketChannel.write(ByteBuffer)", src,
                                                                      new Ret());
        }
