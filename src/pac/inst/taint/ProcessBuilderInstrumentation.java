...
public final class ProcessBuilderInstrumentation {
  ...
  public static final Process start(ProcessBuilder pb) throws IOException {
    ...
    // In order for pb.environment() to return a properly tainted Map, must first call
    // SystemInstrumentation.getenv().
    Ret ret = new Ret();
    ...
    // The return array contains only those env vars in envp_array whose value is trusted.
    final String[] new_env_array =
        BaseConfig.getInstance().processEnvp(env_array, "ProcessBuilder.start(String[])", cmd_str); // IOException
