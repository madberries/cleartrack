...
public final class StringBuilderInstrumentation {
    ...
    public static final StringBuilder append(StringBuilder sb, char c, int c_t, Ret ret) throws IOException {
        if (c == 0 && (c_t & TaintValues.TAINTED) == TaintValues.TAINTED) { // CWE-626 Null Byte Interaction Error
            int replaceChar = BaseConfig.getInstance()
                    .reportNullByte("StringBuilder.append(char)",
    ...
    @InstrumentationMethod(instrumentationLocation = InstrumentationLocation.APP)
    public static final StringBuilder insert(StringBuilder sb, int offset, int offset_t, char c, int c_t, Ret ret)
            throws IOException {
        if (c == 0 && (c_t & TaintValues.TAINTED) == TaintValues.TAINTED) { // CWE-626 Null Byte Interaction Error
            int replaceChar = BaseConfig.getInstance()
                    .reportNullByte("StringBuilder.insert(char)",
                                    "Attempting to insert a null character into the StringBuilder '" + sb
                                            + "' at index " + offset + ".");
            if (replaceChar == 0)
                return sb;
            if (replaceChar > 0) {
    ...
    @InstrumentationMethod(instrumentationLocation = InstrumentationLocation.APP)
    public static final void setCharAt(StringBuilder sb, int index, int index_t, char c, int c_t, Ret ret)
            throws IOException {
        if (c == 0 && (c_t & TaintValues.TAINTED) == TaintValues.TAINTED) // CWE-626 Null Byte Interaction Error
        {
            int replaceChar = BaseConfig.getInstance()
                    .reportNullByte("StringBuilder.setCharAt(char)",
                                    "Attempting to set a null character in the StringBuilder '" + sb + "' at index "
                                            + index + ".");
            if (replaceChar == 0)
                return;
            if (replaceChar > 0) {
