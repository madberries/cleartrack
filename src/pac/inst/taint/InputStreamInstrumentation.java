...
public class InputStreamInstrumentation {
  ...
  /*
   * TODO put this back in once I've implemented this in such a way that we are not gratuitously
   * reporting CWE-252's.
   */
  ...
  private static final int zeroUnwrittenElements(ByteArrayTaint b, int offset, int length,
      int bytesRead, String method) {
  ...
  private static final int zeroUnwrittenElementsAndTaint(ByteArrayTaint b, int offset, int length,
      int bytesRead, int taint, String method) {
    // In the event of EOF, we should zero out all bytes in the byte array.
    int bytesReadVal = bytesRead < 0 ? 0 : bytesRead;
  ...
  public static final int read(InputStream is, ByteArrayTaint b, Ret ret) throws IOException {
    try {
      ...
      if (is.ss_hasUniformTaint) {
        ...
        return zeroUnwrittenElements(b, 0, valueArr.length, is.read(b, ret),
            "InputStream.read(byte[])");
      }
    } catch (SocketTimeoutException e) {
      socketTimeout(e);
  ...
  @InstrumentationMethod(canExtend = true, instrumentationLocation = InstrumentationLocation.JDK,
      name = "read", descriptor = "(Lpac/wrap/ByteArrayTaint;Lpac/util/Ret;)I")
  ...
  @InstrumentationMethod(canExtend = true, instrumentationLocation = InstrumentationLocation.APP)
  public static final int read(InputStream is, ByteArrayTaint b, int offset, int offset_t,
      int length, int length_t, Ret ret) throws IOException {
    try {
      if (is.ss_hasUniformTaint) {
        return zeroUnwrittenElementsAndTaint(b, offset, length, is.read(b.value, offset, length),
            is.ss_taint, "InputStream.read(byte[], int, int)");
      } else {
        return zeroUnwrittenElements(b, offset, length,
            is.read(b, offset, offset_t, length, length_t, ret),
  ...
  @InstrumentationMethod(canExtend = true, instrumentationLocation = InstrumentationLocation.JDK,
      name = "read", descriptor = "(Lpac/wrap/ByteArrayTaint;IIIILpac/util/Ret;)I")
  public static final int read_jdk(InputStream is, ByteArrayTaint b, int offset, int offset_t,
      int length, int length_t, Ret ret) throws IOException {
    try {
      if (is.ss_hasUniformTaint) {
