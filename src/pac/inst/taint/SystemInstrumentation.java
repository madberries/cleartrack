...
public final class SystemInstrumentation {
  ...
  public static final String getProperty(String key, Ret ret) {
    ...
    if (!BaseConfig.getInstance().untrustedKeyCanAccessPropertyValue()
        && !TaintUtils.isTrusted(key)) {
      final NotifyMsg notifyMsg =
          new NotifyMsg("System.getProperty(String)", "System.getProperty(" + key + ")", 642);
  ...
  public static final Map<String, String> getenv(Ret ret) {
    ...
    // GOLD
    Map<String, String> map = System.getenv(ret); // Returns immutable map.
  ...
  /**
   * Set the taint on environment variables according to the config file If environment variables
   * are trusted, just trust the current value. If env vars are not trusted and they match a good
   * value, trust them. otherwise taint them. Note that we never change a value here (i.e., we don't
   * repair).
   * 
   * @param name of the environment variable.
   * @param ret the "returned taint" object.
   * @return String
   */
  @InstrumentationMethod(invocationType = InvocationType.STATIC)
  ...
  @InstrumentationMethod(invocationType = InvocationType.STATIC)
  public static final void arraycopy(Object src, int srcPos, int srcPos_t, Object dest, int destPos,
      int destPos_t, int length, int length_t, Ret d) {
