...
public final class ThreadInstrumentation {
  ...
  public static final void join(Thread t) throws InterruptedException {
    ...
    try {
      // Mark t as the thread curThread is joining with, so we may catch the deadlock in case t
      // blocks indefinitely.
      ...
      // We have properly completed the join on t, so unset the current threads join thread.
      if (curThread != null)
  ...
  public static final void join(Thread t, long millis) throws InterruptedException {
    ...
    try {
      if (millis == 0) {
        curThread = Thread.currentThread();
        // Mark t as the thread curThread is joining with, so we may catch the deadlock in case t
        // blocks indefinitely. We only need to do this if we are not waiting indefinitely.
        curThread.ss_join = t;
      }
      t.join(millis);
    } catch (InterruptedException e) {
  ...
  public static final void run(Thread thread) {
    if (thread.isAlive()) {
      // Just in case run() can recursively call itself or super.run() is called. This should not
      // loop recursively, since this replacement is done only on instrumented application methods.
      // So, ChildThread.run(ret) will be replaced with ThreadInstrumentation.run(ret) ->
      // ChildThread.run() -> ChildThread.run(new Ret()).
      thread.run();
    } else {
