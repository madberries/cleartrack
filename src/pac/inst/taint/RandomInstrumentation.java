...
public final class RandomInstrumentation {
  ...
  public static final int nextInt(Random rand, int i, int i_t, Ret ret) {
    if ((i_t & TaintValues.TRUST_MASK) != TaintValues.TRUSTED
      ...
      // If we had arbitrary precision, this number would be really big, so let's return a random
      // integer over all integers.
      NotifyMsg notifyMsg = new NotifyMsg("Random.nextInt(int)", "Random.nextInt(int)", 190);
      notifyMsg.setAction(RunChecks.REPLACE_ACTION);
      notifyMsg.append("Attempting to generate a random number between 0 and " + i + ", but " + i
          + " is " + TaintUtils.toTrustString(i_t)
          + " and has overflowed/underflowed.  Returning a random integer " + "over all integers.");
  ...
  @InstrumentationMethod(canExtend = true, instrumentationLocation = InstrumentationLocation.JDK,
      name = "nextInt", descriptor = "(IILpac/util/Ret;)I")
