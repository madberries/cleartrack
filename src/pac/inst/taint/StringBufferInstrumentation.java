...
public final class StringBufferInstrumentation {
  ...
  public static final StringBuffer append(StringBuffer sb, char c, int c_t, Ret ret)
      throws IOException {
    synchronized (sb) {
      if (c == 0 && (c_t & TaintValues.TAINTED) == TaintValues.TAINTED) {
        // CWE-626 Null Byte Interaction Error.
        int replaceChar = BaseConfig.getInstance().reportNullByte("StringBuffer.append(char)",
  ...
  @InstrumentationMethod(instrumentationLocation = InstrumentationLocation.APP)
  public static final StringBuffer insert(StringBuffer sb, int offset, int offset_t, char c,
      int c_t, Ret ret) throws IOException {
    synchronized (sb) {
      if (c == 0 && (c_t & TaintValues.TAINTED) == TaintValues.TAINTED) {
        // CWE-626 Null Byte Interaction Error.
        int replaceChar = BaseConfig.getInstance().reportNullByte("StringBuffer.insert(char)",
            "Attempting to insert a null character into the StringBuffer '" + sb + "' at index "
                + offset + ".");
        if (replaceChar == 0)
          return sb;
        if (replaceChar > 0) {
  ...
  @InstrumentationMethod(instrumentationLocation = InstrumentationLocation.APP)
  public static final void setCharAt(StringBuffer sb, int index, int index_t, char c, int c_t,
      Ret ret) throws IOException {
    synchronized (sb) {
      if (c == 0 && (c_t & TaintValues.TAINTED) == TaintValues.TAINTED) {
        // CWE-626 Null Byte Interaction Error.
        int replaceChar = BaseConfig.getInstance().reportNullByte("StringBuffer.setCharAt(char)",
            "Attempting to set a null character in the StringBuffer '" + sb + "' at index " + index
                + ".");
        if (replaceChar == 0)
          return;
        if (replaceChar > 0) {
