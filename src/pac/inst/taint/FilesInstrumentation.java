...
public class FilesInstrumentation {
  ...
  @InstrumentationMethod(invocationType = InvocationType.STATIC,
      instrumentationLocation = InstrumentationLocation.APP)
  public static final Path move(Path source, Path target, CopyOption[] options, Ret ret)
      throws IOException {
  ...
  private static final void checkValue(String method, String attrName, Object value,
      Object statValue) throws NoSuchMethodException, SecurityException {
    if (!value.equals(statValue)) {
      BaseConfig.getInstance().handleToctou(method,
    ...
    if (value instanceof FileTime) {
      ...
      if (key.equals("permissions")) {
        if (!stat.matchesPermissions((Set<PosixFilePermission>) value)) {
          BaseConfig.getInstance().handleToctou(method,
              "file attribute mismatch on 'permissions': " + value + " != " + stat);
        }
      }
      ...
      String statResult =
          String.format("(dev=%s,ino=%s)", Long.toHexString(stat.getDev()), stat.getInode());
  ...
  @InstrumentationMethod(invocationType = InvocationType.STATIC,
      instrumentationLocation = InstrumentationLocation.APP)
  public static final Object getAttribute(Path path, String attribute, LinkOption[] options,
      Ret ret) throws IOException, NoSuchMethodException, SecurityException {
  ...
  @InstrumentationMethod(invocationType = InvocationType.STATIC,
      instrumentationLocation = InstrumentationLocation.APP)
  public static final Map<String, Object> readAttributes(Path path, String attributes,
      LinkOption[] options, Ret ret) throws IOException, NoSuchMethodException, SecurityException {
  ...
  public static <A extends BasicFileAttributes> A readAttributes(Path path, Class<A> type,
    ...
    if (result instanceof PosixFileAttributes) {
      ...
      checkValue(method, "creationTime", fileAttr.creationTime().toMillis(),
          stat.creationTimePrecise());
      checkValue(method, "lastAccessTime", fileAttr.lastAccessTime().toMillis(),
          stat.lastAccessedPrecise());
      checkValue(method, "lastModifiedTime", fileAttr.lastModifiedTime().toMillis(),
          stat.lastModifiedPrecise());
      if (!stat.matchesPermissions(fileAttr.permissions())) {
        BaseConfig.getInstance().handleToctou(method,
            "file attribute mismatch on 'permissions': " + fileAttr + " != " + stat);
      }
