...
public final class MethodInstrumentation extends MemberInstrumentation {
  ...
  public static final Object[] getUninstrumentedArgs(Object[] args) {
    if (args == null || args.length == 0)
      return args;
    boolean hasTaintArgs =
        args[args.length - 1] != null && args[args.length - 1].getClass() == Ret.class;
    int len = hasTaintArgs ? args.length - 1 : args.length;
    // WHY MUST WE UNBOX THE INSTRUMENTED PRIMITIVE AND REBOX IT???
    // ============================================================
    // Since primitive wrapper classes are final, the native call must make this assumption about
    // these classes. Instead of checking for instanceof, it must check for class equality, for
    // example.
  ...
  protected static final boolean isPrimitiveClass(Class<?> klass) {
    return klass == boolean.class || klass == byte.class || klass == short.class
        || klass == char.class || klass == int.class || klass == float.class || klass == long.class
        || klass == double.class;
  }
  ...
  public static final Class<?>[] getUninstrumentedTypes(Class<?>[] parameterTypes) {
    ...
    for (int i = 0; i < length; i++) { // 8776382817
      ...
      if (newType == null) { // Either a primitive, or some object that is not a taint wrapped
                             // primitive array.
        if (isPrimitiveClass(oldType)) {
          newParamTypes.add(oldType);
          i++; // Skip the taint parameter.
        } else {
          newParamTypes.add(oldType);
        }
      } else {
  ...
  @InstrumentationMethod(inline = true,
      skippedDescriptor = "(Ljava/lang/Object;[Ljava/lang/Object;)Ljava/lang/Object;")
