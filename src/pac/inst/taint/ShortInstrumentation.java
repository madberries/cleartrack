...
public final class ShortInstrumentation {
    ...
    @InstrumentationMethod(invocationType = InvocationType.STATIC, name = "valueOf", descriptor = "(SILpac/util/Ret;)Ljava/lang/Short;")
    ...
    public static final short parseShort(String s, Ret ret) {
        ...
        try {
            ...
            if (TaintUtils.isTainted(s)) {
                NotifyMsg notifyMsg = new NotifyMsg("Short.parseShort(String)", "Short.parseShort(" + s + ")", 20);
                notifyMsg.setAction(RunChecks.REPLACE_ACTION);
                notifyMsg.append("Tainted string \"" + s + "\" is not a parseable short (returning the default value: "
                        + DEFAULT_VALUE + ")");
