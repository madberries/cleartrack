...
public final class DoubleInstrumentation {
    ...
    @InstrumentationMethod(invocationType = InvocationType.STATIC, name = "valueOf", descriptor = "(DILpac/util/Ret;)Ljava/lang/Double;")
    ...
    public static final double parseDouble(String s, Ret ret) {
        ...
        try {
            ...
            if (TaintUtils.isTainted(s)) {
                NotifyMsg notifyMsg = new NotifyMsg("Double.parseDouble(String)", "Double.parseDouble(" + s + ")", 20);
                notifyMsg.setAction(RunChecks.REPLACE_ACTION);
                notifyMsg.append("Tainted string \"" + s + "\" is not a parseable double (returning the default value: "
                        + DEFAULT_VALUE + ")");
