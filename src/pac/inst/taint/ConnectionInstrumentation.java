...
public final class ConnectionInstrumentation {
  ...
  public static final void setCatalog(Connection connection, String catalog) throws SQLException {
    ...
    try {
      if (!TaintUtils.isTrusted(catalog)) {
        // CWE-15 is "External control of system or configuration setting".
        final NotifyMsg notifyMsg =
            new NotifyMsg("Connection.setCatalog(String)", "Connection(" + catalog + ")", 15);
        ...
        if (BaseConfig.getInstance().canSetCatalogWithUntrustedData()) {
          ...
          notifyMsg.setExceptionConstructor(
              CleartrackException.class.getConstructor(new Class[] {String.class})); // NoSuchMethodException
  ...
  @InstrumentationMethod
  public static final void setClientInfo(Connection connection, Properties properties)
    ...
    try {
      ...
      if (strbuf.length() > 0) {
        // CWE-15 is "External control of system or configuration setting".
        final NotifyMsg notifyMsg =
            new NotifyMsg("Connection.setClientInfo(Properties)", "setClientInfo(Properties)", 15);
    ...
    try {
      ...
      if (errMsg.length() > 0) {
        final NotifyMsg notifyMsg = new NotifyMsg("Connection.setClientInfo(String,String)",
            "Connection(" + name + ", " + value + ")", 15); // CWE-15 is "External control of system
  ...
  @InstrumentationMethod
  public static final Savepoint setSavepoint(Connection connection, String name)
      throws SQLException {
    ...
    try {
      ...
      if (errMsg != null) {
        // CWE-15 is "External control of system or configuration setting".
        final NotifyMsg notifyMsg =
            new NotifyMsg("Connection.setSavepoint(String)", "Connection(" + name + ")", 15);
        notifyMsg.addTaintOutput(name);
      ...
      throw new RuntimeException(
          "Attack detected in call to Connection.setSavepoint(" + name + ")");
    }
  ...
  @InstrumentationMethod
  public static final void setTypeMap(Connection connection, Map<String, Class<?>> map)
      throws SQLException {
    ...
    try {
      ...
      if (errmsg.length() > 0) {
        final NotifyMsg notifyMsg = new NotifyMsg("Connection.setTypeMap(Map<String,Class<?>>)",
            "Connection(Map<String,Class<?>> map)", 15); // CWE-15 is "External control of system
                                                         // or configuration setting".
      ...
      throw new RuntimeException(
          "Attack detected in call to Connection.setTypeMap(Map<String,Class<?>>)");
    }
  }
