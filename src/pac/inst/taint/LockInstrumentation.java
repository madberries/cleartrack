...
public class LockInstrumentation {
    ...
    // Note: we only need to instrument lock() since unlock() remains the same,
    // and the other "try lock" mechanisms we assume the programmer will interrupt
    // the locks himself and do the right thing.
    //
    // If the programmer does not do the right thing and a deadlock were to occur,
    // our daemon thread will still catch it, but it would just not be reported as
    // a CWE.  We could write individual wrappers for the methods, if at some point
    // we feel the need to.
    ...
    @InstrumentationMethod(instrumentationLocation = InstrumentationLocation.APP, canExtend = true)
    public static final boolean tryLock(Lock lock, long time, TimeUnit unit) throws InterruptedException {
    ...
    public static final void unlock(Lock lock) {
        if (lock instanceof ReentrantLock) {
            // If this lock had not been previously lock, then presume this
            // was not the application intended and ignore the call.
            ...
            lock.unlock();
            if (curThread.ss_lobjs != null) {
                // If this thread no longer holds this lock, then we may safely
                // remove it from the list.
                if (!reLock.isHeldByCurrentThread())
                    curThread.ss_lobjs.remove(lock);
            }
        } else {
            // This may not always work, but assume that other locks are non-reentrant
            // and we really have no way of checking to see if the lock is held by the
            // current thread, for general locks.
            lock.unlock();
            Thread curThread = Thread.currentThread();
            if (curThread.ss_lobjs != null) {
                curThread.ss_lobjs.remove(lock);
