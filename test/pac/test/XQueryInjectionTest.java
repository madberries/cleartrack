...
public class XQueryInjectionTest {
    ...
    public void xqueryUsingString() throws XQException {
        if (XQUERY_SERVER == null || XQUERY_SERVER.equals("")) {
            System.err.println("XQUERY_SERVER environment variable is not set.  Skipping XQuery tests...");
            return;
        }
        ...
        // Nothing special for now... basically just ensure that normal
        // XPath style query attacks are caught.
        XQResultSequence rs = xqe.executeQuery(xqueryString);

        while (rs.next()) {
            String value = rs.getItemAsString(null);
            Assert.assertTrue("XQuery expression returned '" + value + "' when it " + " should have returned 'false'",
                              value.equals("false"));
        }
    ...
    public void testInappropriateEncoding() throws XQException {
        ...
        conn.close();
        Assert.assertNotNull("Stone soup exception was expected due to inappropriate encoding.", caughtEx);
    }
    ...
    public void XPathBlindExplorerInjections() throws IOException, XQException {
        ...
        for (String bookTitle : injections) {
            ...
            // String xqueryString = "doc('books.xml')//book/title/text()='" +
            // bookTitle + "'";

            String xqueryString = "for $x in doc('books.xml')//book/title[text()='" + bookTitle
            ...
            // Assert.fail("The entire XQuery should have been tainted, and so an exception was expected.");
        }
        conn.close();
    ...
    private boolean executeXQuery(XQExpression xqe, String xqueryString) throws XQException {
        ...
        try {
            ...
            Assert.assertTrue("XQuery expression returned '" + results + "' when it " + " should have returned nothing",
                              results.size() == 0);
            if (results.size() > 0) {
                injected = true;
