...
public class DataInputInstrumentationTest {
  ...
  public void readFullyTest() {
    ...
    Assert.assertTrue("tainted input not marked as tainted",
        ByteArrayTaint.isTainted(bytesRead, 0, 9));
    ...
    Assert.assertFalse("untainted range marked as tainted",
        ByteArrayTaint.isTainted(bytesRead2, 0, 2));
    Assert.assertTrue("tainted range not marked as tainted",
        ByteArrayTaint.isTainted(bytesRead2, 3, 7));
    Assert.assertFalse("untainted range marked as tainted",
        ByteArrayTaint.isTainted(bytesRead2, 8, 9));
    ...
    // Make sure the entire input is tainted
    Assert.assertTrue("unstrusted part of byte array marked as trusted",
        ByteArrayTaint.isTainted(bytesRead, 0, 5));
    Assert.assertTrue("trusted input not marked as trusted",
        ByteArrayTaint.isTrusted(bytesRead, 6, 9));
    ...
    Assert.assertFalse("untainted range marked as tainted",
        ByteArrayTaint.isTainted(bytesRead, 0, 2));
    Assert.assertTrue("untainted range marked as tainted",
        ByteArrayTaint.isTrusted(bytesRead, 3, 7));
    Assert.assertFalse("untainted range marked as tainted",
        ByteArrayTaint.isTainted(bytesRead, 8, 9));
  }
  ...
  public void readLineTest() {
    ...
    try {
      for (int i = 0; i < 3; i++) {
        ...
        // Make sure the entire input is tainted.
        Assert.assertTrue("tainted input not marked as tainted",
            TaintUtils.isTainted(s, 0, s.length()));
      }
    } catch (IOException e) {
      Assert.fail("error reading test bytes: " + e.getMessage());
    }
    ...
    try {
      for (int i = 0; i < 3; i++) {
        ...
        Assert.assertTrue("tainted input not marked as tainted",
            TaintUtils.isTrusted(s, 0, s.length() - 1)); // 0, s.length()));
      }
    } catch (IOException e) {
      Assert.fail("error reading test bytes: " + e.getMessage());
    }
  ...
  public void readUTFTest() {
    ...
    // Test that entire input is trusted.
    Assert.assertTrue("tainted input not marked as tainted",
        TaintUtils.isTrusted(s, 0, s.length()));
  }
}
