...
public class ProcessBuilderInstrumentationTest {
  ...
  // Test both ProcessBuilder.command(String...) and ProcessBuilder.command(String[]) with the same
  // data.
  private void doBothCommandShellCalls(final ArrayList<String> cmdRay, final String catchExErrMsg,
      final boolean expectThrow, final String cmdRayTwoErrMsg, final String testString,
      final boolean cmdRayTwoErrBool) {
    ...
    for (int i = 0; i < 2; i++) {
      ...
      if (cmdRayTwoErrMsg != null) {
        ...
        if (cmdRayTwoErrBool) {
          // If i == 0, there is no way to check if instrumented code altered the third arg.
          if (i != 0) {
            Assert.assertTrue(whichCommand + " " + cmdRayTwoErrMsg,
                testString.equals(cmd_list.get(2)));
          }
        } else {
          if (cmdRayTwoErrBool) {
            Assert.assertFalse(whichCommand + " " + cmdRayTwoErrMsg,
                testString.equals(cmd_list.get(2)));
          }
        }
      }
  ...
  public void nonShellTest() {
    ...
    TaintUtils.trust(cmdRay.get(1));
    TaintUtils.taint(cmdRay.get(2));
    doBothCommandShellCalls(cmdRay, "unexpected exception", false, // False means should not throw
                                                                   // exception.
        "failed to replace untrusted attack characters", // Null means do not check for altering
                                                         // cmdRay(2).
    ...
    Assert.assertTrue(
        "ProcessBuilder.start() failed throw RuntimeException from detecting taint $PATH component(s).",
        catchEx);
  }
  ...
  public void shellTest() {
    ...
    // ***
    // Run "csh -c $abc" where $abc expands to "ls *"
    cmdRay.add("$abc");
    TaintUtils.taint(cmdRay.get(2));
    ...
    Assert.assertTrue(
        "ProcessBuilder.start() failed throw RuntimeException from leaf $abc not in a safe directory",
        catchEx);
    cmdRay.remove(2);
    pb_1 = null;
    // ***
    ...
    // Run "csh -c pac"
    // "pac" exists in "bin/"
    // "bin/" is a trusted path component of $PATH that is not listed in config file.
    // trusted_exec_paths.
    ...
    if (procEnv.containsKey("PATH")) {
      ...
      Assert.assertTrue(
          "ProcessBuilder.start() failed to get expected Exception from dangerous \"bin\" path in $PATH",
          catchEx);
    }
    // ***
    ...
    doBothCommandShellCalls(cmdRay,
        "failed to throw exeption from shell's running untrusted command.", true, // True means
                                                                                  // should throw
                                                                                  // exception.
    ...
    doBothCommandShellCalls(cmdRay,
        "failed to throw exeption from shell's running untrusted command.", false, // False means
                                                                                   // should NOT
                                                                                   // throw
                                                                                   // exception.
    ...
    doBothCommandShellCalls(cmdRay,
        "failed to throw RuntimeException from running untrusted command after \";\".", true, null, // Unused
        null, // Unused
        true); // Unused
    // ***
    ...
    doBothCommandShellCalls(cmdRay, "erroneously threw RuntimeException", false, // Should be no
                                                                                 // exception.
        "failed to change untrusted replace illegal prefix blacklist ~/_tricks",
        "/bin/ls __/xyz |/bin/csh -c /bin/pwd;  /bin/ls _/_tricks", true); // cmdRay(2) should equal
                                                                           // the above param.
    // ***
  }
