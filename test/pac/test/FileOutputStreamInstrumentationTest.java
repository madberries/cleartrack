...
public class FileOutputStreamInstrumentationTest {
  ...
  public void fileTest() {
    ...
    Assert.assertTrue("FileOutputStream(String) did not alter path to __junk and create __junk",
        file.exists());
    // ***
    ...
    file = new File(pathName); // File Constructor changes pathName ..junk to __junk
    Assert.assertFalse("FileOutputStream(String, boolean) threw unexpected exception", caughtEx);
    Assert.assertTrue(
        "FileOutputStream(String, boolean) did not alter path to __junk and append to __junk",
        file.exists());
    file.delete();
    // ***
  }
  ...
  public void tooManyFilesTest() {
    ...
    Assert.assertNotNull("we should have thrown an exception when opening too many files",
        caughtEx);
  }
  ...
  public synchronized void tooManyFilesTest2() {
    ...
    // Wait for the file thread to notify the main thread that it has opened maxFiles minus 1 files.
    try {
      wait();
    ...
    // Now wait again for the file thread to be interrupted (or timeout).
    try {
      fileThread.join();
    } catch (InterruptedException e) {
  ...
  public class FileThread extends Thread {
    ...
    public void run() {
      ...
      try {
        ...
        // Now wait here until either their are too many files open or the main thread had
