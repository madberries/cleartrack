...
public class ClipboardTest implements ClipboardOwner {
  ...
  public void clipboardTest() {
    try {
      ...
      // Initially the clipboard contents should have been set from some outside source, so it
      // should be tainted.
      ...
      // Acquire the clipboard contents and check to make sure that the trust metadata matches that
      ...
      if (hasTransferableText) {
        try {
          String result = (String) contents.getTransferData(DataFlavor.stringFlavor);
          Assert.assertTrue("trust metadata from clipboard should be tainted",
