...
public class URLConnectionInstrumentationTest {
    ...
    private void readUrlStream(String urlPath, boolean shouldTrust) throws MalformedURLException, IOException {
        InputStream is = null;
        try {
            is = new URL(urlPath).openStream();
            Assert.assertTrue(shouldTrust ? "URLConnection InputStream not trusted"
                    : "URLConnection InputStream not tainted", TaintUtils.isTrusted(is.read()) == shouldTrust);
        } finally {
            if (is != null)
                is.close();
        }
