...
public class TokenizerTest {
  ...
  public void backslashEscapeTest() throws IOException {
    ...
    String qry = "SELECT * FROM table WHERE x='" + value + "' AND test='1';";
    compareTokens(tkz, qry, "SELECT * FROM table WHERE x='1'' OR ''1''=''1''; --' AND test='1';",
        new String[] {"SELECT", "*", "FROM", "table", "WHERE", "x", "=",
            "'1\\' OR \\'1\\'=\\'1'; --'", "AND", "test", "=", "'1'", ";"});
    ...
    compareTokens(tkz, qry,
        ...
        new String[] {"SELECT", "id", "FROM", "employees", "WHERE", "name", "=", "'fakeuser'",
            "AND", "password", "=",
            "'Demo12345\\' UNION ALL SELECT NULL\\-\\- AND \\'AOEC\\' LIKE \\'AOEC'", ";"});
    ...
    compareTokens(tkz, qry,
        ...
        new String[] {"SELECT", "id", "FROM", "employees", "WHERE", "name", "=", "'fakeuser'",
            "AND", "password", "=", "'Demo12345\\\"\\\"\' or 1 \\= 1 \\-\\- \\'\\'\\(\\(\\\"\\)'",
            ";"});
  }

  private void compareTokens(Tokenizer tkz, String qry, String expectedResult,
      String[] expectedTokens) throws IOException {
    ...
    Assert.assertEquals("The tokenizer failed to escape tainted quotes correctly", expectedResult,
        result);
    int i;
    for (i = 0; i < expectedTokens.length; i++) {
      ...
      Assert.assertEquals(
          "Expected token '" + expected + "' at index " + i + ", but received '" + tkn, expected,
          tkn);
    }
