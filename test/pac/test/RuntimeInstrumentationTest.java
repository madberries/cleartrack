...
public class RuntimeInstrumentationTest {
    ...
    public void nonShellExecCommandArrayTest() {
        ...
        try {
            ...
            // This is only a test to ensure that we are not applying must_trust
            // on the arguments to the java command... This executable doesn't
            // actually exist.
        }
    }
    ...
    public void pipedExecTest() throws IOException {
        ...
        try {
            ...
            Assert.assertTrue("command piped to process should return non-zero exit status", exitStatus != 0);
    ...
    public void testInappropriateEncoding() throws IOException {
        ...
        Assert.assertTrue("Runtime.exec(String) failed to throw RuntimeException due to inappropriate encoding.",
                          caughtEx);
    }
    ...
    public void execTest() {
        try {
            ...
            Assert.assertFalse("System.getenv(\"HOME\") returned a String having a trust level of UNKNOWN.",
            ...
            //                        "<shell-cmd> -c <command> param Any params, injected or not, are ignored
            ...
            Assert.assertTrue("Runtime.exec(String[]); String[2] is \"ls -l $HOME\"; $HOME expands to a tainted String starting wih slash. Slash is replaced by underscore",
                              cmdRay[2].startsWith("ls -l _"));

            // ***
            // Test "/bin/bash -c ls -l '$HOME'"   $HOME is NOT expanded because it is surrounded by single quotes.
            ...
            Assert.assertFalse("Runtime.exec(String[]) threw unexpected RuntimeException", caughtEx);
            Assert.assertTrue("Runtime.exec(String[]); String[2] is \"ls -l '$HOME'\"; $HOME is surrounded by single quotes and so should not expand.",
            ...
            Assert.assertTrue("Runtime.exec(String[]) expected to throw RuntimeException. Illegal untrusted name O\'Leary",
                              caughtEx);
            // ***
            ...
            Assert.assertTrue("Runtime.exec(String) failed to throw RuntimeException when semicolon \";\" command was not found in a trusted path",
                              caughtEx);
            // ***
            ...
            for (String str : attack_chars) {
                ...
                Assert.assertTrue("Runtime.exec(String[]) failed to replace attacking suffix blacklisted attack char",
            ...
            final String[] chars = { " ", "/", "~" }; // These three chars come from config file prefix_black in check filename_check
            ...
            Assert.assertTrue("Runtime.exec(String[]) failed to detect $HOME is illegal", newStr.equals(cmdRay[2]));
            // ***
            ...
            Assert.assertTrue("Runtime.exec(String[]). Expected RuntimeException from untrusted single quotes ",
                              caughtEx);
            // ***
            ...
            Assert.assertTrue("Runtime.exec(String) failed to detect shell running untrusted executable: /bin/ls",
                              caughtEx);
            // ***
            ...
            try {
                runtime.exec(cmdRay); // IOException
                //                                                                                  "/bin/ls ; /bin/pwd"
                // Assert.assertTrue("exec failed to remove injected semi-colon (1)", cmdRay[2].equals("/bin/ls __________"));
                // Assert.assertTrue("exec failed to change \"csh\" to full config file specification \"/bin/csh\"", "/bin/csh".equals(cmdRay[0]));
            } catch (RuntimeException ex) {
                caughtEx = true;
            }
            Assert.assertTrue("Runtime.exec(String[]) failed to throw RuntimeException while shell runs trusted command with dangerous data.\n"
                    + cmdRay[2], caughtEx);
            // ***

            // ***
            //   uses the special: execShellArrayElementTwo
            //   token 0 must_trust  - this should cause exception
            //                    _______
            ...
            Assert.assertTrue("Runtime.exec(String[]) failed to throw exeption from shell's running untrusted command.",
                              caughtEx);
            // ***
            ...
            Assert.assertFalse("Runtime.exec(String[]) threw inapropriate RuntimeException while shell runs trusted command with dangerous data.\n"
                    + cmdRay[2], caughtEx);
            // ***
            ...
            Assert.assertTrue("Runtime.exec(String[]) failed to throw IOException while shell runs trusted command with dangerous data.\n"
                    + cmdRay[2], caughtEx);
            // ***
            ...
            Assert.assertTrue("Runtime.exec(String[]) failed to throw exception from whitelist.\n" + cmdRay[2],
                              caughtEx);
            // ***
            ...
            Assert.assertTrue("Runtime.exec(String[]) failed to throw exception from exception from whitelist not matching \";\"\n"
                    + cmdRay[2], caughtEx);
            // ***
            ...
            Assert.assertTrue("Runtime.exec() failed to throw IOException trying to run untrusted command.", caughtEx);
            // ***
            ...
            try {
                ...
                Assert.fail("Runtime.exec() threw unexpect RuntimeException on leaf that is neither in trusted dir nor $PATH.");
            } catch (IOException ex) {
                caughtEx = true;
            }
            Assert.assertTrue("Runtime.exec() failed to throw IOException on leaf executable that not in trusted directory nor in $PATH.",
                              caughtEx);
            // ***
            ...
            */
            ...
            Assert.assertTrue("Runtime.exec(String) failed to throw RuntimeException from running a shell running an untrusted command.",
                              caughtEx);
            // ***
            ...
            TaintUtils.trust(cmdRay[2]);
            runtime.exec(cmdRay);
            Assert.assertTrue("Runtime.exec(String) did not change csh to /bin/csh", "/bin/csh".equals(cmdRay[0]));
            ...
            runtime.exec(cmdRay); // IOException
            Assert.assertTrue("Runtime.exec(String) did not change csh to /bin/csh", "/bin/csh".equals(cmdRay[0]));
            Assert.assertTrue("Runtime.exec(String). untrusted blacklisted parameters to trusted commands were not sanitized.",
            ...
            if (map != null) {
                ...
                Assert.assertTrue("Runtime.exec(String, String[]) failed to detect taint in $PATH", caughtEx);
            }
            // ***
            ...
            */
            ...
            Assert.assertTrue("Runtime.exec(String[]) failed to throw RuntimeException from running untrusted command after \";\".",
                              caughtEx);
            // ***
            ...
            Assert.assertTrue("Runtime.exec(String[]) failed to change untrusted replace illegal prefix blacklist ~/_tricks",
            ...
            Assert.assertTrue("Runtime.exec(String[] erroneously altered the single untrusted escaped quote file name",
            ...
            Assert.assertTrue("Runtime.exec(String[] either failed to reassemble tokens or failed to detect whitespace",
    ...
    //
        ...
        Assert.assertFalse("A valid file name containing spaces should not be intervened by cleartrack", caught);
    }
    */
    ...
    public void testHOMEVAR() {
        ...
        Assert.assertTrue("$HOME environment variable is tainted. Should be sanitized or caught by cleartrack",
    ...
    public void validCommands() {
        ...
        String command = String.format("echo %s > %s;ls %s&&cat %s|grep %s", file_name, file_name, file_name, file_name,
                                       file_name);
        execCommand(command);

        ////////////////////////////////////////////////////////////////////////////////////////////
        command = String.format("echo %s >> %s;ls %s&cat %s|grep %s", file_name, file_name, file_name, file_name,
                                file_name);
        execCommand(command);

        command = String.format("echo %s > %s 2> File_Name.err ;ls %s&cat %s|grep %s", file_name, file_name, file_name,
                                file_name, file_name);
        execCommand(command);

        command = String.format("echo %s 2>&1 %s;ls %s&cat %s|grep %s", file_name, file_name, file_name, file_name,
                                file_name);
        execCommand(command);
        ...
        Assert.assertFalse("% is a trusted character. Only character Y is tainted and this command should be allowed to execute",
                           caught);
    ...
    /**
     * Other Commands and some combinations
     * Vars. PATH, TERM, SHELL, USER, PWD, DISPLAY
     *
     *  echo "File_Name" > File_Name;ls File_Name&&cat File_Name|grep File_Name
     *  echo "File_Name" > File_Name;ls File_Name&cat File_Name|grep File_Name
     *
     */
    @Test
    ...
    public void testTickCommand() {
        ...
        System.out.println("Testing command: " + command + " " + untrusted);
        cmd[2] = command + untrusted;
        //      TaintUtils.taint(untrusted);// Is setting tainted data after cmd[2] has been initialized cheating?
        boolean caught = false;
        try {
            execCommands(cmd);
        } catch (CleartrackException e) {
