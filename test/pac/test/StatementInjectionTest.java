...
public class StatementInjectionTest {
  ...
  public static final String TEST_TABLE_COLUMNS[] =
      {"id", "VARCHAR(20)", "count", "INTEGER(10)", "name", "VARCHAR(20)", "hired", "DATE", "email",
          "VARCHAR(30)", "position", "VARCHAR(20)", "password", "VARCHAR(20)"};
  ...
  /**
   * Sets up the database that will be used for all unit tests. Instantiates an embedded SQL server,
   * loads the appropriate client JDBC driver, and populates the database with tables of test data.
   * 
   * @throws SQLException on error creating test database.
   * @throws ClassNotFoundException on missing JDBC driver.
   */
  @BeforeClass
   ...
   * @param columns Even elements are column names. Odd elements are the type of the preceding named
   *        column.
   * @throws SQLException
   */
   ...
   * 
   * @param stmt Statement for executing SQL commands.
   * @param tableName Name of the table in the database.
   * @param data An array of row data arrays. Each sub-array is the column data for one row in the
   *        table.
   * @throws SQLException
   */
  private void populateTable(Statement stmt, String tableName, Object[][] data)
      throws SQLException {
  ...
  public String lookupEmail(final String TEST_NAME) throws SQLException {
    ...
    stmt = connection.createStatement();
    String query =
        String.format("SELECT email FROM %s WHERE name='%s';", TEST_TABLE_NAME, TEST_NAME);
  ...
  public static ArrayList<String> executeSqlStatement(Connection connection, String sql)
      throws SQLException {
  ...
  public String verifyPasswordNoQuotes(final String TEST_NAME, final String TEST_PASSWORD)
      throws SQLException {
    Statement stmt = connection.createStatement();
  ...
  public String verifyPasswordNonString(final String TEST_NAME, final String TEST_PASSWORD)
      throws SQLException {
    String resultString = null;
    Statement stmt = null;
    ResultSet rs = null;
    ...
    stmt = connection.createStatement();
    String query = String.format("SELECT id FROM %s WHERE name='%s' OR count=%s;", TEST_TABLE_NAME,
  ...
  /**
   * Given a test name and a tag, retrieves the corresponding query from the file of queries.
   */
  String getQueryFromFile(String testName, String tag) {
    ...
    try {
      ...
      /*
       * Find the (first) line with matching test name and tag. The lines have the format:
       * 
  ...
  public void testSetCatalog() {
    // Unfortunately we must skip this if MySQL is not set up, since H2 does not support some of
    // these features.
    ...
    try {
      ...
      Assert.assertTrue(
          "Connection.setCatalog(String) setting catalog with tainted string should throw StoneSouopException",
          caughtEx);
      String newCatalogName = connection.getCatalog();
      Assert.assertTrue(
          "Connection.setCatalog(String) setting catalog with tainted string should not have altered connection's current catalog",
      ...
      Assert.assertTrue(
          "Connection.setCatalog(String) setting catalog to a non-existant name failed to throw a connect exception",
          caughtEx);
      newCatalogName = connection.getCatalog();
      Assert.assertFalse(
          "Connection.setCatalog(String) setting catalog with trusted string should not have altered connection's current catalog",
      ...
      Assert.assertTrue(
          "Connection.setClientInfo(Properties) property \"propName\" is tainted and should thrown CleartrackException",
          caughtEx);
      Assert.assertTrue(
          "Connection.setClientInfo(Properties) property \"propName\" is tainted and should not have been set",
          val == null);
      ...
      try {
        ...
        Assert.fail(
            "Connection.setClientInfo(Properties) was called with trusted properties. Should not have thrown StroneSoupException");
      }
      String newVal = connection.getClientInfo("propName_2");
      Assert.assertTrue(
          "Connection.setClientInfo(Properties) Properties is trusted and should have been set",
          newVal.equals(value));

      // ********
      // **** Connection.setClientInfo(String, String)
      // ***
      // Test Connection.setClientInfo(String, String) test that untrusted name and/or value will
      // throw an exception.
      name = "myPropName";
      TaintUtils.taint(name);
      value = "myPropValue";
      ...
      Assert.assertTrue(
          "Connection.setClientInfo(String,String) name and/or value are tainted. Should have thrown exception",
          caughtEx);
      newVal = connection.getClientInfo(name);
      Assert.assertFalse(
          "Connection.setClientInfo(String,String) name and/or value are tainted. property should not have been set",
          name.equals(newVal));

      // ***
      // Test Connection.setClientInfo(String, String) test that trusted name and/or value alters.
      name = "myPropName_2";
      TaintUtils.trust(name);
      value = "myPropValue_2";
      TaintUtils.trust(value);
      ...
      Assert.assertFalse(
          "Connection.setClientInfo(String,String) name and value are trusted. Should not have thrown exception",
          caughtEx);
      newVal = connection.getClientInfo(name);
      Assert.assertTrue(
          "Connection.setClientInfo(String,String) name and/or value are trusted. property should have been set",
          value.equals(newVal));

      // ******
      // *** setSavepoint(Connection connection, String name)
      // ***
      // Test setSavepoint(String name) test that tainted name does nothing.
      ...
      Assert.assertTrue(
          "Connection.setSavepoint(String) name is tainted. Should have thrown exception",
          caughtEx);
      Assert.assertTrue(
          "Connection.setSavepoint(String) name is tainted. Should not have created a savepoint",
          savepoint == null);

      // ***
      // Test setSavepoint(String name) test that trusted name creates a savepoint.
      name = "savepointName";
      TaintUtils.trust(name);
      caughtEx = false;
      try {
      ...
      Assert.assertFalse(
          "Connection.setSavepoint(String) name is trusted. Should NOT have thrown exception",
          caughtEx);
      Assert.assertFalse(
          "Connection.setSavepoint(String) name is trusted. Should have created a savepoint",
          caughtEx);

      // ******
      // *** setTypeMap(Map<String,Class<?>>
      // ***
      // *** Test setTypeMap(Map<String,Class<?>> with untrusted keys in map.
      ...
      Assert.assertTrue(
          "Connection.setTypeMap(Map) contains unrusted keys. Should have thrown exception",
          caughtEx);
      ...
      Assert.assertFalse(
          "Connection.setTypeMap(Map) contains tusted keys. Should not have thrown exception",
          caughtEx);
      map = connection.getTypeMap(); // SQLException
      Assert.assertFalse(
          "Connection.setTypeMap(Map) set map with trusted keys. Connection.getMap should have returne the map",
          map == null);
  ...
  public void testNumberOfEmployees() throws SQLException {
    ...
    Assert.assertEquals("Unexpected number of employees in database.", resultNum,
        TEST_TABLE_DATA.length);
  }
  ...
  public void testBenignQuery() throws SQLException {
    ...
    if (result) {
      ...
      if (rs.next()) {
        ...
        Assert.assertTrue("int obtained from SQL query should be tainted",
            TaintUtils.isTainted(resultInt));
      }
    }
  ...
  public void testSafeLookupEmail() throws SQLException {
    ...
    Assert.assertEquals("Query for employee email address did not return expected result.",
        resultString, TEST_EMAIL);
  }
  ...
  public void testInappropriateEncoding() throws SQLException, EncodingException {
    ...
    Assert.assertNotNull("Query expected a stone soup exception due to inappropriate encoding.",
        caughtEx);
  ...
  public void testInjectNonString1() {
    ...
    Assert.assertNull("Password query returned a result for a bogus non-string count.",
        resultString);
  }
  ...
  Connection getPostgresqlConnection() throws SQLException {
    ...
    stmt.execute(
        "CREATE TABLE employees( id VARCHAR(20),  count INTEGER,  name VARCHAR(20),  hired DATE,  email VARCHAR(30),  position VARCHAR(20),  password VARCHAR(20));");
    // Populate the test table.
    populateTable(stmt, TEST_TABLE_NAME, TEST_TABLE_DATA);
    stmt.close();
  ...
  Connection getMSSqlConnection() throws SQLException {
    ...
    conn = DriverManager
        .getConnection("jdbc:sqlserver://" + MSSQL_SERVER + ";user=cleartrack;password=test");
  ...
  public void connectionPostgreSql() throws SQLException {
    if (PSQL_SERVER == null || PSQL_SERVER.equals("")) {
      System.err
          .println("PSQL_SERVER environment variable is not set.  Skipping PostGreSQL tests...");
    } else {
  ...
  public void connectionMSSqlServer() throws SQLException {
    if (MSSQL_SERVER == null || MSSQL_SERVER.equals("")) {
      System.err.println(
          "MSSQL_SERVER environment variable is not set.  Skipping Microsoft SQL Server tests...");
    } else {
  ...
  /*
   * Test sql injections strings for Blind Boolean, Error, Union, Stacked and Timed.
   */
  ...
  public void directSqlTest() throws SQLException {
    useMysql();// Mysql appears to be running faster than h2, but it shouldn't because cleartrack
               // intercepts all sql statements and throws an exception even before the sql
               // statement reaches DB specific code. // usePostgres();
    boolean enabled = isInstrumentationEnabled();
    // Few sql statements are sufficient to test completely tainted sql statements.
    HashMap<String, String> fileList = StatementInjectionTest.getInjectionDataSmallSet();
    Statement stmt = connection.createStatement();
  ...
  public void useMysql() throws SQLException {
    if (MYSQL_SERVER == null || MYSQL_SERVER.equals("")) {
      System.err
          .println("MYSQL_SERVER environment variable is not set.  Continue using H2 database...");
      return;
    }
    ...
    for (List<String> adata : rows) {
      ...
      for (Map.Entry<String, Statement> adb : dbs.entrySet()) {
        ...
        if (continueWithAllDbsQuoted) {
          ...
          } catch (CleartrackException sse) {// Cleartrack rejected.
            cleartrackExceptions++;
            continueWithAllDbsQuoted = false;
        ...
        if (continueWithAllDbsNonQuoted) {
          ...
          try {
            resultString = null;
            resultString = verifyPasswordNoQuotes(name, pass, adb.getValue());// Cleartrack should
                                                                              // only allow numbers.
            sqlExecuted++;
            Assert.assertTrue(
                "Cleartrack should only allow numbers for non quoted password. But it allowed: "
                    + pass,
                isPasswordNumber);
            // sqlExecutedb = true;
          } catch (PSQLException e) {
            if (isPasswordNumber) {// Postgres will throw an exception if password is a number.
                                   // Suppress it.
                                   // System.out.println("Postgres exception: " + e.getMessage());
  ...
  static public HashMap<String, String> getInjectionData() {
    ...
    injectiontypes.put("Sql Boolean Blind",
        "test/pac/test/sqlinjection/sql-inject-data/sql-boolean-blind.tsv");
  ...
  boolean testInjections(List<List<String>> rows) {
    ...
    for (List<String> adata : rows) {
      ...
      if (func.equalsIgnoreCase("verifyPassword")) {
        ...
        try {
          resultString = null;
          resultString = verifyPasswordNoQuotes(name, pass); // Cleartrack should only allow
                                                             // numbers.
          sqlExecuted++;
          Assert.assertTrue(
              "Cleartrack should only allow numbers for non quoted password. But it allowed: "
          ...
          if (isPasswordNumber) {// Postgres will throw an exception is password is a number.
                                 // Suppress it.
                                 // System.out.println("Postgres exception: " + e.getMessage());
          } else {
            Assert.assertTrue("There should not be sql exception for non quoted password", false);
  ...
  public void testSanitiseFunctionInjection() throws SQLException {
    ...
    try {
      ...
      // If diversity is on, we should expect a cleartrack exception.
    }
  ...
  // Injection data sanitized by ESAPI should execute without cleartrack intervention.
  @Test
    ...
    for (List<String> adata : rows) {
      ...
      if (func.equalsIgnoreCase("verifyPassword")) {
        ...
        try {
          ...
          System.out.println("SSE; " + sse.getMessage());
          Assert.assertTrue(
              "There should not be CleartrackException Exception for Sanitized data: pass = "
                  + pass,
              false);
        }
