...
public class PropertiesInstrumentationTest {
    ...
    public void standardPropertiesTest() {
        ...
        // we reset this property to "root" from the command 
        // line, so this value should be tainted
    ...
    private void assertEnvVarTrust(String envVar, boolean trust) {
        ...
        if (value == null)
            return;
        String trustStr = trust ? "TRUSTED" : "TAINTED";
        Assert.assertTrue(envVar + " environment variable should be " + trustStr, TaintUtils.isTrusted(value) == trust);
    }
    ...
    public void propertiesTest() {
        try {
            ...
            while (keys.hasMoreElements()) {
                ...
                Assert.assertTrue("property read from untrusted Reader. The propterty should NOT be trusted",
            ...
            while (keys.hasMoreElements()) {
                final String key = (String) keys.nextElement();
                final String val = props.getProperty(key);
                Assert.assertFalse("property read from untrusted InputStream. The propterty should NOT be trusted",
                                   TaintUtils.isTrusted(val));
            }
