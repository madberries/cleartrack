...
public class XPathInstrumentationTest {
    ...
    public void verifyPasswordTest() {
        ...
        try {
            XPathExpression xlogin = xpath.compile("//users/user[login/text()='" + login + "' and password/text() = '"
                    + pass + "']/home_dir/text()");
            Document d = DocumentBuilderFactory.newInstance().newDocumentBuilder()
                    .parse(new FileInputStream("test/pac/test/users.xml"));

            String homedir = xlogin.evaluate(d);
            Assert.assertTrue("Bypassed XPath query using bogus password.", homedir == null || homedir.length() == 0);
        } catch (Exception e) {
