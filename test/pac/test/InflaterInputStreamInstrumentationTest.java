...
/**
 * Tests to ensure that the constructors to InflaterInputStream properly show taint when based on a
...
public class InflaterInputStreamInstrumentationTest {
  ...
  public void constructerTest() {
    try {
      ...
      str = TaintUtils.trust("/usr/bin/less"); // All InputStreams created from trusted location
                                               // will be trusted.
      f = new File(str);
      fis = new FileInputStream(f);
      dis = new DeflaterInputStream(fis);
      iis = new InflaterInputStream(dis);
      Assert.assertTrue("InflaterInputStream based on file should be trusted",
          TaintUtils.isTrusted(iis.read()));
