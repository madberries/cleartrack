...
public class FileInstrumentationTest {
  ...
  private void ensureTaintEquals(File tempFile, String tmpDir, String prefix, String suffix) {
    ...
    Assert.assertEquals(
        "path prefix '" + pathPrefix + "' not found, or is not a prefix of '" + path + "'", 0,
        start);
    int end = path.indexOf(suffix);
    Assert.assertFalse(
        "path suffix '" + suffix + "' not found, or is not a suffix of '" + path + "'",
    ...
    // One of the slashes appears to come from a property, where the other is a string constant (and
    // therefore has no taint source). So, let's just compare with a taint mask.
  ...
  public void fileTest() {
    ...
    Assert.assertTrue("File(String) did not replace untrusted attack ../test",
        file.getPath().equals("__/test"));
    // ***

    // ***
    // Test constructor File("test") - "test" is trusted.
    // Test that returned File's toString() is tainted.
    pathName = TaintUtils.trust("test");
    try {
      file = new File(pathName);
    } catch (RuntimeException ex) {
    ...
    Assert.assertTrue("File(File, String) incorrectly altered path",
        "/usr/bin/fax".equals(file.toString()));
    ...
    Assert.assertTrue("File(File, String) failed to alter path",
        "_usr/bin/fax".equals(file.toString()));
    ...
    Assert.assertFalse("File(String, String) failed to trust File path location",
        TaintUtils.isTainted(file.getPath()));
    Assert.assertTrue("File(String, String) incorrecttly altered path",
        file.getPath().equals("/usr/bin/fax"));
    // ***
    ...
    // ***
    pathName_2 = TaintUtils.trust("/afs/csail");
    pathName = TaintUtils.trust("lib"); // Note: Can't user "conifg_inst" or "bin" again because
                                        // getCononicalPath will return a path that already.
    file = new File(pathName);
    try {
  ...
  public void sanityTest() {
    ...
    try {
      ...
      Assert.assertEquals("canExecute() calls should match", expected.canExecute(),
          file.canExecute());
      ...
      Assert.assertEquals("lastModified() calls should match", expected.lastModified(),
          file.lastModified());
