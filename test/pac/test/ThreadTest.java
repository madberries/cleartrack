...
public class ThreadTest {
  ...
  public void tooManyLocksTest() {
    for (int i = 0; i < 1; i++) {
      ...
      Assert.assertFalse("we failed to unlock an improperly locked thread",
  ...
  public void improperThreadStart() {
    ...
    thread.run(); // Should return execution immediately if properly calling Thread.start().
  ...
  public void verifyTest() {
    // Just keep this test to ensure that there is no runtime verify error on methods that are
    // synchronized.
  ...
  /**
   * Queries the Java runtime for the bytes used by the JVM over the maximum heap space allowed by
  ...
  private class DoubleLockedThread extends Thread {
    ...
    public void run() {
      ...
      try {
        while (!finished) {
          // Make sure we only print every few iterations, so that we dont get a java heap space
          // error.
          createWork(1000); // Create some busy work.
          synchronized (lock1) {
            createWork(1000); // Create some more busy work.
            synchronized (lock2) {
              createWork(1000); // Create yet more busy work.
            }
          }
          i++;
  ...
  private class OverAllocThread extends Thread {
    ...
    public void run() {
      ...
      try {
        // Ensure that our memory allocations will eventually run out of memory.
        int i = 0;
        final int timeout = 10;
        final boolean catchGeneric = rand.nextBoolean();
        double used = getUsedSpace(runtime);
        // We need to keep looping past 75%, so that we eventually find that the thread has been
        // interrupted.
        int secs = 0;
          ...
          if (used < .8f) { // Stop allocating after this point.
            ...
            if (!catchGeneric || secs == timeout) {
              ...
              try {
                ...
                // Even though we may catch the CleartrackException here, we will reset the
                // interrupt bit and catch it on the next iteration.
