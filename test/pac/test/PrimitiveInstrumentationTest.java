...
public class PrimitiveInstrumentationTest {
  ...
  public void stackFrameTest() {
    // ASM does not compute stack frames correctly, when merging between dangerous types and their
    // respective subtype.
    Integer x;
    if (rand.nextBoolean())
  ...
  private void testAllConstant(boolean boolVal, byte byteVal, short shortVal, char charVal,
      int intVal, float floatVal, long longVal, double doubleVal) {
  ...
  public void testConstants() throws IllegalArgumentException, IllegalAccessException,
    ...
    testAllConstant(TEST_BOOL, TEST_BYTE, TEST_SHORT, TEST_CHAR, TEST_INT, TEST_FLOAT, TEST_LONG,
        TEST_DOUBLE);
    ...
    double doubleVal2 =
        (Double) PrimitiveInstrumentationTest.class.getField("TEST_DOUBLE").get(null);
    testAllConstant(boolVal2, byteVal2, shortVal2, charVal2, intVal2, floatVal2, longVal2,
        doubleVal2);
  }
  ...
  private void overflowUnderflowIntegerTest(Operation operation) {
    ...
    for (int i = 0; i < total_ops; i++) {
      ...
      // Check for an overflow / underflow.
      if (result.compareTo(maxInt) > 0) {
        Assert.assertTrue("We should have overflowed on " + aInt + " " + operation.getOp() + " "
            + bInt + " = " + result, TaintUtils.isOverflowSet(resultInt));
        overflowed++;
      } else if (result.compareTo(minInt) < 0) {
        Assert.assertTrue("We should have underflowed on " + aInt + " " + operation.getOp() + " "
            + bInt + " = " + result, TaintUtils.isUnderflowSet(resultInt));
        underflowed++;
      } else {
        Assert.assertFalse(
            "We should not have overflowed or underflowed on " + aInt + " " + operation.getOp()
                + " " + bInt + " = " + result,
            TaintUtils.isOverflowSet(resultInt) || TaintUtils.isUnderflowSet(resultInt));
      }
    }
    System.out.println(
        "Tested " + operation + " operation on " + total_ops + " pairs of random integers ("
    ...
    for (int i = 0; i < total_ops; i++) {
      ...
      if (result.compareTo(maxInt) > 0) {
        ...
        if (operation == Operation.multiply) {
          Assert.assertTrue(
              "Overflowed/underflowed result should been replaced with OVERFLOW/UNDERFLOW",
              "OVERFLOW".equals("" + resultInt) || "UNDERFLOW".equals("" + resultInt));
        } else {
          Assert.assertEquals("We should have replaced overflowed value with \"OVERFLOW\"",
              "" + resultInt, "OVERFLOW");
        }
        overflowed++;
      } else if (result.compareTo(minInt) < 0) {
        ...
        if (operation == Operation.multiply) {
          ...
          Assert.assertEquals("We should have replaced underflowed value with \"UNDERFLOW\"",
              "" + resultInt, "UNDERFLOW");
        }
        underflowed++;
      } else {
