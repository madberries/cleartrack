...
public class FilesInstrumentationTest {
    ...
    public void fileTest() {
        ...
        Assert.assertTrue("File(File, String) failed to replace untrusted attack path", pathName.equals("_src/csail"));
        // ***
        ...
        Assert.assertTrue("File(File, String) File param is tainted, so returned File should be tainted",
                          TaintUtils.isTainted(path.toFile().getPath()));
        Assert.assertTrue("File(File, String) failed to alter path", "_usr/bin/fax".equals(path.toFile().toString()));
    ...
    public void sanityTest() {
        ...
        try {
            Assert.assertEquals("isWritable() calls should match", expected.isWritable(), Files.isWritable(path));
            Assert.assertEquals("isReadable() calls should match", expected.isReadable(), Files.isReadable(path));
            Assert.assertEquals("isExecutable() calls should match", expected.isExecutable(), Files.isExecutable(path));
            Assert.assertEquals("isDirectory() calls should match", expected.isDirectory(), Files.isDirectory(path));
            ...
            Assert.assertEquals("notExists() calls should match", expected.notExists(), Files.notExists(path));
            Assert.assertEquals("isSymbolicLink() calls should match", expected.isSymbolicLink(),
                                Files.isSymbolicLink(path));
            Assert.assertEquals("getLastModifiedTime() calls should match", expected.getLastModifiedTime(),
                                Files.getLastModifiedTime(path));
            Assert.assertEquals("getPosixFilePermissions() calls should match", expected.getPosixFilePermissions(),
            ...
            // TODO need to handle uid and gid first, before I can enable these checks
    ...
    private void checkAttribute(FileInfo expected, Path path, String attribute) throws IOException {
        ...
        Assert.assertEquals("getAttribute(\"" + attribute + "\") calls should match", expected.getAttribute(attribute),
